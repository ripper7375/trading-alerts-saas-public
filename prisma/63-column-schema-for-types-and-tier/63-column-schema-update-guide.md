# 63-Column Database Schema Update Guide

**Version:** 4.0.0
**Last Updated:** 2026-02-24
**Supersedes:** `61-column-schema-update-guide.md` (v3.0.0, 2026-02-11)
**Purpose:** Comprehensive guide for updating any part of the codebase to align with the 63-column database schema — adds `ha_body_zscore` and `body_classification`, corrects 10 KC band names, renames `ha_strength` → `ha_body_size`, `ha_classification` → `ha_color`, S/R columns → `sr_1–8`, `zigzag_peak/bottom` → `zigzag_high/low`
...
### Schema Changes (MQL5 indicator export alignment)

Seven schema corrections made to align Prisma column names with actual MQL5 indicator export headers:

| Change                            | Type          | Description                                                                                  |
| --------------------------------- | ------------- | -------------------------------------------------------------------------------------------- |
| **10 KC columns renamed**         | Column rename | Placeholder names → correct ATR-band names (kc_ultra_extreme_upper … kc_ultra_extreme_lower) |
| **ha_strength renamed**           | Column rename | `ha_strength` → `ha_body_size`                                                               |
| **ha_body_zscore added**          | New column    | Heiken Ashi body size Z-score (PRO tier)                                                     |
| **body_classification added**     | New column    | Candle body classification integer (PRO tier, Body Momentum group)                           |
| **ha_classification → ha_color**  | Column rename | 0-5 enum → 1/-1 integer (bullish/bearish)                                                    |
| **sr_support/resistance renamed** | Column rename | sr_support_1-4 → sr_1-4 · sr_resistance_1-4 → sr_5-8                                         |
| **zigzag_peak/bottom renamed**    | Column rename | `zigzag_peak` → `zigzag_high` · `zigzag_bottom` → `zigzag_low`                               |

### Column Count Summary

| Category             | v2.0 (60-col) | v3.0 (61-col) | v4.0 (63-col) | Delta v3→v4 |
| -------------------- | ------------- | ------------- | ------------- | ----------- |
| System columns       | 8             | 9             | **9**         | —           |
| FREE tier indicators | 16            | 16            | **16**        | —           |
| PRO tier indicators  | 36            | 36            | **38**        | **+2**      |
| **Total**            | **60**        | **61**        | **63**        | **+2**      |
...
### What is the 63-Column Schema?

**Current Schema (63-column flat structure):**

```typescript
{
  // System columns (9) — unchanged from v3.0
  ...
  // PRO tier indicators (38 columns) — 2 added, many renamed in v4.0
  // Group 1: Moving Averages (3)
  tema: 43260,
  hrma: 43255,
  smma: 43258,
  // Group 2: Body Momentum (3) ← was 2; body_classification ADDED in v4.0
  body_direction: 1,
  body_size: 0.75,
  body_classification: 0,       // ← NEW in v4.0
  // Group 3: Heiken Ashi (8) ← was 7; ha_body_zscore ADDED, ha_strength RENAMED in v4.0
  ha_open: 43262,
  ha_high: 43300,
  ha_low: 43200,
  ha_close: 43278,
  ha_color: 1,                  // ← RENAMED from ha_classification (was 0-5 enum)
  ha_trend: 1,
  ha_body_size: 0.85,           // ← RENAMED from ha_strength in v4.0
  ha_body_zscore: -0.42,        // ← NEW in v4.0
  // Group 4: Keltner Channel (10) ← band names CORRECTED in v4.0
  kc_ultra_extreme_upper: 43420,
  kc_extreme_upper: 43385,
  kc_uppermost: 43350,
  kc_upper: 43315,
  kc_upper_middle: 43280,
  kc_lower_middle: 43260,
  kc_lower: 43225,
  kc_lowermost: 43190,
  kc_extreme_lower: 43155,
  kc_ultra_extreme_lower: 43120,
  // Group 5: Support/Resistance (8) ← names CORRECTED in v4.0
  sr_1: null,                   // sr_support_1 (support closest)
  sr_2: null,                   // sr_support_2
  sr_3: null,                   // sr_support_3
  sr_4: null,                   // sr_support_4
  sr_5: 43300,                  // sr_resistance_1 (resistance closest)
  sr_6: 43350,                  // sr_resistance_2
  sr_7: 43400,                  // sr_resistance_3
  sr_8: 43450,                  // sr_resistance_4
  // Group 6: ZigZag + EMA (3) ← names CORRECTED in v4.0
  zigzag_high: 43300,           // ← RENAMED from zigzag_peak in v4.0
  zigzag_low: 43200,            // ← RENAMED from zigzag_bottom in v4.0
  ema: 43257,
  // Group 7: Dual TEMA H/L (2)
  dual_tema_high: 43285,
  dual_tema_low: 43245,
  // Group 8: Pinbar Detection (1)
  pinbar: 0,
}
```
...
- **From:** 61-column flat structure (9 system + 16 FREE + 36 PRO)
- **To:** 63-column flat structure (9 system + 16 FREE + 38 PRO)
- **Tier Access:**
  - FREE tier: **25 columns** (9 system + 16 FREE indicators) — unchanged from v3.0
  - PRO tier: **63 columns** (9 system + 16 FREE + 38 PRO indicators) — was 61
...
### PRO Tier Indicators (38 columns) — +2 added in v4.0, many renamed

**Group 1: Moving Averages (3 columns) — unchanged**
...
**Group 2: Body Momentum (3 columns) — was 2; body_classification ADDED**

```typescript
{
  body_direction: number | null; // 1 = bullish, -1 = bearish, 0 = doji
  body_size: number | null; // Z-score of candle body size
  body_classification: number | null; // ← NEW in v4.0 (0-5 enum, see Indicator #2)
}
```

**Group 3: Heiken Ashi (8 columns) — was 7; ha_body_zscore ADDED, renames**
...
```typescript
{
  ...
  ha_color: number | null; // ← RENAMED from ha_classification (1/-1 integer)
  ha_trend: number | null; // streak count with sign
  ha_body_size: number | null; // ← RENAMED from ha_strength in v4.0
  ha_body_zscore: number | null; // ← NEW in v4.0
}
```

**Group 4: Keltner Channels (10 columns) — band names CORRECTED in v4.0**

```typescript
{
  kc_ultra_extreme_upper: number | null; // HRMA + 4.0×ATR
  kc_extreme_upper: number | null; // HRMA + 3.0×ATR
  kc_uppermost: number | null; // HRMA + 2.0×ATR
  kc_upper: number | null; // HRMA + 1.0×ATR
  kc_upper_middle: number | null; // HRMA (centre, upper)
  kc_lower_middle: number | null; // HRMA (centre, lower)
  kc_lower: number | null; // HRMA - 1.0×ATR
  kc_lowermost: number | null; // HRMA - 2.0×ATR
  kc_extreme_lower: number | null; // HRMA - 3.0×ATR
  kc_ultra_extreme_lower: number | null; // HRMA - 4.0×ATR
}
```

**Groups 5-8:** (Column counts unchanged from v3.0 — names corrected)

- Group 5: Support/Resistance (8 columns) — sr_support_1-4 → sr_1-4, sr_resistance_1-4 → sr_5-8
- Group 6: ZigZag + EMA (3 columns) — zigzag_peak → zigzag_high, zigzag_bottom → zigzag_low
- Group 7: Dual TEMA High/Low (2 columns) — unchanged
- Group 8: Pinbar Detection (1 column) — unchanged
...
**CRITICAL:** All renamed columns must be updated everywhere — types, constants, queries, tests!

**v4.0 renames (61 → 63):**

| Old Column Name (v3.0) | New Column Name (v4.0)   | Type      | Notes                                        |
| ---------------------- | ------------------------ | --------- | -------------------------------------------- |
| `kc_upper`             | `kc_ultra_extreme_upper` | `FLOAT`   | KC band rename — correct ATR multiplier name |
| `kc_middle`            | `kc_extreme_upper`       | `FLOAT`   | KC band rename                               |
| `kc_lower`             | `kc_uppermost`           | `FLOAT`   | KC band rename                               |
| `kc_upper_ema`         | `kc_upper`               | `FLOAT`   | KC band rename                               |
| `kc_middle_ema`        | `kc_upper_middle`        | `FLOAT`   | KC band rename                               |
| `kc_lower_ema`         | `kc_lower_middle`        | `FLOAT`   | KC band rename                               |
| `kc_squeeze`           | `kc_lower`               | `FLOAT`   | KC band rename                               |
| `kc_squeeze_pro`       | `kc_lowermost`           | `FLOAT`   | KC band rename                               |
| `kc_width`             | `kc_extreme_lower`       | `FLOAT`   | KC band rename                               |
| `kc_width_ema`         | `kc_ultra_extreme_lower` | `FLOAT`   | KC band rename                               |
| `ha_strength`          | `ha_body_size`           | `FLOAT`   | HA rename — matches MQL5 export header       |
| `ha_classification`    | `ha_color`               | `INTEGER` | HA rename — 0-5 enum → 1/-1 integer          |
| `zigzag_peak`          | `zigzag_high`            | `FLOAT`   | ZigZag rename — matches Prisma schema        |
| `zigzag_bottom`        | `zigzag_low`             | `FLOAT`   | ZigZag rename — matches Prisma schema        |
| `sr_support_1`         | `sr_1`                   | `FLOAT`   | S/R simplification                           |
| `sr_support_2`         | `sr_2`                   | `FLOAT`   | S/R simplification                           |
| `sr_support_3`         | `sr_3`                   | `FLOAT`   | S/R simplification                           |
| `sr_support_4`         | `sr_4`                   | `FLOAT`   | S/R simplification                           |
| `sr_resistance_1`      | `sr_5`                   | `FLOAT`   | S/R simplification                           |
| `sr_resistance_2`      | `sr_6`                   | `FLOAT`   | S/R simplification                           |
| `sr_resistance_3`      | `sr_7`                   | `FLOAT`   | S/R simplification                           |
| `sr_resistance_4`      | `sr_8`                   | `FLOAT`   | S/R simplification                           |
| (not present)          | `ha_body_zscore`         | `FLOAT`   | NEW column — Heiken Ashi body Z-score        |
| (not present)          | `body_classification`    | `INTEGER` | NEW column — Body Momentum classification    |

**v3.0 renames (60 → 61) — for reference:**
...
**FREE Tier:** **25 total columns** — unchanged from v3.0

- 9 system columns
- 16 FREE indicator columns

**PRO Tier:** **63 total columns** (was 61 in v3.0)

- 9 system columns
- 16 FREE indicator columns
- 38 PRO indicator columns (was 36)
...
const allowedColumns =
  tier === 'PRO' ? ALL_63_COLUMNS : SYSTEM_COLUMNS + FREE_TIER_COLUMNS; // 25 columns (unchanged)

// Constants update required:
// Was: ALL_61_COLUMNS_SELECT
// Now: ALL_63_COLUMNS_SELECT (10 KC renames, 5 HA/SR/ZZ renames, 2 new columns)
```
...
model MarketData {
  ...
  // PRO tier indicators (38 columns) — v4.0 corrected names
  // Group 1: Moving Averages (3)
  tema             Float?
  hrma             Float?
  smma             Float?
  // Group 2: Body Momentum (3) — body_classification NEW in v4.0
  body_direction   Float?
  body_size        Float?
  body_classification Int?  // ← NEW in v4.0
  // Group 3: Heiken Ashi (8) — ha_body_zscore NEW, ha_strength RENAMED, ha_classification RENAMED
  ha_open          Float?
  ha_high          Float?
  ha_low           Float?
  ha_close         Float?
  ha_color         Float?   // ← RENAMED from ha_classification in v4.0 (1/-1)
  ha_trend         Float?
  ha_body_size     Float?   // ← RENAMED from ha_strength in v4.0
  ha_body_zscore   Float?   // ← NEW in v4.0
  // Group 4: Keltner Channel (10) — all names CORRECTED in v4.0
  kc_ultra_extreme_upper Float?
  kc_extreme_upper       Float?
  kc_uppermost           Float?
  kc_upper               Float?
  kc_upper_middle        Float?
  kc_lower_middle        Float?
  kc_lower               Float?
  kc_lowermost           Float?
  kc_extreme_lower       Float?
  kc_ultra_extreme_lower Float?
  // Group 5: Support/Resistance (8) — names CORRECTED in v4.0
  sr_1             Float?   // ← was sr_support_1
  sr_2             Float?   // ← was sr_support_2
  sr_3             Float?   // ← was sr_support_3
  sr_4             Float?   // ← was sr_support_4
  sr_5             Float?   // ← was sr_resistance_1
  sr_6             Float?   // ← was sr_resistance_2
  sr_7             Float?   // ← was sr_resistance_3
  sr_8             Float?   // ← was sr_resistance_4
  // Group 6: ZigZag + EMA (3) — names CORRECTED in v4.0
  zigzag_high      Float?   // ← RENAMED from zigzag_peak in v4.0
  zigzag_low       Float?   // ← RENAMED from zigzag_bottom in v4.0
  ema              Float?
  // Group 7: Dual TEMA H/L (2)
  dual_tema_high   Float?
  dual_tema_low    Float?
  // Group 8: Pinbar Detection (1)
  pinbar           Int?
...
export interface MarketDataRecord {
  ...
  /** NEW in v4.0: candle body classification 0-5 enum */
  body_classification: number | null;
  // Group 3: Heiken Ashi (8) — renames + new column in v4.0
  ha_open: number | null;
  ha_high: number | null;
  ha_low: number | null;
  ha_close: number | null;
  /** RENAMED from ha_classification in v4.0. Values: 1 (bullish), -1 (bearish). */
  ha_color: number | null;
  ha_trend: number | null;
  /** RENAMED from ha_strength in v4.0. HA candle body size. */
  ha_body_size: number | null;
  /** NEW in v4.0. HA body size Z-score. */
  ...
  kc_ultra_extreme_lower: number | null; // HRMA - 4.0×ATR
  // Group 5: Support/Resistance (8) — names CORRECTED in v4.0
  sr_1: number | null; // was sr_support_1
  sr_2: number | null; // was sr_support_2
  sr_3: number | null; // was sr_support_3
  sr_4: number | null; // was sr_support_4
  sr_5: number | null; // was sr_resistance_1
  sr_6: number | null; // was sr_resistance_2
  sr_7: number | null; // was sr_resistance_3
  sr_8: number | null; // was sr_resistance_4
  // Group 6: ZigZag + EMA (3) — names CORRECTED in v4.0
  /** RENAMED from zigzag_peak in v4.0 */
  zigzag_high: number | null;
  /** RENAMED from zigzag_bottom in v4.0 */
...
```typescript
// Updated select objects for Prisma queries — v4.0 (63 columns)
export const ALL_63_COLUMNS_SELECT = {
  // System columns (9) — unchanged from v3.0
  timestamp: true,
  symbol: true,
  open: true,
  ...
  body_direction: true,
  body_size: true,
  body_classification: true, // ← NEW in v4.0
  ha_open: true,
  ha_high: true,
  ha_low: true,
  ha_close: true,
  ha_color: true, // ← RENAMED from ha_classification in v4.0
  ha_trend: true,
  ha_body_size: true, // ← RENAMED from ha_strength in v4.0
  ha_body_zscore: true, // ← NEW in v4.0
  kc_ultra_extreme_upper: true,
  kc_extreme_upper: true,
  kc_uppermost: true,
  kc_upper: true,
  kc_upper_middle: true,
  kc_lower_middle: true,
  kc_lower: true,
  kc_lowermost: true,
  kc_extreme_lower: true,
  kc_ultra_extreme_lower: true,
  sr_1: true, // ← was sr_support_1
  sr_2: true, // ← was sr_support_2
  sr_3: true, // ← was sr_support_3
  sr_4: true, // ← was sr_support_4
  sr_5: true, // ← was sr_resistance_1
  sr_6: true, // ← was sr_resistance_2
  sr_7: true, // ← was sr_resistance_3
  sr_8: true, // ← was sr_resistance_4
  zigzag_high: true, // ← RENAMED from zigzag_peak in v4.0
  zigzag_low: true, // ← RENAMED from zigzag_bottom in v4.0
  ema: true,
  dual_tema_high: true,
  dual_tema_low: true,
  pinbar: true,
...
1. **63 Columns Total:** 9 system + 16 FREE + 38 PRO (was 9 + 16 + 36 in v3.0)
2. **2 New Columns:** `ha_body_zscore` (Heiken Ashi) + `body_classification` (Body Momentum)
3. **10 KC Band Names Corrected:** Placeholder names → correct ATR-multiplier band names
4. **5 Additional Renames:** ha_strength→ha_body_size, ha_classification→ha_color, zigzag_peak→zigzag_high, zigzag_bottom→zigzag_low, sr_support/resistance_N→sr_1-8
5. **FREE Tier Unchanged:** Still 25 columns (9 system + 16 FREE indicators)
6. **PRO Tier:** 63 columns (was 61) — 38 PRO indicators (was 36)

### Quick Reference

| Aspect            | FREE Tier | PRO Tier     |
| ----------------- | --------- | ------------ |
| Total Columns     | 25        | **63** (↑61) |
| System Columns    | 9         | 9            |
| Indicator Columns | 16        | **54** (↑52) |
| PRO Indicators    | N/A       | **38** (↑36) |

### Schema Changes at a Glance (v4.0)

| Change                           | Type       | Scope        | Impact                    |
| -------------------------------- | ---------- | ------------ | ------------------------- |
| **ha_body_zscore added**         | NEW column | PRO/HA       | +1 column, 61→63          |
| **body_classification**          | NEW column | PRO/Momentum | +1 column                 |
| **10 KC renames**                | Renamed    | PRO/KC       | All 10 KC names corrected |
| **ha_strength→ha_body_size**     | Renamed    | PRO/HA       | 1 column renamed          |
| **ha_classification→ha_color**   | Renamed    | PRO/HA       | 0-5 enum → 1/-1 integer   |
| **zigzag_peak→zigzag_high**      | Renamed    | PRO/ZigZag   | 1 column renamed          |
| **zigzag_bottom→zigzag_low**     | Renamed    | PRO/ZigZag   | 1 column renamed          |
| **sr_support/resistance→sr_1-8** | Renamed    | PRO/SR       | 8 columns simplified      |

---

**Document Version:** 4.0.0
**Last Updated:** 2026-02-24
**Replaces:** `61-column-schema-update-guide.md` v3.0.0 (2026-02-11)
**Next Review:** After completing all codebase updates for 63-column schema
