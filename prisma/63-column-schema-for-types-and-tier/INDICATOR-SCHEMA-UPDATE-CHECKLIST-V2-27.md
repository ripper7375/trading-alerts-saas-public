...
**Schema Version:** 61-Column (EA v2.27)
...
```
MarketData = 9 system + 16 FREE indicator + 36 PRO indicator = 61 total columns
...
- [ ] Update the model docstring: `/// Structure: 9 system + 16 FREE tier + N PRO tier = N total columns`
- [ ] Update section header comment: `// MARKET DATA (61-COLUMN SCHEMA)`
...
- [ ] Update JSDoc header: `Schema: 61-column MarketData flat schema`
...
- [ ] Update `## V7 Architecture` line: `61-Column Database: 9 system + 16 FREE + N PRO indicator columns`
- [ ] Update `4. **Indicator Types**` bullet: `61-column database schema types`
...
- [ ] Update PRO Tier Specifications: `Indicators: N (all FREE + N PRO-only)`, `Indicator Columns: N columns`, `Total Columns: N (9 system + N indicator)`
...
- [ ] Update `**Total Columns:**` block: `PRO Tier: 9 system + N indicator = N columns`
...
**Section: `NEW 61-COLUMN DATABASE SCHEMA TYPES` (mid-file)**
...
- [ ] Update docstring column count: `all 61 columns`
...
- [ ] Update: `PRO: 9 system + N indicator = N total columns`
- [ ] Update inline comment: `// 9 + N = N`

**`isValidColumnName` docstring:**

- [ ] Update: `61-column schema` (or new total)
...
- [ ] Update file header JSDoc: `61-Column Schema` → new total if changed
- [ ] Update comment: `PRO tier: 61 columns (9 system + 52 indicators)` → new counts
...
- [ ] Update: `61-COLUMN SCHEMA DATA STRUCTURES` → new total if column count changes
...
- [ ] Update `61-Column Schema Structure:` listing
...
- [ ] Update `Market Data Model (61-Column Schema)` describe block name if total changes
...
- [ ] Update: `61-Column Schema` → new total
...
- [ ] `toBe(61)` → new PRO total column count
...
- [ ] `toHaveLength(61)` → new total column count
...
- [ ] Update: `61-column database schema` if it changes
...
- [ ] Update: `// Indicator Access Tests (61-Column Schema)`
...
| `constants.test.ts`                                                 | 4 `toHaveLength()` calls, 1 `toBe(36)`, 1 `toBe(52)`, 1 `toBe(61)`                                                  |
| `validator.test.ts`                                                 | 4 `toHaveLength()` calls, 1 `toHaveLength(61)`, 1 `toHaveLength(36)`                                                |
...
\_Last verified against schema version: 61-column (EA v2.27)
