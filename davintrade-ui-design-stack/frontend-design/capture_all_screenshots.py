...
CONCURRENCY = 6  # 6 parallel pages for smooth rendering
...
async def process_task(sem, context, task_info, total_count, progress_counter, lock):
    cc, item_no, url, dest_file, filename = task_info
    async with sem:
        ...
        async with lock:
            ...
            if success:
                print(f"[{curr}/{total_count}] Captured [{cc.upper()}] Row {item_no:02d} -> {cc}/{filename}")
...
async def main():
    ...
    for cc in COUNTRY_CODES:
        dir_path = os.path.join(OUTPUT_BASE_DIR, cc)
    ...
    for r in range(2, sheet.max_row + 1):
        ...
        for c, cc in col_to_cc.items():
            ...
            dest_file = os.path.join(OUTPUT_BASE_DIR, cc, filename)
            tasks_info.append((cc, item_no, url, dest_file, filename))

    total = len(tasks_info)
    print(f"Total screenshots queued: {total}")
    ...
    async with async_playwright() as p:
        # Launch installed Chrome/Edge browser
