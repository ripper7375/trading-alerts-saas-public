   ...
   for(int i = 0; i < len; i++)
     {
      ...
      if(ch == quote)
        {
         if(inQuotes)
           {
            // Double-quote escape ("")
            if(doublequote && i + 1 < len && line[i + 1] == quote)
              {
               current += CharToString(quote);
               i++;
              }
            else
              {
      ...
      if(!inQuotes && CharToString(ch) == delimiter)
        {
         int sz = ArraySize(fields);
         ArrayResize(fields, sz + 1);
         fields[sz] = skipinitialspace ? StringTrim(current) : current;
         current = "";
         continue;
        }
...
void CSVReader::ParseCSVLine(string line, string &fields[])
  {
    ::ParseCSVLine(line, fields, m_escape, m_quote, m_doublequote, m_skipinitialspace, m_delimiter);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//|                                                                  |
...
public:
                     ...
                     CSVWriter(CFile &file, const string delimiter = ",", const char quotechar = '"', const char escapechar = '\\', const bool doublequote = true);
