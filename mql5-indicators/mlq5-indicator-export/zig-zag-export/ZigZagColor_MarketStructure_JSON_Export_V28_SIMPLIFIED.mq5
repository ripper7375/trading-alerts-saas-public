...
input int    xBartoPrint   = 500;    // Bar to Print in Debug Prints
input string    InpExportFileName = "MarketStructureAnalysis.json";  // Export file name
input int       InpMaxBarsExport = 500;      // Max bars to export (0 = all bars)
...
// Only check buffers when size actually changes or on first run
   if(rates_total != g_LastBufferSize || prev_calculated == 0)
     {
      if(CheckAndResizeLiveDataBuffers(rates_total))
        {
         if(xInpDebugMode)
            Print("Buffer check completed - ", rates_total, " bars available");
        }
...
// Only print when actual resizing occurred AND debug mode is enabled
   if(resized && xInpDebugMode)
     {
      Print("Buffers resized to ", buffer_size, " elements (needed: ", rates_total, ")");
     }
