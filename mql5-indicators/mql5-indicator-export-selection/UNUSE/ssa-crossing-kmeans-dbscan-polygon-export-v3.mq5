...
input int                  InpMaxRayLines      = 10; // Max Ray Lines to Draw (0-12)
...
void PerformClustering(const int rates_total, const datetime &time[])
{
   ...
   // 6. Draw Ray Lines based on user configuration (Max 12)
   int user_ray_limit = (int)MathMax(0, MathMin(12, InpMaxRayLines));
   int ray_limit = MathMin(user_ray_limit, centroid_count);
...
//+------------------------------------------------------------------+
//| Get Top 12 Centroids logic (used for accurate backfilling)       |
//+------------------------------------------------------------------+
void GetTop12CentroidsForShift(int shift, const int rates_total, const datetime &time_arr[], double &out_cen[])
{
   ArrayResize(out_cen, 12);
   ...
   int limit = MathMin(12, temp_count);
...
//+------------------------------------------------------------------+
//| Export Logic with exact 16 columns requested                     |
//+------------------------------------------------------------------+
bool ExportData()
{
   ...
   // --- UPDATED EXPORT HEADER (12 CEN Columns) ---
   ok &= FileWrite(fh, "timestamp\tsymbol\ttimeframe\tclose\tCEN_0\tCEN_1\tCEN_2\tCEN_3\tCEN_4\tCEN_5\tCEN_6\tCEN_7\tCEN_8\tCEN_9\tCEN_10\tCEN_11") > 0;
   ...
   for(int i = 0; i < bars_to_export; i++)
   {
      ...
      double top12[12];
      GetTop12CentroidsForShift(shift, g_rates_total, g_time, top12);

      string cen_str[12];
      for(int c=0; c<12; c++) {
         cen_str[c] = (top12[c] != 0.0) ? DoubleToString(top12[c], _Digits + 3) : "";
      }

      // 16 %s string placeholders
      string line = StringFormat("%lld\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s",
