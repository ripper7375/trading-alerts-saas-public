...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Fractal SR (V5.54 Simplified)");
...
void BuildMultiPointTrendlines(const int rates_total)
{
   ...
   if(ArraySize(peak_lines) >= 1)
   {
      ...
      if(InpEnableColorIntensity)
      {
         color line_color = CalculateAdaptiveColor(peak_lines[0].line_price_at_current, current_price, InpPeakLineColor);
      ...
      if(InpShowLabels)
      {
         ...
         DrawLineLabel("P_P1", peak_lines[0].line_price_at_current, label_time, label_text, InpPeakLineColor, InpLabelFontSize);
      }
   }
   ...
   if(ArraySize(bottom_lines) >= 1)
   {
      ...
      if(InpEnableColorIntensity)
      {
         color line_color = CalculateAdaptiveColor(bottom_lines[0].line_price_at_current, current_price, InpBottomLineColor);
      ...
      if(InpShowLabels)
      {
         ...
         DrawLineLabel("B_B1", bottom_lines[0].line_price_at_current, label_time, label_text, InpBottomLineColor, InpLabelFontSize);
      }
   }
}
         ...
         if(ExtUpperBuffer[i] != EMPTY_VALUE && ExtUpperBuffer[i] > 0)
           {
               if(ExtUpperBuffer[i] < 1000000)  // Simple sanity check
                 {
                  if(peak_count == 0) most_recent_peak = ExtUpperBuffer[i];
                  else if(peak_count == 1) second_recent_peak = ExtUpperBuffer[i];
                  peak_count++;
                 }
           }
        }
         ...
         if(ExtLowerBuffer[i] != EMPTY_VALUE && ExtLowerBuffer[i] > 0)
           {
               if(ExtLowerBuffer[i] < 1000000)  // Simple sanity check
                 {
                  if(bottom_count == 0) most_recent_bottom = ExtLowerBuffer[i];
                  else if(bottom_count == 1) second_recent_bottom = ExtLowerBuffer[i];
                  bottom_count++;
                 }
           }
        }

      if(peak_count >= 2 && bottom_count >= 2) break;
     }
  }
   ...
   for(int i = 0; i < rates_total && !IsStopped(); i++)
     {
      ...
      if(peak_count < 2)
        {
         if(ExtUpperBuffer119[i] != EMPTY_VALUE && ExtUpperBuffer119[i] > 0)
           {
               if(ExtUpperBuffer119[i] < 1000000)  // Simple sanity check
                 {
                  if(peak_count == 0) most_recent_peak = ExtUpperBuffer119[i];
                  else if(peak_count == 1) second_recent_peak = ExtUpperBuffer119[i];
                  peak_count++;
                 }
           }
        }
      ...
      if(bottom_count < 2)
        {
         if(ExtLowerBuffer119[i] != EMPTY_VALUE && ExtLowerBuffer119[i] > 0)
           {
               if(ExtLowerBuffer119[i] < 1000000)  // Simple sanity check
                 {
                  if(bottom_count == 0) most_recent_bottom = ExtLowerBuffer119[i];
                  else if(bottom_count == 1) second_recent_bottom = ExtLowerBuffer119[i];
                  bottom_count++;
                 }
           }
        }
...
void CheckProximityAndAlert(const int rates_total)
  {
   ...
   string status_message = "=== Fractal Support/Resistance Monitor V5.54 Simplified ===\n";
   ...
   if(most_recent_peak_108 > 0) status_message += "Most Recent Peak: " + DoubleToString(most_recent_peak_108, _Digits) + "\n";
   if(most_recent_bottom_108 > 0) status_message += "Most Recent Bottom: " + DoubleToString(most_recent_bottom_108, _Digits) + "\n";
   
   status_message += "\n--- 119 Fractals (Alert Trigger) ---\n";
   if(most_recent_peak > 0) status_message += "Most Recent Peak: " + DoubleToString(most_recent_peak, _Digits) + "\n";
   if(most_recent_bottom > 0) status_message += "Most Recent Bottom: " + DoubleToString(most_recent_bottom, _Digits) + "\n";
      ...
      if(IsPriceNearTrendline(current_price, current_bar, InpTolerancePercent_Alert, matched_trendlines))
        {
         ...
         if(InpAlertCooldownSeconds == 0 || (current_time - ExtCondition1LastAlert) >= InpAlertCooldownSeconds)
      ...
      if(IsPriceNearTrendline(current_price, current_bar, InpTolerancePercent_Alert, matched_trendlines))
        {
         ...
         if(InpAlertCooldownSeconds == 0 || (current_time - ExtCondition2LastAlert) >= InpAlertCooldownSeconds)
            alert_triggered = true;
        }
     }
      ...
      if(IsPriceNearTrendline(current_price, current_bar, InpTolerancePercent_Alert, matched_trendlines))
        {
         ...
         if(InpAlertCooldownSeconds == 0 || (current_time - ExtCondition3LastAlert) >= InpAlertCooldownSeconds)
            alert_triggered = true;
        }
     }
      ...
      if(IsPriceNearTrendline(current_price, current_bar, InpTolerancePercent_Alert, matched_trendlines))
        {
         ...
         if(InpAlertCooldownSeconds == 0 || (current_time - ExtCondition4LastAlert) >= InpAlertCooldownSeconds)
            alert_triggered = true;
        }
     }
   ...
   if(condition_number > 0)
     {
      ...
      if(alert_triggered && InpMasterAlertSwitch) status_message += "✅ Alert SENT\n";
      else if(alert_triggered && !InpMasterAlertSwitch) status_message += "🔇 Alert BLOCKED (Master Switch OFF)\n";
     }
   else
     {
   ...
   if(InpShowChartComment) Comment(status_message);
   
   if(alert_triggered && InpMasterAlertSwitch)
     {
      if(condition_number == 1) ExtCondition1LastAlert = current_time;
      else if(condition_number == 2) ExtCondition2LastAlert = current_time;
      else if(condition_number == 3) ExtCondition3LastAlert = current_time;
      else if(condition_number == 4) ExtCondition4LastAlert = current_time;
      
      Alert(alert_message);

      if(InpPlaySound && InpAlertSound != "") PlaySound(InpAlertSound);
      if(InpSendNotification) SendNotification(alert_message);
     }
  }
