...
void OnDeinit(const int reason)
  {
   ...
   ObjectDelete(0, "ShortTerm_Support_Line"); // Clean up trendline
  }
...
void BuildBestFlipLine(const int rates_total)
  {
   ...
   if(BestLine.score > 0) {
      // 1. Populate the buffer ONLY within the exact user date range for export
      for(int i = end_idx; i <= start_idx; i++) {
      ...
      // 2. Render as a Graphical Object to prevent chart distortion
      ...
      ObjectSetInteger(0, obj_name, OBJPROP_RAY_RIGHT, false); // Strict bounds
   }
  }
