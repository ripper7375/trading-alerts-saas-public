...
#property description "Combines SSA Clustering, Dynamic N-Centroid Regression, and Symmetrical EDT Snapping"
...
input int               InpRegCentroids = 3; // Number of Centroids for Regression (3 to 6)
input int               InpEDTLookbackBars = 500;
input int               InpMaxEDTLines = 4;
input int               InpEDTMinTouches = 1; // Relaxed: Parallel boundaries only mathematically need 1 extreme anchor
input double            InpMinLineSeparationPercent = 0.5; // Relaxed slightly to prevent deleting valid lines
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "SSA Regression EDT V3.40");
...
void PerformClusteringAndEDT(const int rates_total, const datetime &time[], const double &close_arr[])
{
   ...
   // Safety Check: Clamp the user input between 3 and 6 mathematically
   int target_reg = (int)MathMax(3, MathMin(InpRegCentroids, 6)); 
   
   // If we don't have enough centroids for the target, we can't draw a reliable line yet
...
void BuildSymmetricalEDTs(double base_m, double base_c, const FractalPoint &fractals[], int rates_total, double current_close, int edtStartIdx)
{
   ...
   for(int i = 0; i < f_count; i++) {
      ...
      if(touches >= InpEDTMinTouches) {
         ...
         // V3.30: Primary Score = Touches. Secondary Score (Tie-breaker) = Recency (Highest bar index)
