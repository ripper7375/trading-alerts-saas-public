...
#property description "DavinTrade V3.60: Dynamic N-Centroid Regression, Symmetrical EDT, and R-Square/MSE Engine"
#property indicator_chart_window

// Total Buffers: 17 for Plots + 1 Hidden for Cluster Tracking + 2 for Data Streaming (R2 & MSE) = 20
...
input int               InpEDTLookbackBars = 2000;
...
double ExtCrossInCluster[]; // 17 (Hidden - Cluster Tracking)
double ExtRSquare[];        // 18 (Hidden - Data Streaming for EA)
double ExtMSE[];            // 19 (Hidden - Data Streaming for EA)
...
int OnInit()
{
   ...
   // Hidden Data Streaming Buffers
   SetIndexBuffer(18, ExtRSquare, INDICATOR_DATA);
   SetIndexBuffer(19, ExtMSE, INDICATOR_DATA);
   PlotIndexSetDouble(18, PLOT_EMPTY_VALUE, EMPTY_VALUE);
   PlotIndexSetDouble(19, PLOT_EMPTY_VALUE, EMPTY_VALUE);
   ArrayInitialize(ExtRSquare, EMPTY_VALUE);
   ArrayInitialize(ExtMSE, EMPTY_VALUE);
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DavinTrade SSA EDT V3.60");
...
void PerformClusteringAndEDT(const int rates_total, const datetime &time[], const double &close_arr[])
{
   ...
   // ---------------------------------------------------------------------
   // NEW V3.60: calculate R-Square and MSE over the Observation Period
   // ---------------------------------------------------------------------
   int leftmost_bar = rates_total; // Oldest
   int rightmost_bar = -1;         // Newest
   ...
   // Find the boundaries
   for(int i = 0; i < p_count; i++) {
      if(assignments[i] == oldest_cluster_id && points[i].bar < leftmost_bar) {
          leftmost_bar = points[i].bar;
      }
      if(assignments[i] == newest_cluster_id && points[i].bar > rightmost_bar) {
          rightmost_bar = points[i].bar;
      }
   }
   ...
   if(leftmost_bar <= rightmost_bar && leftmost_bar != rates_total) {
      ...
      for(int i = leftmost_bar; i <= rightmost_bar; i++) {
          if(ExtSSACross[i] == EMPTY_VALUE || ExtSSACross[i] == 0.0) continue; // Event-based logic
      ...
      // Pass 2: R2 and MSE
      if(n_crossings >= 2) {
          double mean_crossing_price = sum_crossings / n_crossings;
          double sum_sq_err = 0;
          double tot_sum_sq = 0;
          
          for(int i = leftmost_bar; i <= rightmost_bar; i++) {
              if(ExtSSACross[i] == EMPTY_VALUE || ExtSSACross[i] == 0.0) continue;
              double actual_y = ExtSSACross[i];
              double pred_y = base_m * i + base_c;
              
              sum_sq_err += MathPow(actual_y - pred_y, 2);
              tot_sum_sq += MathPow(actual_y - mean_crossing_price, 2);
          }
          
          current_mse = sum_sq_err / n_crossings;
          if(tot_sum_sq != 0) current_rsquare = 1.0 - (sum_sq_err / tot_sum_sq);
      }
   }
   
   // Assign values to the hidden buffers for EA Streaming (Always stored at the current live bar)
   int current_bar_idx = rates_total - 1;
   ExtRSquare[current_bar_idx] = current_rsquare;
   ExtMSE[current_bar_idx] = current_mse;
   
   // Format and display the UI Comment block
   string comment_text = StringFormat(
       "--- DavinTrade Base Line Goodness of Fit ---\n" +
       ...
       "MSE: %.6f",
       target_reg, 
       (rightmost_bar - leftmost_bar + 1), leftmost_bar, rightmost_bar,
       n_crossings, 
       current_rsquare, current_mse
   );
...
void BuildSymmetricalEDTs(double base_m, double base_c, const FractalPoint &fractals[], int rates_total, double current_close, int edtStartIdx)
{
   ...
   for(int i = 0; i < f_count; i++) {
      ...
      if(touches >= InpEDTMinTouches) {
         ...
         tl.score = (touches * 10000.0) + fractals[i].bar; 
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   // Clustering -> Regression -> EDT -> R2/MSE
