...
#property description "Fractal Clusters with Centroid Equilibrium Lines and Stars"
...
void OnDeinit(const int reason)
  {
   ...
   ObjectsDeleteAll(0, "ClusterCentroidStar_"); // Delete centroid stars
  }
...
void PerformClustering(const int rates_total, const datetime &time[])
  {
   ObjectsDeleteAll(0, "ClusterHull_"); 
   ObjectsDeleteAll(0, "ClusterCentroid_");
   ObjectsDeleteAll(0, "ClusterCentroidStar_"); // Reset stars too
   ...
   for(int k = 0; k < InpClusterCount; k++) {
      ...
      // 5. Draw Bounding Polygon & Centroid Line
      if(ArraySize(cluster_points) >= 3) {
         ...
         // 6. De-normalize Centroid & Draw Equilibrium Line
         double real_centroid_price = center_y * (max_price - min_price) + min_price;
         int real_centroid_bar = (int)MathRound(center_x * (max_bar - min_bar) + min_bar);
         ...
         // --- MODIFIED: Use OBJ_TEXT for perfect centering and exact size control ---
         ...
         ObjectSetString(0, star_name, OBJPROP_TEXT, ShortToString(171)); // Star symbol code
         ObjectSetInteger(0, star_name, OBJPROP_FONTSIZE, 14); // Reasonably scaled size
         ObjectSetInteger(0, star_name, OBJPROP_ANCHOR, ANCHOR_CENTER); // Perfect vertical/horizontal center
