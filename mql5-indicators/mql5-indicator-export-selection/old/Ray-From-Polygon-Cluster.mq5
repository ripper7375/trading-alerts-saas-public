...
#property description "Fractal Clusters with Centroid Stars & S/R Rays"
...
void OnDeinit(const int reason)
  {
   ...
   ObjectsDeleteAll(0, "ClusterRay_"); // Delete rays
  }
...
void PerformClustering(const int rates_total, const datetime &time[])
  {
   ObjectsDeleteAll(0, "ClusterHull_"); 
   ObjectsDeleteAll(0, "ClusterCentroidStar_");
   ObjectsDeleteAll(0, "ClusterRay_"); // Clean up rays
   ...
   for(int k = 0; k < InpClusterCount; k++) {
      ...
      if(ArraySize(cluster_points) >= 3) {
         ...
         // --- NEW LOGIC: Find Longest and Second Longest Sides ---
         int longest_idx = -1;
         ...
         for(int h = 0; h < h_count; h++) {
            ...
            // Calculate Euclidean distance in normalized space for accuracy
         ...
         for(int h = 0; h < h_count; h++) {
            ...
            ObjectSetInteger(0, line_name, OBJPROP_RAY_RIGHT, false); // Keep polygon borders bounded
            ObjectSetInteger(0, line_name, OBJPROP_BACK, true);
            ObjectSetInteger(0, line_name, OBJPROP_SELECTABLE, false);
         }
         
         // --- NEW LOGIC: Draw Rays for Longest Sides ---
         if(longest_idx != -1) {
            FractalPoint p1 = hull[longest_idx];
            FractalPoint p2 = hull[(longest_idx + 1) % h_count];
            string ray_name1 = "ClusterRay_" + IntegerToString(k) + "_1";
            ObjectCreate(0, ray_name1, OBJ_TREND, 0, p1.time, p1.price, p2.time, p2.price);
            ObjectSetInteger(0, ray_name1, OBJPROP_COLOR, c_color);
            ObjectSetInteger(0, ray_name1, OBJPROP_WIDTH, 1); // Thinner line for the extended ray
         ...
         if(second_longest_idx != -1) {
            ...
            string ray_name2 = "ClusterRay_" + IntegerToString(k) + "_2";
            ObjectCreate(0, ray_name2, OBJ_TREND, 0, p1.time, p1.price, p2.time, p2.price);
            ObjectSetInteger(0, ray_name2, OBJPROP_COLOR, c_color);
            ObjectSetInteger(0, ray_name2, OBJPROP_WIDTH, 1); // Thinner line for the extended ray
