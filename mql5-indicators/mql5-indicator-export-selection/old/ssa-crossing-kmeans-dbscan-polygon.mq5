...
double ssaCrossInClusterBuffer[]; // Hidden buffer for export logic
...
int OnInit()
{
   ...
   // Export button can be enabled here if desired
   // CreateExportButton();
...
void OnDeinit(const int reason)
{
   ...
   // ObjectDelete(0, EXPORT_BUTTON_NAME); // Uncomment if button is used
   ChartRedraw(0);
}
...
void PerformClustering(const int rates_total, const datetime &time[])
{
   ObjectsDeleteAll(0, "ClusterHull_"); 
   ObjectsDeleteAll(0, "ClusterCentroidStar_");
   ArrayInitialize(ssaCrossInClusterBuffer, 0.0); // Reset cluster state
   ...
   // --- Populate hidden buffer for export ---
   for(int i = 0; i < p_count; i++) {
      if(assignments[i] != -1) {
         ssaCrossInClusterBuffer[points[i].bar] = 1.0; 
      }
   }
   ...
   for(int k = 0; k < total_clusters; k++) {
      ...
      if(filtered_count >= 3) { 
         ...
         // --- REDUCED CENTROID SIZE HERE ---
         ObjectSetInteger(0, star_name, OBJPROP_FONTSIZE, 10); // Changed from 16 to 10
...
bool ExportData()
{
   ...
   ok &= FileWrite(fh, "timestamp\tsymbol\ttimeframe\tclose\tssa\tema_ssa\tssa_high\tssa_low\tssa_cross\tssa_cross_in_polygon") > 0;
   ...
   for(int i = 0; i < bars_to_export; i++)
   {
      ...
      // --- READ HIDDEN BUFFER FOR POLYGON FLAG ---
      string ssa_in_poly_str = (ssaCrossInClusterBuffer[idx] > 0.0) ? "1" : "0";

      string line = StringFormat("%lld\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s",
                                 (long)(g_time[idx] - gmt_offset), symbol, tf_str,
                                 DoubleToString(g_close[idx], _Digits), ssa_str, ema_ssa_str,
                                 ssa_high_str, ssa_low_str, ssa_cross_str, ssa_in_poly_str);
