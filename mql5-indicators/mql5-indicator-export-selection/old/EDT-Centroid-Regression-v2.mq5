...
#property description "Combines SSA Clustering Centroids with Linear Regression and Recency-Biased EDT Snapping"
...
input int               InpEDTLookbackBars = 500; // NEW: Forces EDT recency
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "SSA Regression EDT (Lookback)");
...
void PerformClusteringAndEDT(const int rates_total, const datetime &time[], const double &close_arr[])
{
   ...
   BuildEDTs(base_m, base_c, fractals, rates_total, close_arr[rates_total-1], edtStartIdx);
}

//+------------------------------------------------------------------+
//| EDT Snapping onto the Base Regression Slope (Recency Biased)     |
//+------------------------------------------------------------------+
void BuildEDTs(double base_m, double base_c, const FractalPoint &fractals[], int rates_total, double current_close, int edtStartIdx)
{
   ...
   TrendLine candidates[]; int cand_count = 0;
   ...
   for(int i = 0; i < f_count; i++) {
      ...
      if(touches >= InpMinTouches && pass_flip) {
         ...
         candidates[cand_count].slope = base_m;
         candidates[cand_count].y_intercept = test_intercept;
         candidates[cand_count].touches = touches;
         candidates[cand_count].score = touches * 10000.0; 
         cand_count++;
      }
   }
   ...
   for(int i = 0; i < cand_count - 1; i++) {
      for(int j = i + 1; j < cand_count; j++) {
         if(candidates[j].score > candidates[i].score) {
            TrendLine tmp = candidates[i]; candidates[i] = candidates[j]; candidates[j] = tmp;
         }
      }
   }

   // Filter close lines & Pick Top N
   ...
   for(int i = 0; i < cand_count && final_count < max_allowed; i++) {
      ...
      double cand_price_now = base_m * (rates_total - 1) + candidates[i].y_intercept;
      ...
      if(!too_close) {
         ArrayResize(final_edts, final_count + 1); final_edts[final_count] = candidates[i]; final_count++;
      }
   }
