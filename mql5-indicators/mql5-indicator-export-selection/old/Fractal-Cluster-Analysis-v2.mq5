...
#property description "Fractal Clusters with Custom K-Means & Outlier Rejection"
...
input double            InpOutlierDistanceMultiplier = 1.5; // Max Distance Multiplier to close clusters
...
void PerformClustering(const int rates_total, const datetime &time[])
  {
   ...
   for(int k = 0; k < InpClusterCount; k++) {
      ...
      for(int i = 0; i < f_count; i++) {
         if(assignments[i] == k) {
            double dist = MathSqrt(MathPow(fractals[i].norm_x - center_x, 2) + MathPow(fractals[i].norm_y - center_y, 2));
            if(dist <= max_allowed_dist) {  // Only accept if within distance limit
      ...
      // 5. Draw Bounding Polygon around remaining "Core" cluster
