...
#property description "DavinTrade V3.94: Time-Weighted WLS CFL + Outermost EDTs (Multi-Chart Safe)"
...
#define EXPORT_BUTTON_NAME "V394ExportButton"
...
input bool              InpShowComments = false;           // Prevents flickering when multiple indicators are on one chart
input int               InpRegCentroids = 5;               // (3-12) Max Centroids to Evaluate (Comb. Masking)
input double            InpTimeDecayLambda = 0.001;        // Exponential WLS Decay (0.0 = Ordinary Least Squares)
input int               InpCFLVisualLookback = 500;        // Visual Drawing Limit
input bool              InpExtendLinesToCurrent = true;    // Extend CFL & EDTs to Live Bar

input color             InpCFLColor = clrDarkOrange;       // Color of Top CFL
input int               InpEDTMinTouches = 3;              // Minimum Fractal Touches for EDT
input color             InpEDTColor = clrDarkViolet;       // Color of Outermost EDTs
...
input string            InpExportFileName = "DavinTrade_V394_CFL_WLS_Data";
...
double ExtUOEDT[];         // 8 (Upper Outermost EDT)
double ExtLOEDT[];         // 9 (Lower Outermost EDT)
...
int OnInit()
{
   ...
   PlotIndexSetString(7, PLOT_LABEL, "Top CFL Base Line");
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DavinTrade SSA CFL WLS V3.94");
   CreateExportButton();
...
void OnDeinit(const int reason)
{
   ...
   ObjectsDeleteAll(0, "ClusterHull_V394_");
   ObjectsDeleteAll(0, "ClusterCentroidStar_V394_");
...
void CreateExportButton()
{
   ...
   ObjectSetString(0, EXPORT_BUTTON_NAME, OBJPROP_TEXT, "Export V3.94 Data");
...
void PerformClusteringAndCFL(const int rates_total, const datetime &time[], const double &close_arr[])
{
   ...
   if(p_count < InpMinPts) {
      if(InpShowComments) Comment("--- DavinTrade V3.94 (CFL+WLS+EDT) ---\nAwaiting more data: Points < MinPts");
      return;
   }
   ...
   for(int k = 0; k < total_clusters; k++) {
      ...
      if(filtered_count >= 3) { 
         ...
         for(int h = 0; h < h_count; h++) {
            ...
            string line_name = "ClusterHull_V394_" + IntegerToString(k) + "_" + IntegerToString(h);
         ...
         string star_name = "ClusterCentroidStar_V394_" + IntegerToString(k);
   ...
   if(centroid_count < 3) {
      if(InpShowComments) Comment(StringFormat("--- DavinTrade V3.94 (CFL+WLS+EDT) ---\nAwaiting more data: Found %d / 3 Centroids needed.", centroid_count));
      return; 
   }
   ...
   for(int mask = 1; mask < total_combos; mask++) {
       ...
       for(int b=0; b<n_reg; b++) {
           if((mask & (1<<b)) != 0) {
               double w = cen_weights[b]; // Apply exponential decay weight
   ...
   if(InpShowComments) {
       string comment_text = StringFormat(
           "--- DavinTrade V3.94 A/B Statistical Pipeline ---\n" +
           "Centroids used in Top WLS CFL: %d\n" +
...
bool ExportData(bool silent = false)
{
   ...
   FileWrite(fh, "Regression Centroids (Best WLS CFL): " + IntegerToString(g_stat_centroids));
   FileWrite(fh, "Time-Decay Lambda: " + DoubleToString(g_stat_lambda, 4));
   ...
   // Refactored Export Header
   FileWrite(fh, "timestamp\tsymbol\ttimeframe\tclose\tCEN_0\tCEN_1\tCEN_2\tCEN_3\tCEN_4\tCEN_5\tCEN_6\tCEN_7\tCEN_8\tCEN_9\tCEN_10\tCEN_11\tCFL_1\tUOEDT\tLOEDT\thoriz_high_map\thoriz_low_map\tssa\tema_ssa\tcrossing");
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   if(prev_calculated == 0) {
       ...
       last_math_time = TimeLocal(); // Initial calculation is due
   } else if (new_bar) {
       math_update_due = true;
