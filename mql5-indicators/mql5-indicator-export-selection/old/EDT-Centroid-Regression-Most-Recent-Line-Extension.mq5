...
#property version   "3.891_2"
#property description "DavinTrade V3.891_2: Refactored with Configurable Line Extensions"
...
#define EXPORT_BUTTON_NAME "V3891_2_ExportButton"
...
input string            InpExportFileName = "DavinTrade_V3891_2_Data";
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DavinTrade SSA EDT V3.891_2");
   CreateExportButton();
...
void OnDeinit(const int reason)
{
   ...
   ObjectsDeleteAll(0, "ClusterHull_V891_2_");
   ObjectsDeleteAll(0, "ClusterCentroidStar_V891_2_");
...
void CreateExportButton()
{
   ...
   ObjectSetString(0, EXPORT_BUTTON_NAME, OBJPROP_TEXT, "Export V3.891_2 Data");
...
void PerformClusteringAndEDT(const int rates_total, const datetime &time[], const double &close_arr[])
{
   ...
   if(p_count < InpMinPts) {
      if(InpShowComments) Comment("--- DavinTrade V3.891_2 ---\nAwaiting more data: Points < MinPts");
      return;
   }
   ...
   for(int k = 0; k < total_clusters; k++) {
      ...
      if(filtered_count >= 3) { 
         ...
         for(int h = 0; h < h_count; h++) {
            ...
            string line_name = "ClusterHull_V891_2_" + IntegerToString(k) + "_" + IntegerToString(h);
         ...
         string star_name = "ClusterCentroidStar_V891_2_" + IntegerToString(k);
   ...
   if(InpShowComments) {
      string comment_text = StringFormat(
          "--- DavinTrade V3.891_2 A/B Statistical Pipeline ---\n" +
...
//+------------------------------------------------------------------+
//| Symmetrical EDT Snapping Logic (V3.891_2 Dynamic Strict Limits)  |
//+------------------------------------------------------------------+
