...
//|                                             https://www.mql5.com |
//|                                                                  |
//| Modified: Added V4 Math Optimizations, WP & 59s Auto-Export      |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, MetaQuotes Ltd."
#property link      "https://www.mql5.com"
#property version   "2.10"
#property description "Optimized EDT based on S&R FL with Auto-Refresh"
...
input int               InpStartBar = 1000;      
input int               InpEndBar = 200;         
...
input double            InpMinLineSeparationPercent = 1.0; 
input color             InpBaseLineColor = clrBlue;
input color             InpEDTColor = clrRed;    
...
input string            Sep5 = "===== Export & Timer Settings =====";
input string            InpExportFileName = "EDT_Lines_Opt";
input bool              InpIncludeHeader = true;
input bool              InpAutoUpdateTimer = true; // Auto-Calculate & Export at 59s
...
int OnInit()
  {
   ...
   ArraySetAsSeries(ExtEDT7, true); ArraySetAsSeries(ExtEDT8, true);
   ...
   PlotIndexSetInteger(0, PLOT_ARROW_SHIFT, -InpSymbolOffset); PlotIndexSetDouble(0, PLOT_EMPTY_VALUE, EMPTY_VALUE);
   ...
   PlotIndexSetInteger(1, PLOT_ARROW_SHIFT, InpSymbolOffset); PlotIndexSetDouble(1, PLOT_EMPTY_VALUE, EMPTY_VALUE);
   ...
   PlotIndexSetInteger(2, PLOT_ARROW_SHIFT, -InpSymbolOffset119); PlotIndexSetDouble(2, PLOT_EMPTY_VALUE, EMPTY_VALUE);
   ...
   PlotIndexSetInteger(3, PLOT_ARROW_SHIFT, InpSymbolOffset119); PlotIndexSetDouble(3, PLOT_EMPTY_VALUE, EMPTY_VALUE);

   // Configure Base S&R FL Plot
   PlotIndexSetInteger(4, PLOT_DRAW_TYPE, DRAW_LINE); PlotIndexSetInteger(4, PLOT_LINE_STYLE, STYLE_SOLID);
   PlotIndexSetInteger(4, PLOT_LINE_WIDTH, 2); PlotIndexSetInteger(4, PLOT_LINE_COLOR, InpBaseLineColor);
   PlotIndexSetDouble(4, PLOT_EMPTY_VALUE, EMPTY_VALUE); PlotIndexSetString(4, PLOT_LABEL, "Base S&R FL");

   // Configure EDT Plots
   for(int i = 5; i <= 13; i++) {
      PlotIndexSetInteger(i, PLOT_DRAW_TYPE, DRAW_LINE); PlotIndexSetInteger(i, PLOT_LINE_STYLE, STYLE_DASH);
      PlotIndexSetInteger(i, PLOT_LINE_WIDTH, 1); PlotIndexSetInteger(i, PLOT_LINE_COLOR, InpEDTColor);
      PlotIndexSetDouble(i, PLOT_EMPTY_VALUE, EMPTY_VALUE); PlotIndexSetString(i, PLOT_LABEL, "EDT #" + IntegerToString(i-4));
   }

   IndicatorSetString(INDICATOR_SHORTNAME, "Opt EDT Model WP Timer");
   CreateExportButton();
   
   if(InpAutoUpdateTimer) EventSetTimer(1);
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
  {
    if(id == CHARTEVENT_CUSTOM + 1000 && sparam == "EXPORT_ALL") {
        if(ExportEDTData(true)) Print("SUCCESS: [Export All] EDT data exported silently");
        else Print("ERROR: [Export All] Failed to export EDT data.");
        return;
    }

    if(id == CHARTEVENT_OBJECT_CLICK && sparam == EXPORT_BUTTON_NAME) {
        ObjectSetInteger(0, EXPORT_BUTTON_NAME, OBJPROP_STATE, false);
        ExportEDTData(false); ChartRedraw(0);
    }
  }

//+------------------------------------------------------------------+
//| Timer Event for 59s Auto-Recalculate & Export                    |
//+------------------------------------------------------------------+
void OnTimer()
  {
   if(!InpAutoUpdateTimer) return;
   
   MqlDateTime time_struct;
   TimeToStruct(TimeLocal(), time_struct); // Use PC Local Time so it fires even if MT5 ticks stop
   
   static int last_trigger_min = -1;
   
   if(time_struct.sec == 59 && time_struct.min != last_trigger_min) {
       last_trigger_min = time_struct.min;
       
       // Use ArraySize to guarantee we don't exceed indicator buffer bounds asynchronously
       int current_rates = ArraySize(ExtUpperBuffer); 
       
       if(current_rates > 0) {
           double current_close = iClose(_Symbol, _Period, 0);
           BuildEDTLines(current_rates, current_close);
           ChartRedraw(0);
           ExportEDTData(true); // Export silently to prevent log spam
       }
   }
  }
...
//+------------------------------------------------------------------+
//| Main EDT Algorithm with Window Period & V4 Optimizations         |
//+------------------------------------------------------------------+
      ...
      for(int j = i + 1; j < f_count; j++) {
         ...
         if(touches >= InpMinTouches && pass_flip) {
            ...
            if(current_score > BaseLine.score) {
               BaseLine.slope = slope;
               BaseLine.y_intercept = y_intercept;
               BaseLine.bar_start = fractals[i].bar;
               BaseLine.touches = touches;
               BaseLine.score = current_score;
            }
         }
      }
   }

   if(BaseLine.score < 0) return; 
   ...
   for(int L = 0; L < final_count; L++) {
      ...
      for(int i = 0; i <= start_idx; i++) {
         ...
         if(chrono_bar >= final_edts[L].bar_start) {
            ...
            switch(L) {
               case 0: ExtEDT1[i] = val; break; 
               case 1: ExtEDT2[i] = val; break;
               case 2: ExtEDT3[i] = val; break; 
               case 3: ExtEDT4[i] = val; break;
               case 4: ExtEDT5[i] = val; break; 
               case 5: ExtEDT6[i] = val; break;
               case 6: ExtEDT7[i] = val; break;
               case 7: ExtEDT8[i] = val; break;
...
//+------------------------------------------------------------------+
//| Export trendline data to file (Added Silent Parameter)           |
//+------------------------------------------------------------------+
bool ExportEDTData(bool silent = false)
  {
    ...
    if(write_success && !silent) Print("EDT data exported to: ", filename);
