...
#property version   "3.891_1"
#property description "DavinTrade V3.891_1: Refactored for Multi-Chart Compatibility"
...
#define EXPORT_BUTTON_NAME "V3891_1_ExportButton"
...
input string            InpExportFileName = "DavinTrade_V3891_1_Data";
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DavinTrade SSA EDT V3.891_1");
   CreateExportButton();
...
void OnDeinit(const int reason)
{
   ...
   ObjectsDeleteAll(0, "ClusterHull_V891_1_");
   ObjectsDeleteAll(0, "ClusterCentroidStar_V891_1_");
...
void CreateExportButton()
{
   ...
   // Shifted YDISTANCE to 150 to clear V3.894 which uses 110
   ...
   ObjectSetString(0, EXPORT_BUTTON_NAME, OBJPROP_TEXT, "Export V3.891_1 Data");
...
void PerformClusteringAndEDT(const int rates_total, const datetime &time[], const double &close_arr[])
{
   ...
   if(p_count < InpMinPts) {
      if(InpShowComments) Comment("--- DavinTrade V3.891_1 ---\nAwaiting more data: Points < MinPts");
      return;
   }
   ...
   for(int k = 0; k < total_clusters; k++) {
      ...
      if(filtered_count >= 3) { 
         ...
         for(int h = 0; h < h_count; h++) {
            ...
            string line_name = "ClusterHull_V891_1_" + IntegerToString(k) + "_" + IntegerToString(h);
         ...
         string star_name = "ClusterCentroidStar_V891_1_" + IntegerToString(k);
   ...
   if(InpShowComments) {
      string comment_text = StringFormat(
          "--- DavinTrade V3.891_1 A/B Statistical Pipeline ---\n" +
   ...
   // FIX: Bound fractal collection to the regression observation window
   for(int i = leftmost_bar; i <= rightmost_bar; i++) {
...
//+------------------------------------------------------------------+
//| Symmetrical EDT Snapping Logic (V3.891_1 Dynamic Strict Limits)  |
//+------------------------------------------------------------------+
void BuildSymmetricalEDTs(double base_m, double base_c, const FractalPoint &fractals[], int rates_total, double current_close, int drawStartIdx)
{
   ...
   ArrayInitialize(ExtEDT3, EMPTY_VALUE);
   ...
   for(int i = 0; i < f_count; i++) {
      ...
      if(touches >= InpEDTMinTouches) {
         ...
         tl.touches = touches;
         // FIX: Penalize lines that are too far from the baseline intercept
   ...
   // Cap at 8 to respect strict buffers
