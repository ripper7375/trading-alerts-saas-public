...
//|                                             https://www.mql5.com |
//|
//| Modified: Added V4 Math Optimizations & 1 Window Period          |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, MetaQuotes Ltd."
#property link      "https://www.mql5.com"
#property version   "2.00"
#property description "Optimized Equal Distance Trendlines (EDT) based on S&R FL"
...
input int               InpStartBar = 1000;      // Starting bar of Window Period
input int               InpEndBar = 200;         // Ending bar of Window Period
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Opt EDT Model WP");
   CreateExportButton();
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
  {
    if(id == CHARTEVENT_CUSTOM + 1000 && sparam == "EXPORT_ALL") {
        if(ExportEDTData()) Print("SUCCESS: [Export All] EDT data exported successfully");
...
// OPTIMIZED: Uses pre-fetched ATR
...
void BuildEDTLines(const int rates_total, double current_close)
  {
   ...
   // 1. Gather all fractals strictly WITHIN the Window Period
   ...
   if(BaseLine.score < 0) return; // No Base Line found
