...
#property description "Fractal Clusters with Centroid Equilibrium Lines"
...
void PerformClustering(const int rates_total, const datetime &time[])
  {
   ...
   for(int k = 0; k < InpClusterCount; k++) {
      ...
      if(ArraySize(cluster_points) >= 3) {
         ...
         ObjectSetInteger(0, cent_name, OBJPROP_STYLE, STYLE_DASHDOT); // Visually distinct
         ObjectSetInteger(0, cent_name, OBJPROP_RAY_RIGHT, true);      // Extend to the right
