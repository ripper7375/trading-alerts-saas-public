//+------------------------------------------------------------------+
//|          Fractal S&R Multi-Point Trendlines V5.54-R1            |
//|                             Copyright 2000-2025, MetaQuotes Ltd. |
//|  AUGMENTED: Least-squares regression consensus price per line    |
//+------------------------------------------------------------------+
...
//--- Regression consensus price buffers (non-plotted; parallel to the 6 line buffers)
double ExtRegrPeakLine1[];
double ExtRegrPeakLine2[];
double ExtRegrPeakLine3[];
double ExtRegrBottomLine1[];
double ExtRegrBottomLine2[];
double ExtRegrBottomLine3[];
...
struct FractalLine
  {
   ...
   int               touched_bars[];
   double            touched_prices[];       // prices of all touching fractals (for regression)
   int               length_bars;
   double            slope;
...
private:
   ...
   double    m_line_regression_slope[];
   double    m_line_regression_intercept[];
...
public:
   ...
   void Store(FractalLine &lines[], double &high_map[], double &low_map[], int lookback)
   {
      ...
      ArrayResize(m_line_regression_slope, line_count);
      ArrayResize(m_line_regression_intercept, line_count);
      ...
      for(int i = 0; i < line_count; i++)
      {
         ...
         m_line_regression_slope[i] = lines[i].regression_slope;
         m_line_regression_intercept[i] = lines[i].regression_intercept;
      }
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Fractal SR Regression (V5.54-R1)");

   Print("=== Fractal SR V5.54-R1 (Regression) ===");
   Print("  Regression consensus price computed via least-squares through all touches");
    ...
    if(InpIncludeHeader)
      {
        string header = "timestamp\tsymbol\ttimeframe\tclose"
                        ...
                        "\tregr_peak_line_1\tregr_peak_line_2\tregr_peak_line_3"
                        "\tregr_bottom_line_1\tregr_bottom_line_2\tregr_bottom_line_3";
    ...
    for(int row = 0; row < copied && row < InpExportBars; row++)
      {
        ...
        // Regression consensus prices (6 columns)
        string rp1 = (buf_idx >= ArraySize(ExtRegrPeakLine1)   || ExtRegrPeakLine1[buf_idx]   == EMPTY_VALUE || ExtRegrPeakLine1[buf_idx]   == 0.0) ? "" : DoubleToString(ExtRegrPeakLine1[buf_idx],   2);
        line += rp1 + "\t";
        string rp2 = (buf_idx >= ArraySize(ExtRegrPeakLine2)   || ExtRegrPeakLine2[buf_idx]   == EMPTY_VALUE || ExtRegrPeakLine2[buf_idx]   == 0.0) ? "" : DoubleToString(ExtRegrPeakLine2[buf_idx],   2);
        line += rp2 + "\t";
        string rp3 = (buf_idx >= ArraySize(ExtRegrPeakLine3)   || ExtRegrPeakLine3[buf_idx]   == EMPTY_VALUE || ExtRegrPeakLine3[buf_idx]   == 0.0) ? "" : DoubleToString(ExtRegrPeakLine3[buf_idx],   2);
        line += rp3 + "\t";
        string rb1 = (buf_idx >= ArraySize(ExtRegrBottomLine1) || ExtRegrBottomLine1[buf_idx] == EMPTY_VALUE || ExtRegrBottomLine1[buf_idx] == 0.0) ? "" : DoubleToString(ExtRegrBottomLine1[buf_idx], 2);
        line += rb1 + "\t";
        string rb2 = (buf_idx >= ArraySize(ExtRegrBottomLine2) || ExtRegrBottomLine2[buf_idx] == EMPTY_VALUE || ExtRegrBottomLine2[buf_idx] == 0.0) ? "" : DoubleToString(ExtRegrBottomLine2[buf_idx], 2);
        line += rb2 + "\t";
        string rb3 = (buf_idx >= ArraySize(ExtRegrBottomLine3) || ExtRegrBottomLine3[buf_idx] == EMPTY_VALUE || ExtRegrBottomLine3[buf_idx] == 0.0) ? "" : DoubleToString(ExtRegrBottomLine3[buf_idx], 2);
        line += rb3;
...
void ClearAllTrendlineBuffers()
  {
   ...
   ArrayInitialize(ExtRegrPeakLine1, EMPTY_VALUE);
   ArrayInitialize(ExtRegrPeakLine2, EMPTY_VALUE);
   ArrayInitialize(ExtRegrPeakLine3, EMPTY_VALUE);
   ArrayInitialize(ExtRegrBottomLine1, EMPTY_VALUE);
   ArrayInitialize(ExtRegrBottomLine2, EMPTY_VALUE);
   ArrayInitialize(ExtRegrBottomLine3, EMPTY_VALUE);
  }
                                   ...
                                   bool is_peak_lines)
{
   ...
   for(int i = 0; i < fractal_count - 1; i++)
   {
      ...
      for(int j = i + 1; j < fractal_count; j++)
      {
         ...
         //--- Collect touched bar indices and prices for regression
         int    reg_bars[];
         double reg_prices[];
         ArrayResize(reg_bars,   touched_count);
         ArrayResize(reg_prices, touched_count);
         for(int t = 0; t < touched_count; t++)
         {
            reg_bars[t]   = fractal_bars[touched_indices[t]];
         ...
         for(int t = 0; t < touched_count; t++)
         {
            ...
            lines[line_count].touched_prices[t] = fractal_prices[touched_indices[t]];
         }
...
//+------------------------------------------------------------------+
//| Fill a buffer with regression consensus prices over the same     |
//| bar range as the original trendline.                             |
//+------------------------------------------------------------------+
void DrawRegressionLine(double &buffer[], FractalLine &line, int rates_total)
{
   ...
   int start_bar = line.bar_start;
   int end_bar   = (InpExtensionBars > 0) ? rates_total - 1 : line.bar_end;

   for(int i = 0; i < rates_total; i++)
   {
      int bar_number = rates_total - 1 - i;
      if(bar_number >= start_bar && bar_number <= end_bar)
         buffer[i] = reg_slope * bar_number + reg_intercept;
      else
         buffer[i] = EMPTY_VALUE;
   }
}
...
void BuildMultiPointTrendlines(const int rates_total)
{
   ...
   if(ArraySize(peak_lines) >= 1)
   {
      ...
      DrawRegressionLine(ExtRegrPeakLine1, peak_lines[0], rates_total);
      if(InpEnableColorIntensity)
      {
   ...
   if(ArraySize(peak_lines) >= 2)
   {
      ...
      DrawRegressionLine(ExtRegrPeakLine2, peak_lines[1], rates_total);
      if(InpEnableColorIntensity)
      {
   ...
   if(ArraySize(peak_lines) >= 3)
   {
      ...
      DrawRegressionLine(ExtRegrPeakLine3, peak_lines[2], rates_total);
      if(InpEnableColorIntensity)
      {
   ...
   if(ArraySize(bottom_lines) >= 1)
   {
      ...
      DrawRegressionLine(ExtRegrBottomLine1, bottom_lines[0], rates_total);
      if(InpEnableColorIntensity)
      {
   ...
   if(ArraySize(bottom_lines) >= 2)
   {
      ...
      DrawRegressionLine(ExtRegrBottomLine2, bottom_lines[1], rates_total);
      if(InpEnableColorIntensity)
      {
   ...
   if(ArraySize(bottom_lines) >= 3)
   {
      ...
      DrawRegressionLine(ExtRegrBottomLine3, bottom_lines[2], rates_total);
      if(InpEnableColorIntensity)
      {
