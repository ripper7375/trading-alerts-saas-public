...
#property version   "3.894"
#property description "DavinTrade V3.894: Statistical Sync & Rightmost Boundary Enforced"
...
#define EXPORT_BUTTON_NAME "V3894ExportButton"
...
input string            InpExportFileName = "DavinTrade_V3894_Data";
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DavinTrade SSA EDT V3.894");
   CreateExportButton();
...
void CreateExportButton()
{
   ...
   ObjectSetString(0, EXPORT_BUTTON_NAME, OBJPROP_TEXT, "Export V3.894 Data");
...
void PerformClusteringAndEDT(const int rates_total, const datetime &time[], const double &close_arr[])
{
   ...
   if(p_count < InpMinPts) {
      if(InpShowComments) Comment("--- DavinTrade V3.894 ---\nAwaiting more data: Points < MinPts");
      return;
   }
   ...
   if(drawEndIdx < drawStartIdx || drawEndIdx >= rates_total) drawEndIdx = rates_total - 1; // Math Failsafe bounding

   // Capped loop to cleanly slice line rendering at Box B bounds
   for(int i = drawStartIdx; i <= drawEndIdx; i++) {
      ExtBaseLine[i] = base_m * i + base_c;
   }
   
   if(InpShowComments) {
      string comment_text = StringFormat(
          "--- DavinTrade V3.894 A/B Statistical Pipeline ---\n" +
   ...
   BuildSymmetricalEDTs(base_m, base_c, fractals, rates_total, close_arr[rates_total-1], drawStartIdx, drawEndIdx, min_price, max_price);
}

//+------------------------------------------------------------------+
//| Symmetrical EDT Snapping Logic (V3.894 Dynamic Strict Limits)     |
//+------------------------------------------------------------------+
void BuildSymmetricalEDTs(double base_m, double base_c, const FractalPoint &fractals[], int rates_total, double current_close, int drawStartIdx, int drawEndIdx, double channel_min, double channel_max)
{
   ...
   for(int i = 0; i < count_above && picked_above < target_above; i++) {
      ...
      if(!too_close) {
         ...
         for(int k = drawStartIdx; k <= drawEndIdx; k++) { // Restricted Loop for Rightmost sync
