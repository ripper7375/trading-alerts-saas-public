      ...
      if(trend.Size() == len)
      {
         ...
         for(int i = 1; i < len; i++)
         {
            ...
            if(crossUp || crossDown)
               ssaCrossBuffer[idx] = ssaTrendBuffer[idx]; // Plot star at SSA Trend value
            else
   ...
   else
   {
      ...
      if(trend.Size() == 1)
      {
         ...
         if(ssaTrendBuffer[prev] != EMPTY_VALUE && ssaSignalBuffer[prev] != EMPTY_VALUE)
         {
            ...
            if(crossUp || crossDown)
               ssaCrossBuffer[last] = ssaTrendBuffer[last];
            else
...
//| Columns : timestamp symbol timeframe close ssa ema_ssa ssa_high ssa_low SSA Crossing |
...
bool ExportData()
{
   ...
   //--- Prepare lowercase timeframe suffix for headers
   string tf_lower = tf_str;
   StringToLower(tf_lower);

   //--- Header row with dynamic timeframe headers appended
   string header_row = StringFormat("timestamp_%s\tsymbol_%s\ttimeframe_%s\tclose_%s\tssa_%s\tema_ssa_%s\tssa_high_%s\tssa_low_%s\tSSA Crossing_%s",
                                    tf_lower, tf_lower, tf_lower, tf_lower, tf_lower, tf_lower, tf_lower, tf_lower, tf_lower);
   ok &= FileWrite(fh, header_row) > 0;
   ...
   for(int i = 0; i < bars_to_export; i++)
   {
      ...
      //--- SSA Crossing format (1 for cross, 0 for no cross)
      string ssa_cross_str = "0"; // default to 0
