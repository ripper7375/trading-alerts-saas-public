...
#property description "Fractal Clusters with Centroid Equilibrium Lines, Stars, and S/R Levels"
...
void OnDeinit(const int reason)
  {
   ...
   ObjectsDeleteAll(0, "ClusterResist_"); // Delete resistance lines
   ObjectsDeleteAll(0, "ClusterSupport_"); // Delete support lines
  }
...
void PerformClustering(const int rates_total, const datetime &time[])
  {
   ...
   for(int k = 0; k < InpClusterCount; k++) {
      ...
      // 5. Draw Bounding Polygon, Centroid Line, and S/R Lines
      if(ArraySize(cluster_points) >= 3) {
         ...
         // --- ADDED FIX: Recalculate centroid based ONLY on filtered points ---
         ...
         // Draw Resistance Line (Highest Fractal)
         if(highest_time != 0) {
         ...
         // Draw Support Line (Lowest Fractal)
         if(lowest_time != 0) {
         ...
         // 6. De-normalize UPDATED Centroid & Draw Equilibrium Line
         // NOTE: Using final_center_y and final_center_x instead of center_y and center_x
