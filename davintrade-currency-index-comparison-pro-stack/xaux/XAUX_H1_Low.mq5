...
#property description "Gold Index (XAUX) Low Indicator with H1 Optimization"
...
input double XAUUSD_Inception = 4324.44;  // XAUUSD Inception Rate
input double XAUEUR_Inception = 3681.01;  // XAUEUR Inception Rate
input double XAUJPY_Inception = 677958.0; // XAUJPY Inception Rate
input double XAUGBP_Inception = 3209.67;  // XAUGBP Inception Rate
input double XAUAUD_Inception = 6482.43;  // XAUAUD Inception Rate
...
int OnInit()
{
    ...
    LabelPrefix = "XAUX_LOW_" + IntegerToString(GetTickCount64());
...
bool InitializeBuffer()
{
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "XAUX Index Low");
...
void ValidateSymbols()
{
    ...
    CommentText = "=== XAUX Low Indicator ===\n";
...
//+------------------------------------------------------------------+
//| Calculate XAUX Low value for a specific position                 |
//+------------------------------------------------------------------+
double CalculateXAUX(int position)
{
    ...
    // To minimize Gold price in each currency:
    // Direct pairs: use Low price
    // Inverse pairs: use High price (denominator maximizes, minimizing ratio)
    double p_xauusd = iLow(Pairs[0].symbol, PERIOD_H1, position);
    double p_eurusd = Pairs[1].available ? iHigh(Pairs[1].symbol, PERIOD_H1, position) : 0.0;
    double p_usdjpy = Pairs[2].available ? iLow(Pairs[2].symbol, PERIOD_H1, position)  : 0.0;
    double p_gbpusd = Pairs[3].available ? iHigh(Pairs[3].symbol, PERIOD_H1, position) : 0.0;
    double p_audusd = Pairs[4].available ? iHigh(Pairs[4].symbol, PERIOD_H1, position) : 0.0;
    
    if(p_xauusd <= 0 || !MathIsValidNumber(p_xauusd))
        return EMPTY_VALUE;
        
    // 2. Translate fetched prices into the 5 Gold basket assets (Low)
    ...
    if(currentHourTime > lastHourTime)
    {
        if(lastHourValue != EMPTY_VALUE)
        {
            ...
            {
                Print("XAUX Low value exceeded ±3% threshold. Current: ", xaux_value, 
                      " Last: ", lastHourValue, 
