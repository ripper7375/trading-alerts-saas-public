...
#property indicator_color1  clrDodgerBlue,clrPink  // Only two colors: DodgerBlue for bullish, Pink for bearish
...
input color LabelColor = clrGold;           // Label Color
...
// Buffers for OHLC values
double OpenBuffer[];
double HighBuffer[];
double LowBuffer[];
double CloseBuffer[];
double ColorBuffer[];        // For candle colors
...
string GetIndicatorName()
{
    ...
    return StringSubstr(shortName, 0, 4);  // Return first 4 characters (e.g. AUDX, CHFX, CADX, XAUX)
}
...
int OnInit()
{
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "XAUX H1 Candles");
    
    // Get indicator handles (with failover path)
    handle_open = iCustom(Symbol(), Period(), "XAUX_H1_Open", false, false);
    if(handle_open == INVALID_HANDLE)
        handle_open = iCustom(Symbol(), Period(), "xaux\\XAUX_H1_Open", false, false);

    handle_high = iCustom(Symbol(), Period(), "XAUX_H1_High", false, false);
    if(handle_high == INVALID_HANDLE)
        handle_high = iCustom(Symbol(), Period(), "xaux\\XAUX_H1_High", false, false);

    handle_low = iCustom(Symbol(), Period(), "XAUX_H1_Low", false, false);
    if(handle_low == INVALID_HANDLE)
        handle_low = iCustom(Symbol(), Period(), "xaux\\XAUX_H1_Low", false, false);

    handle_close = iCustom(Symbol(), Period(), "XAUX_H1_Close", false, false);
    if(handle_close == INVALID_HANDLE)
        handle_close = iCustom(Symbol(), Period(), "xaux\\XAUX_H1_Close", false, false);
    ...
    indexName = GetIndicatorName();  // Will return XAUX
                ...
                const int &spread[])
{
    ...
    for(int i = start; i < rates_total && !IsStopped(); i++)
    {
        ...
        // Ensure candle integrity (High >= max(Open, Close), Low <= min(Open, Close))
        double valid_high = MathMax(high_price, MathMax(open_price, close_price));
        double valid_low  = MathMin(low_price, MathMin(open_price, close_price));
        
        // Set color based on candle direction (0 for bullish - DodgerBlue, 1 for bearish - Pink)
        ColorBuffer[i] = close_price >= open_price ? 0 : 1;
        
        // Store OHLC values
        OpenBuffer[i] = open_price;
        HighBuffer[i] = valid_high;
        LowBuffer[i] = valid_low;
        CloseBuffer[i] = close_price;
    }
    
    // Modify label positioning using the offset
    if(ShowCurrencyIndexLabel && rates_total > 0)
    {
        double open_values_latest[];
        ArraySetAsSeries(open_values_latest, true);
        
        // Get the latest open value
        if(CopyBuffer(handle_open, 0, 0, 1, open_values_latest) > 0)
        {
            // Position with user-defined offset
            datetime currentTime = iTime(_Symbol, Period(), 0);
            datetime labelTime = currentTime + (PeriodSeconds(Period()) * LabelRightOffset);
            double openPrice = open_values_latest[0];
