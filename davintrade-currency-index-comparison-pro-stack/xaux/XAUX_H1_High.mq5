...
#property description "Gold Index (XAUX) High Indicator with H1 Optimization"
...
// Inception Rates (High)
input group "Inception Rates"
input double XAUUSD_Inception = 4356.14;  // XAUUSD Inception Rate
input double XAUEUR_Inception = 3705.17;  // XAUEUR Inception Rate
input double XAUJPY_Inception = 682132.0; // XAUJPY Inception Rate
input double XAUGBP_Inception = 3230.99;  // XAUGBP Inception Rate
input double XAUAUD_Inception = 6516.22;  // XAUAUD Inception Rate
...
int OnInit()
{
    ...
    LabelPrefix = "XAUX_HIGH_" + IntegerToString(GetTickCount64());
...
bool InitializeBuffer()
{
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "XAUX Index High");
...
void ValidateSymbols()
{
    ...
    CommentText = "=== XAUX High Indicator ===\n";
...
//+------------------------------------------------------------------+
//| Calculate XAUX High value for a specific position                |
//+------------------------------------------------------------------+
double CalculateXAUX(int position)
{
    ...
    // 1. Fetch appropriate extremal prices from MT5:
    // To maximize Gold price in each currency:
    // Direct pairs: use High price
    // Inverse pairs: use Low price (denominator minimizes, maximizing ratio)
    double p_xauusd = iHigh(Pairs[0].symbol, PERIOD_H1, position);
    double p_eurusd = Pairs[1].available ? iLow(Pairs[1].symbol, PERIOD_H1, position)  : 0.0;
    double p_usdjpy = Pairs[2].available ? iHigh(Pairs[2].symbol, PERIOD_H1, position) : 0.0;
    double p_gbpusd = Pairs[3].available ? iLow(Pairs[3].symbol, PERIOD_H1, position)  : 0.0;
    double p_audusd = Pairs[4].available ? iLow(Pairs[4].symbol, PERIOD_H1, position)  : 0.0;
    
    if(p_xauusd <= 0 || !MathIsValidNumber(p_xauusd))
        return EMPTY_VALUE;
        
    // 2. Translate fetched prices into the 5 Gold basket assets (High)
    ...
    if(currentHourTime > lastHourTime)
    {
        if(lastHourValue != EMPTY_VALUE)
        {
            ...
            {
                Print("XAUX High value exceeded ±3% threshold. Current: ", xaux_value, 
                      " Last: ", lastHourValue, 
