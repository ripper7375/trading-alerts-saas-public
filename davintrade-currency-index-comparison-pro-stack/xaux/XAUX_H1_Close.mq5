...
#property description "Gold Index (XAUX) Close Indicator with H1 Optimization"
...
// Inception Rates (Close)
input group "Inception Rates"
input double XAUUSD_Inception = 4348.18;  // XAUUSD Inception Rate
input double XAUEUR_Inception = 3696.91;  // XAUEUR Inception Rate
input double XAUJPY_Inception = 681037.0; // XAUJPY Inception Rate
input double XAUGBP_Inception = 3223.65;  // XAUGBP Inception Rate
input double XAUAUD_Inception = 6496.04;  // XAUAUD Inception Rate
...
int OnInit()
{
    ...
    LabelPrefix = "XAUX_CLOSE_" + IntegerToString(GetTickCount64());
...
bool InitializeBuffer()
{
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "XAUX Index Close");
...
void ValidateSymbols()
{
    ...
    CommentText = "=== XAUX Close Indicator ===\n";
...
//+------------------------------------------------------------------+
//| Calculate XAUX Close value for a specific position               |
//+------------------------------------------------------------------+
double CalculateXAUX(int position)
{
    ...
    // 1. Fetch real prices from MT5 for XAUUSD, EURUSD, USDJPY, GBPUSD, AUDUSD
    double p_xauusd = iClose(Pairs[0].symbol, PERIOD_H1, position);
    double p_eurusd = Pairs[1].available ? iClose(Pairs[1].symbol, PERIOD_H1, position) : 0.0;
    double p_usdjpy = Pairs[2].available ? iClose(Pairs[2].symbol, PERIOD_H1, position) : 0.0;
    double p_gbpusd = Pairs[3].available ? iClose(Pairs[3].symbol, PERIOD_H1, position) : 0.0;
    double p_audusd = Pairs[4].available ? iClose(Pairs[4].symbol, PERIOD_H1, position) : 0.0;
    
    if(p_xauusd <= 0 || !MathIsValidNumber(p_xauusd))
        return EMPTY_VALUE;
        
    // 2. Translate fetched prices into the 5 Gold basket assets
    ...
    if(currentHourTime > lastHourTime)
    {
        if(lastHourValue != EMPTY_VALUE)
        {
            ...
            {
                Print("XAUX value exceeded ±3% threshold. Current: ", xaux_value, 
                      " Last: ", lastHourValue, 
