...
/**
 * Ported from seed-code's landing-pricing.tsx, which hardcoded the PRO price
 * ($49/month) with no live-data binding at all. The main repo's prior landing
 * page (app/(marketing)/landing-content.tsx, retired this session) had real,
 * SystemConfig-backed affiliate pricing via useAffiliateConfig() -- a
 * ?ref=CODE query param drives a discount banner + discounted price +
 * commission calc. Grafted that wiring into this component's new visual
 * design rather than dropping it (Session 9-2, Decision per Davin's live
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 md:px-6">
        ...
        {/* Affiliate discount banner -- only when a ?ref= code is present */}
        {affiliateCode && (
          <div className="mx-auto mb-10 max-w-2xl rounded-2xl border border-emerald-500/40 bg-gradient-to-r from-emerald-500/15 via-emerald-500/5 to-slate-100 p-4 text-center dark:from-emerald-500/10 dark:via-[#0d101a] dark:to-[#090b14]">
            <p className="text-sm text-slate-700 dark:text-slate-300">
              <span className="font-bold text-emerald-700 dark:text-emerald-400">
                {t('Special Offer!')}
              </span>{' '}
              {t('Sign up with code')}{' '}
              <code className="rounded bg-emerald-500/15 px-2 py-0.5 font-mono text-xs text-emerald-800 dark:text-emerald-200">
                {affiliateCode}
              </code>{' '}
              {t('and get')}{' '}
              <span className="font-bold text-emerald-700 dark:text-emerald-400">
                {discountPercent}% {t('off your first month!')}
              </span>
            </p>
          </div>
        )}
        ...
        <div className="mx-auto grid max-w-4xl grid-cols-1 gap-8 md:grid-cols-2">
          ...
          <div className="relative flex flex-col justify-between rounded-2xl border-2 border-amber-500/60 bg-gradient-to-b from-amber-50 to-white p-8 shadow-2xl shadow-amber-500/10 dark:from-[#131826] dark:to-[#0c0f18]">
            <div className="absolute -top-3.5 right-8 rounded-full bg-gradient-to-r from-amber-500 to-amber-600 px-3 py-1 text-xs font-bold uppercase tracking-wider text-slate-950 shadow-md">
              {t('POPULAR PRO TIER')}
            </div>
