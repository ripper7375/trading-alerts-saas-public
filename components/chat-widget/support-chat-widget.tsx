...
// Ported + adapted from
// seed-code/trading-conversational-ai-ui-pages-increment/components/chat-widget/support-chat-widget.tsx
// (Session 14-2, Decision 3). Adds the chat_error banner (RATE_LIMIT_EXCEEDED /
...
import type { ChatError } from '@/lib/socket-client';
...
const SUPPORT_EMAIL = 'support@davintrade.app';
...
function ChatErrorBanner({
  ...
  return (
    <div className="mx-4 mt-2 flex items-start justify-between gap-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-[11px] leading-relaxed text-amber-800 dark:text-amber-300">
      <div className="flex items-start gap-1.5">
        <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
        <span>
          {message}{' '}
          {(error.code === 'SERVER_ERROR' || error.code === 'UNAUTHORIZED') && (
            <a
...
export function SupportChatWidget() {
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 z-50 w-[92vw] rounded-2xl border border-slate-700/80 bg-white/95 shadow-2xl shadow-black/90 backdrop-blur-2xl transition-all duration-300 dark:bg-[#0a0d16]/95 sm:right-6 sm:w-[420px] ${
        isMinimized
    ...
    >
      ...
      {!isMinimized && (
        <>
          ...
          <div className="scrollbar-thin scrollbar-thumb-slate-800 flex-1 space-y-3 overflow-y-auto p-4">
            {messages.map((msg) => (
              <div
                key={msg.id}
            ...
            {isTyping && (
              <div className="flex items-center space-x-2 font-mono text-xs italic text-amber-700 dark:text-amber-400/90">
          ...
          <div className="no-scrollbar flex items-center space-x-1.5 overflow-x-auto border-t border-slate-200 bg-slate-50 px-4 py-2 dark:border-slate-800/80 dark:bg-[#080b13]">
            {topicOptions.map((topic) => (
              <button
                ...
                className={`whitespace-nowrap rounded-full border px-2.5 py-1 text-[11px] font-semibold transition-all ${
                  activeTopic === topic
