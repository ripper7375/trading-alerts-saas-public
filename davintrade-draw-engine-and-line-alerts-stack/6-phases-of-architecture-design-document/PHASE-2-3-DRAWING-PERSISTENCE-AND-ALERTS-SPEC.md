...
export const DrawingTypeZ = z.enum([
  'TRENDLINE',
  'HLINE',
  'CHANNEL',
  'FIB_RETRACE',
...
export const DrawingUpdateZ = DrawingCreateZ.partial().omit({
  symbol: true,
  timeframe: true,
  type: true,
});
...
export async function publishAlertsChanged(p: {
  ...
  await redis.publish('alerts:changed', JSON.stringify(p));
}
```
