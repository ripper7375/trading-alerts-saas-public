...
jest.mock('@/lib/cache/redis', () => ({
  ...
  invalidatePattern: (...args: unknown[]) =>
    mockRedisInvalidatePattern(...args),
}));

// Mock the database queries module
const mockGetIndicatorDataFromDb = jest.fn(async () => ({
  ohlc: [
    { time: Date.now() / 1000, open: 1.0, high: 1.01, low: 0.99, close: 1.0 },
  ],
...
jest.mock('@/lib/db/queries', () => ({
  __esModule: true,
  getIndicatorDataFromDb: (...args: unknown[]) =>
    mockGetIndicatorDataFromDb(...args),
}));
...
describe('Cache Integration', () => {
  ...
  describe('Cache-through pattern', () => {
    it('should return cached data on cache hit', async () => {
      ...
      const cachedData = {
        ohlc: [
          { time: 123456, open: 1.08, high: 1.09, low: 1.07, close: 1.085 },
        ],
        fractals: { peaks: [], bottoms: [] },
        horizontal_trendlines: { support: [], resistance: [] },
        diagonal_trendlines: { support: [], resistance: [] },
  ...
  describe('Cache invalidation', () => {
    it('should invalidate specific symbol/timeframe cache', async () => {
      ...
      const h1Keys = Array.from(mockRedisStore.keys()).filter((k) =>
        k.includes(':H1:')
      );
