...
jest.mock('@/lib/db/queries', () => ({
  __esModule: true,
  getIndicatorDataFromDb: jest.fn(async (symbol: string, timeframe: string) => {
    const key = `${symbol}:${timeframe}`;
    return mockDbData.get(key) || {
...
describe('Cache Integration', () => {
  ...
  describe('Cache invalidation', () => {
    it('should invalidate specific symbol/timeframe cache', async () => {
      ...
      // H1 should be cleared
      expect(mockRedisStore.has(cacheKey)).toBe(false);
    });
  ...
  describe('Cache statistics', () => {
    ...
    it('should calculate hit rate correctly', async () => {
      // Create 1 hit and 1 miss
      await getIndicatorDataCached('EURUSD', 'H1', 100); // miss
      await getIndicatorDataCached('EURUSD', 'H1', 100); // hit
  ...
  describe('Redis fallback behavior', () => {
    it('should fall back to database when Redis fails', async () => {
      ...
      (redis.get as jest.Mock).mockRejectedValueOnce(new Error('Redis connection failed'));
      ...
      // Restore
      (redis.get as jest.Mock).mockImplementation(originalGet);
    });
  });
});
