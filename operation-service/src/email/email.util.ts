// Ported verbatim from lib/email/email.ts (Session 3-4, F29: Davin's decision
// to port the Resend email-sending logic directly into operation-service so
// the service is self-contained rather than calling back into the monolith).
// Kept as a full copy — including templates not yet called by anything in
// this service (alert-triggered, subscription, trial-reminder, upgrade-
// prompt, new-device-login, password-changed) — rather than a curated
// subset, matching how errors.ts was ported (L4: a partial port that
// silently drops behavior is the hazard, not the extra code). NEXTAUTH_URL
// is reused as-is (not renamed) for the same reason auth.service.ts reuses
// NEXTAUTH_SECRET verbatim: it's the one env var name every consumer of
