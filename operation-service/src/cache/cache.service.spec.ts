...
describe('CacheService', () => {
  ...
  describe('get', () => {
    it('returns null when the key does not exist', async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith('op:cache:missing');
    });
    ...
    it('parses and returns the stored value', async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith('op:cache:key-1');
    });
  });
  ...
  describe('del', () => {
    it('deletes the prefixed key', async () => {
      ...
      expect(mockClient.del).toHaveBeenCalledWith('op:cache:key-1');
    });
  });
  ...
  describe('ttl', () => {
    it('returns the remaining TTL for the prefixed key', async () => {
      ...
      expect(mockClient.ttl).toHaveBeenCalledWith('op:cache:key-1');
    });
  });
  ...
  describe('flushPattern', () => {
    ...
    it('deletes every key found across a single scan batch', async () => {
      mockClient.scan.mockResolvedValue([
        '0',
        ['op:cache:session:a', 'op:cache:session:b'],
      ]);
    ...
    it('iterates until the cursor returns to 0 across multiple scan batches', async () => {
      mockClient.scan
        .mockResolvedValueOnce(['17', ['op:cache:session:a']])
        .mockResolvedValueOnce(['0', ['op:cache:session:b']]);
      mockClient.del.mockResolvedValue(2);
