...
/**
 * Global catch-all — registered as APP_FILTER in AppModule. Coexists with
 * operation-service's existing route-scoped AuthErrorFilter
 * (@UseFilters(AuthErrorFilter) on AuthController, @Catch(AuthError)):
 * Nest resolves the most specific matching filter per exception, so
 * AuthError thrown inside AuthController still goes through
 * AuthErrorFilter; everything else (including non-AuthError exceptions
 * inside AuthController) falls through to this one.
 */
@Catch()
