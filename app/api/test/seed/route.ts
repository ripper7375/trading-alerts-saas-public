...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const hasValidSecret = testSecret && testSecret === expectedSecret;

  if (!ALLOWED_ENVIRONMENTS.includes(nodeEnv) && !hasValidSecret) {
    return NextResponse.json(
      {
        error:
          'Test seed endpoint only available in development/test environment or with valid x-test-secret header',
      },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    const proUser = createdUsers.find(
      (u) => u.email === 'pro-test@trading-alerts.test'
    );
    if (proUser) {
    ...
    const affiliateUser = createdUsers.find(
      (u) => u.email === 'affiliate-test@trading-alerts.test'
    );
    if (affiliateUser) {
...
export async function DELETE(_request: NextRequest): Promise<NextResponse> {
  ...
  if (!ALLOWED_ENVIRONMENTS.includes(nodeEnv)) {
    return NextResponse.json(
      {
        error:
          'Test seed endpoint only available in development/test environment',
      },
      { status: 403 }
    );
  }
