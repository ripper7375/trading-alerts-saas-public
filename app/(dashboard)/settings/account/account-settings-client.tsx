...
/**
 * Account Settings Page (client half)
 *
 * Features:
 * - Change password section (current, new, confirm)
 * - Password strength indicator
 * - Pending account-deletion banner + session-based cancel (Session 6-5)
 * - Two-factor authentication entry point (Session 6-5: the real, fully
 *   wired 2FA flows — setup/verify/disable/backup codes, all against live
 *   /api/user/2fa/* endpoints — live on /settings/security; this page no
 *   longer duplicates them with a local, non-functional toggle)
...
/** The 24h execution window only starts once confirmed; before that, the
 * 7-day link-expiry (expiresAt) is the only real deadline that exists. */
...
export function AccountSettingsClient({
  ...
  const handleDeleteAccount = async (): Promise<void> => {
    ...
    try {
      ...
      // Close dialog, notify user, and show the pending-deletion banner
      // immediately (no reload needed) using the request's own response.
  ...
  // Handle session-based cancellation of a pending/confirmed deletion request
