...
> = {
  operational: {
    label: 'Operational',
    labelKey: 'marketing.status.operational',
    icon: CheckCircle2,
  ...
  degraded: {
    label: 'Degraded',
    labelKey: 'marketing.status.degraded',
    icon: AlertTriangle,
  ...
  not_configured: {
    ...
    labelKey: 'marketing.status.not_configured',
    icon: MinusCircle,
        ...
        <div className="space-y-8">
          <div className="space-y-3">
            <Badge className="border-emerald-500/40 bg-emerald-500/15 px-3 py-1 font-mono text-xs text-emerald-700 dark:text-emerald-400">
              {dt(
                'marketing.status.live_telemetry_monitor',
            ...
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100 md:text-4xl">
                  {dt('marketing.status.title', 'DavinTrade System Status')}
                </h1>
            ...
            <CardContent className="flex items-center gap-4 p-6">
              ...
              <div>
                <h3 className={`text-base font-bold ${overall.textClassName}`}>
                  {status.overall === 'operational'
                    ? dt(
                        'marketing.status.all_systems_operational',
                        'All Systems Operational'
                      )
                    : status.overall === 'degraded'
                      ? dt(
                          'marketing.status.some_systems_degraded',
                          'Some Systems Are Degraded'
                        )
                      : dt(
                          'marketing.status.partially_unavailable',
                          'Status Partially Unavailable'
                        )}
                </h3>
                <p className="text-xs text-slate-600 dark:text-slate-400">
                  {dt(
                    'marketing.status.live_check_note',
                    'Live check against this environment -- not a rolling historical average.'
                  )}
                </p>
              </div>
            </CardContent>
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
              {dt('marketing.status.subsystem_health', 'Subsystem Health')}
            </h3>
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4 text-center dark:border-slate-800 dark:bg-[#090b14]/60">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              {dt(
                'marketing.status.admin_panel_note',
                'System operations, Flask API terminals, scheduled jobs, and queue diagnostics are managed in the Admin Panel.'
              )}
            </p>
            <a
            ...
            >
              {dt(
                'marketing.status.go_to_admin_panel',
