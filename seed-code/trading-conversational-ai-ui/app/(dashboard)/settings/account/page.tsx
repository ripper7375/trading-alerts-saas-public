  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-y-auto bg-[#06070a] select-none">
      <AppHeader
        ...
        subtitle="GDPR Data Export & 7-Day Account Grace Period Deletion"
      />
      <main className="mx-auto w-full max-w-3xl flex-1 space-y-6 p-4 md:p-6">
        ...
        <div className="space-y-3 rounded-2xl border border-slate-800/80 bg-[#090c14] p-6 shadow-xl">
          ...
          <p className="text-xs text-slate-400">
            Export all your active alert rules, historical AI prompts, and
            billing logs in JSON format.
          </p>
          <Button
            variant="outline"
        ...
        <div className="space-y-4 rounded-2xl border border-rose-500/40 bg-rose-500/5 p-6 shadow-xl">
          ...
          {!isRequested ? (
            <div className="space-y-3 text-xs text-slate-300">
              <p>
                Initiating account deletion schedules your account for permanent
                erasure after a{' '}
                <strong className="text-amber-400">7-day grace period</strong>.
              </p>
              <p className="text-slate-400">
                During this period, you can log in anytime to cancel the
                deletion request.
              </p>
            ...
            <div className="space-y-3 rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 text-xs">
              <div className="flex items-center gap-2 font-bold text-amber-300">
                <ShieldAlert className="h-4 w-4" /> Account Scheduled for
                Deletion
              </div>
              <p className="text-slate-300">
                Your account is scheduled to be erased in{' '}
                <strong>6 days, 23 hours</strong>. All active alert triggers
                will stop firing upon expiration.
              </p>
              <Button
                onClick={() => setIsRequested(false)}
                className="h-8 bg-amber-500 text-xs font-extrabold text-slate-950 hover:bg-amber-400"
              >
                <CheckCircle2 className="mr-1 h-3.5 w-3.5" /> Cancel Deletion
                Request & Keep Account
              </Button>
            </div>
          )}
        </div>
