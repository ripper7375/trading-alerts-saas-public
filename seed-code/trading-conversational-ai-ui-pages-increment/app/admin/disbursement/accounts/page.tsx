    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {accounts.map((acc) => (
          <Card
            key={acc.id}
            className="space-y-4 border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800/80 dark:bg-[#090b14]/90"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-amber-500/30 bg-amber-500/10 font-bold text-amber-600 dark:text-amber-400">
            ...
            <div>
              <div className="text-[10px] tracking-wider text-slate-600 uppercase dark:text-slate-400">
                {t('Available Floating Balance')}
              </div>
              <div className="mt-0.5 font-mono text-2xl font-extrabold text-slate-900 dark:text-slate-100">
                {formatCurrency(acc.balance)}
              </div>
            </div>

            <div className="border-t border-slate-200 pt-2 text-[11px] text-slate-500 dark:border-slate-800">
              {t(acc.type)}
            </div>
          </Card>
        ))}
