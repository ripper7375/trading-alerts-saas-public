...
export default function AdminDisbursementConfigPage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setSuccess(
        t(
          'Disbursement configuration saved successfully.',
          'บันทึกการตั้งค่าระบบการจ่ายเงินเรียบร้อยแล้ว'
        )
      );
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-y-auto bg-[#050609] text-slate-100 select-none">
      <AppHeader
        title={t(
          'Admin Disbursement: Treasury & Payout Rules',
          'การจ่ายเงิน: กฎและข้อกำหนดการโอน'
        )}
        subtitle={t(
          'Minimum Payout Thresholds, Automated Cron Execution Schedules & API Keys',
          'เกณฑ์ยอดโอนขั้นต่ำ ตารางการประมวลผลอัตโนมัติ และคีย์ API'
        )}
      />
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <Card className="space-y-5 border-slate-800/80 bg-[#090b14]/90 p-6">
            <h3 className="border-b border-slate-800 pb-3 text-sm font-bold text-slate-100">
              {t(
                'Automated Settlement Parameters',
                'พารามิเตอร์การตัดรอบอัตโนมัติ'
              )}
            </h3>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-300">
                  {t('Minimum Payout Threshold (USD)', 'ยอดโอนขั้นต่ำ (USD)')}
                </Label>
                <Input
                  type="number"
                  step="1"
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-300">
                  {t('Monthly Disbursement Day', 'วันที่ตัดรอบโอนเงินของเดือน')}
                </Label>
                <Input
                  type="number"
                  min="1"
              ...
              <div className="flex items-center justify-between rounded-xl border border-slate-800 bg-[#06080e] p-3.5">
                <div className="space-y-0.5">
                  <div className="text-xs font-bold text-slate-200">
                    {t(
                      'Automated Wise Cron Job Execution',
                      'ประมวลผลการโอนอัตโนมัติผ่าน Cron'
                    )}
                  </div>
                  <p className="text-[11px] text-slate-400">
                    {t(
                      'Automatically executes validated batches without manual admin trigger.',
                      'โอนเงินอัตโนมัติในวันที่กำหนดโดยไม่ต้องกดอนุมัติด้วยตนเอง'
                    )}
                  </p>
                </div>
                <Switch
                ...
                <div className="space-y-0.5">
                  <div className="text-xs font-bold text-slate-200">
                    {t(
                      ...
                      'ต้องมีแอดมิน 2 คนอนุมัติเมื่อยอดเกิน $5,000'
                    )}
                  </div>
                  <p className="text-[11px] text-slate-400">
                    {t(
                      'High security threshold for large treasury balance withdrawals.',
                      'ระบบรักษาความปลอดภัยสำหรับการถอนเงินจำนวนมาก'
                    )}
                  </p>
                </div>
                <Switch
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              {isSaving
                ...
                : t('Save Disbursement Config', 'บันทึกการตั้งค่า')}
            </Button>
          </div>
        </form>
      </main>
