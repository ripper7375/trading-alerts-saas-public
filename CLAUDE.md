...
- **Current:** Session 6-1 (Frontend Gap Matrix & Endpoint Mapping, F11, CONTRACT variant),
...
- **Previous:** Session 4B-22 (Phase 4 Exit Review, VERIFY-RETIRE/AUDIT variant), CONFIRMED and
...
- **Next session (Phase 6 track — THE literal next session overall as of 2026-08-10):**
  **Session 6-1** is now CONFIRMED, executed, and CLOSED — **with `DECISION-LOG.md` F11 still
  OPEN** (see Current above: the gap matrix is real and re-verified, but Davin's row-by-row
  triage was not obtained this session — that is F11's actual remaining content, not a
  documentation gap). `docs/migration-orders/phase-6-frontend-gap-matrix.md` is the produced
  artifact; F61/F62/F63 are committed OPEN. **Session 6-1b**
  (`6-1b-mock-data-hotfix.migration-order.md`, PORT variant, low dial) is now PRE-DRAFTed and is
  the literal next session — scoped to A1-1 (`/settings/billing`), A1-2
  (`/admin/fraud-alerts/[id]`), A1-3's mock-activity half (`/admin`), and A1-4's alert-count half
  (`/settings`) only; no redesign, no new components, no layout changes (those are 6-2/6-5/6-6's
  job on pages that are truthful by then). Two named Phase 4 exceptions run as their own
  independent tracks and do NOT block 6-1b: `DECISION-LOG.md` F49 (dLocal `payment_method_flow`,
  ...
  **Phase 6 is now 12 sessions, in this order:** 6-1 (gap matrix, audit only — done) → **6-1b**
  (mock-data hotfix, PORT/low dial: the 3 pages rendering fabricated data in production, next) →
  6-2 (IA + design system + shared shells; resolves F62; adds `app/not-found.tsx`; kills the dead
  `/analytics` + `/indicators` nav links) → 6-3 (alerts/charts) → 6-4 (notifications; builds the
