/**
 * Affiliate Report Builder Service (Session 4A-6, File 2/3)
 *
 * Ported from lib/affiliate/report-builder.ts. Converted to a real
 * `@Injectable()` with `PrismaService` constructor-injected (was free
 * functions taking the `prisma` singleton import), same pattern as
 * Session 4A-4's ConversionProcessorService. Query/aggregation logic
 * byte-identical.
 *
 * Generates reports for the affiliate dashboard (code inventory,
 * commission tracking) and the admin global code-flows report.
 */
...
export class ReportBuilderService {
  ...
  async buildCodeInventoryReport(
    ...
    const additionsByReason = await this.prisma.affiliateCode.groupBy({
   ...
   */
  async buildGlobalCodeInventoryReport(period: {
    start: Date;
    end: Date;
