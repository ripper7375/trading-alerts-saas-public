 ...
 *
 * Ported byte-for-byte from lib/dlocal/constants.ts (Session 4A-4, File 2/4).
 ...
 * - Backend: AffiliateConfigService.getBasePriceUsd()
 * - Frontend: useAffiliateConfig() hook (monolith only)
 *
 ...
 *
 * IMPORTANT: These are fallback default values only.
 *
 * For dynamic prices from SystemConfig, use:
 * - Backend: AffiliateConfigService.getBasePriceUsd() / getThreeDayPriceUsd()
 * - Frontend: useAffiliateConfig() hook (monolith only)
 */
export const PRICING = {
  /** Default 3-day trial price - use AffiliateConfigService.getThreeDayPriceUsd() for dynamic value from SystemConfig */
  THREE_DAY_USD: 1.99,
  /** Default monthly price - use AffiliateConfigService.getBasePriceUsd() for dynamic value from SystemConfig */
  MONTHLY_USD: 29.0,
} as const;
