 ...
 *
 * Ported from lib/disbursement/services/retry-handler.ts (Session 4A-2,
 * File 3/6). Manages retry logic for failed disbursement transactions.
 * Implements exponential backoff and max retry limits. Converted to
 * `@Injectable()` with `PrismaService` constructor-injected (was a plain
 * class taking a raw `PrismaClient` positional argument). The optional
 * `config` override (source: a plain second constructor argument) is kept
 * as `@Optional()` so Nest's DI container doesn't try to resolve it as a
 * provider token — nothing in the ported code overrides it outside tests.
 */
...
import { DEFAULT_RETRY_CONFIG } from './disbursement.constants';
...
export class RetryHandlerService {
  ...
  constructor(
    ...
    @Optional() config?: Partial<RetryConfig>
  ) {
