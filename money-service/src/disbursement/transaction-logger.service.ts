 ...
 *
 * Ported from lib/disbursement/services/transaction-logger.ts (Session
 * 4A-2, File 3/6). Provides audit trail logging for disbursement
 * operations. Logs batch creation, execution, payment success/failure
 * events. Converted to a real `@Injectable()` with `PrismaService`
 * constructor-injected per the order's port steps (was a plain class taking
 * a raw `PrismaClient` positional argument).
 */

import { Injectable } from '@nestjs/common';
import { AuditLogStatus, Prisma } from '@prisma/client';
