 ...
 *
 * Ported from lib/disbursement/services/batch-manager.ts (Session 4A-2,
 * File 3/6). Creates and manages payment batches for affiliate commission
 * payouts. Handles batch lifecycle: creation, status updates, retrieval,
 * and deletion. Converted to `@Injectable()`; `TransactionLoggerService`
 * and `TransactionService` are now constructor-injected via Nest's DI
 * container instead of being `new`-ed internally (source instantiated both
 * itself from the raw `PrismaClient` it received) — required for these to
 * become real Nest providers per the order's "convert ... to @Injectable()s"
 * step; no behavior change, same two collaborators, same call sites.
 */
...
import { generateBatchNumber, MAX_BATCH_SIZE } from './disbursement.constants';
import type { CommissionAggregate } from './disbursement.types';
...
export class BatchManagerService {
  ...
  async getBatchStatistics(): Promise<{
    ...
    return {
      ...
      byStatus,
      totalAmount: Number(
        (batches['_sum'] as unknown as { totalAmount?: number } | undefined)
          ?.totalAmount ?? 0
      ),
      totalPayments:
        (batches['_sum'] as unknown as { paymentCount?: number } | undefined)
          ?.paymentCount ?? 0,
    };
  }
}
