...
**Variant:** PORT · **Status:** CONFIRMED and executed, CLOSED (2026-08-02)  
...
- [x] `MIGRATE_TIER=true` set on Vercel production + live smoke test verified. Davin gave separate, explicit live approval (distinct from the session's general go-ahead) before the flag was touched. `MIGRATE_TIER` added to Vercel production (value-blind per L17), `vercel --prod --archive=tgz --yes` (L36) redeployed clean and aliased to the real production URL. Unauthenticated `/api/tier/{symbols,combinations,check/XAUUSD}` all confirmed still 401 post-redeploy (auth check runs before the flag check — proves the new code is genuinely live). Davin ran the live smoke test himself from his own browser DevTools console on the production tab (session cookie applied automatically, no token ever extracted or handled by the Executor, same method as every prior 4B cutover) — all 3 calls returned real `tier: 'PRO'` data (`symbols`, `check/XAUUSD` → `allowed: true`, `combinations`). Independently cross-checked against `operation-service`'s own Railway HTTP logs, not trusted from the response body alone (L18): `GET /tier/symbols 200`, `GET /tier/combinations 200`, `GET /tier/check/XAUUSD 200`, all timing-matched to the smoke test. **Verification is COMPLETE, not partial — unlike Drawings/Notifications, all 3 of this domain's endpoints were exercised live in one shot** (this domain has no id-scoped sub-resources to leave untested).
...
5. **L28-class gap found while wiring Step 4 (recurrence, not a new lesson — already well-documented, not re-promoted):** `app/api/tier/check/[symbol]/route.ts` had zero test coverage anywhere in the repo before this session — `__tests__/api/tier.test.ts` only ever tested `symbols`/`combinations`. Built 5 new tests for it (401, tier-independent allow, lowercase normalization, denial with reason/accessibleSymbols, forwarding) plus 1 forwarding test each for the other two routes (8 new tests total, on top of the 13 pre-existing ones).
6. **A Jest module-hoisting trap found and fixed while updating the existing test file:** adding a class-based `OperationServiceError` mock (`MockOperationServiceError extends Error`) alongside the file's pre-existing static top-level route imports threw `ReferenceError: Cannot access 'MockOperationServiceError' before initialization` — Babel's CommonJS interop hoists ES `import` statements above same-file `class` declarations regardless of their textual order, so the mock factory ran before the class existed. Fixed by switching every route call in the file to a per-test dynamic `await import(...)`, matching `__tests__/api/notifications.test.ts`'s own established (and unaffected) convention. Not a new lesson — this is exactly why that convention exists, just not previously needed in this particular file.
7. **Unrelated, flagged not acted on:** Davin's live smoke test surfaced a browser console 404 on `icons/icon-144x144.png` (PWA manifest icon) — confirmed pre-existing and unrelated (no such file exists in `public/icons/`; last commit touching manifest/icon files predates this session by multiple sessions). Not fixed here — out of this session's scope, flagged for a future session.
