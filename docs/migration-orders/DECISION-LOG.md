...
| F55  | Realtime WS connection authenticates server-side then repeatedly disconnects/reconnects in a loop (many cycles, several ~25-30s apart) — `isConnected` never observed true client-side despite genuine server-side auth success | OPEN — found Session 4B-18c (2026-08-03), needs its own investigation session (not a tiny PORT fix)                                                                                                    |
...
- Status: **OPEN** — needs its own dedicated investigation session (genuinely new territory, not
  a config/CORS/CSP fix — likely `RealtimeGateway`'s connection lifecycle and/or a Railway
  proxy/idle-timeout interaction with Socket.IO's ping/pong keep-alive)
- Session: found 4B-18c, 2026-08-03 · Date: 2026-08-03
- Found while: re-testing F54's own fix live — the WS transport now genuinely connects
  (`101 Switching Protocols`, confirmed in DevTools' native WS-filtered Network view), and
  `operation-service`'s live logs show the real user (`cmsa5a8pa0001d8v2ikyfm5h5`) completing
  `RealtimeGateway.handleConnection`'s real JWE verification successfully — but the chart page's
  own connection indicator stayed red/"Disconnected" throughout, and no `alert_fired`/
  `notification` pairing was ever exercised (nothing to arm an alert against with no stable
  connection).
- **Evidence, not speculation:** `operation-service`'s Railway logs for Davin's test window
  (`~02:55–03:44 UTC`, 2026-08-03) show the SAME user authenticating via 15+ DISTINCT socket IDs
  in that ~50-minute span, each disconnecting shortly after (ranging from a few seconds up to a
  few minutes), then a new socket authenticating again — a genuine repeated connect→authenticate→
  disconnect→reconnect cycle, not a single stable connection. Several of the connect→disconnect
  gaps cluster suspiciously close to **~25–30 seconds** (e.g. `02:57:44→02:58:10` ≈26s,
  `02:58:12→02:58:43` ≈31s, `02:58:45→02:59:13` ≈28s) — consistent with Socket.IO's default
  `pingInterval` (25s) / `pingTimeout` (20s) keep-alive cycle being missed, which would point at a
  Railway-side proxy/load-balancer idle-connection timeout not recognizing engine.io ping/pong
  frames as activity, though this is NOT confirmed, only a well-evidenced hypothesis — other
  gaps in the same log are much shorter (a few seconds) or longer (several minutes), so a single
  clean explanation isn't yet proven.
- **Ruled out before treating this as a real, new finding (read-only diagnosis only, zero code
  changed in `operation-service`):** re-read `realtime.gateway.ts`'s `handleConnection` in full —
  it DOES correctly call `client.emit('authenticated', { success: true, userId: claims.id })`
  immediately after `client.join()` on the success path (this is not a missing-emit bug); no
  code path in `handleConnection` or `handleDisconnect` explicitly disconnects a successfully
  authenticated client. `handleDisconnect(client: Socket)`'s own signature does not capture
  Socket.IO's own disconnect `reason` string (e.g. `"ping timeout"`, `"transport close"`,
  `"client namespace disconnect"`) even though the underlying `disconnect` event carries one —
  this is itself a diagnostic gap worth closing in the fix session, not just the symptom.
- **Not investigated further or fixed this session** — per `4b-18c-realtime-csp-connect-src-fix
.migration-order.md`'s own explicit instruction: a third distinct root cause after two real,
  independently-fixed bugs (F53, F54) is a strong signal this needs a broader, non-PORT-shaped
  investigation session, not another tiny targeted guess. `realtime.gateway.ts` and
  `hooks/use-realtime-socket.ts` were both read-only this session (matching this order's own
  explicit do-not-touch list) — zero bytes changed in either.
- Approved by: n/a (technical finding, not yet a decision — the fix/investigation session itself
  needs its own scoping and Davin's normal APPROVED sign-off before executing).
