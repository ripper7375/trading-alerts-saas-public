...
| F58  | Every operation-service `/user/*` route (profile, 2FA, sessions, preferences — cut over since Session 4B-11) returns "User not found" for a user created via `token-register` (this session's own bridge registration path), despite the row provably existing | OPEN — found live during Session 4B-21's own Step 2 local smoke test; blocks the production flag flip until root-caused                                                                                                                                       |
...
- Impact if not resolved before the flag flip: every NEW user who registers through the bridge
  ...
  while existing users (created pre-migration) are unaffected. Login/logout/registration/password-
  reset/email-verification themselves are unaffected either way (proven working, `AuthController`
  only).
- Owner: Davin — needs Railway dashboard/log access this environment doesn't have (check
  operation-service's actual last-deployed commit/build timestamp, redeploy if stale, or investigate
  further if a fresh deploy doesn't resolve it). Blocks Checklist Steps 3-6 (Davin approves the flip,
  flip the flag, production smoke test, retire `CredentialsProvider`) until resolved — a red result
  at this session's own mandated Step 2 gate, per the order's own explicit rule.
- Approved by: n/a — OPEN, not yet resolved
