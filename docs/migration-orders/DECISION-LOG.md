...
| F72  | **Contabo chat stack scope** — domain/TLS, NLLB-200 v1 scope, `client_message` has **no auth on the socket at all**                                                                                                                                                        | RESOLVED — Session 14-0 (2026-08-30): `chat-api.davintrade.app` (corrected from a draft assumption of `.com` — see F72's full entry), NLLB-200 deferred, bot worker calls the LLM directly metered via 11-3's `trackAiTokenUsage()`, dual-mode BFF token-bridge socket auth mirroring F8; Davin's live sign-off quoted, full detail in `history/decisions-archive.md`                                                                                                  |
