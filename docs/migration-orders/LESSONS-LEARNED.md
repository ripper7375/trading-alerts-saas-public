...
never surfaced). Session 8-2: no new lesson added (stayed at the cap) -- recurrence notes appended
to L19 (three first-deploy gotchas: `railway up`'s upload source is the shell's current directory,
not the linked service's; a new service needs `engines`/`postinstall` parity with its siblings
...
Davin's sign-off existed only uncommitted; resolved live, as always). Session 14-3: no new lesson
added (stayed at the cap) -- recurrence notes appended to L3 (same status-integrity pattern, 7th
consecutive session in a row) and L24 (7th occurrence of the `prisma.shutdown.spec.ts` flake). A
genuinely new candidate lesson surfaced (`git push` to `main` is not reliable proof a Vercel
...
- Recurrence count: 33+ times through Session 14-3 (2026-08-30; confirmed recurring at 8-1, 8-2,
  11-1, 11-2, 11-3, 14-0, 14-1, 14-2, and 14-3 since the last count update) — the single most-recurring finding class
...
- Recurrence (Session 8-2, 2026-08-24), three distinct gotchas deploying a service for the very
  first time: (1) `railway up` uploads whatever the CURRENT WORKING DIRECTORY is at the moment the
  command runs, not the linked service's own directory — a stale shell `cd` from an earlier step
  silently uploaded the whole monorepo instead of `railway-gateway/`; always chain `cd` into the
  SAME invocation and confirm with a literal `pwd` first. (2) A brand-new service needs the exact
  same `engines`/`postinstall` parity as its already-deployed siblings BEFORE its first deploy —
  Nixpacks defaulted to Node 18 (no `engines` field), and a missing `postinstall: "prisma generate"`
  let `npm ci` silently skip client generation entirely; both failures only surface on Railway's
  build, never locally where `node_modules/.prisma` was already generated by hand. (3) A
  plausible-looking `${{Service.VAR}}` reference is not proof it resolves to the same topology a
  sibling service actually uses (internal vs. public, separate host/port/password vs. one URL) —
...
- Recurrence (ad-hoc, 2026-08-30, post-14-2): the same "floor not ceiling" gap applied to a single flagged finding, not an order's checklist — Davin's "the help page shows the wrong email" turned out, on a live grep, to be 6 occurrences across 7 `app/` files (`error.tsx`, `global-error.tsx`, 4 marketing pages, settings/help), not the one page named. Naming one instance of a bug is not proof it's the only one — grep the pattern before declaring a spot-fix complete.
...
- Recurrence count (7+ occurrences: 11-2, 11-3, 14-0, 14-1, 14-2, 14-3 — per-session notes retired
  at the 3-line cap): `money-service`'s `prisma.shutdown.spec.ts` SIGTERM-timeout flake under Jest's
  own parallel-worker CPU contention in the full `npm test` run. Same resolution every time, no
...
### L29 — `@nestjs/swagger` only introspects schemas from class-validator DTOs, not Zod — this hits BOTH request/response bodies and query parameters, and the query gap is silent, not generic

- Rule: on a Zod-validated NestJS service, `@nestjs/swagger` emits correct route paths/methods but generic `type: object` for bodies (Zod's `z.infer<>` erases at compile time, no decorator metadata) — accept that or plan a separate `zod-to-openapi` step. Separately and more dangerously, it can emit `parameters.query?: never` on EVERY operation of a service (found on money-service, worse than the body gap: a caller passing real query params gets a compile error, not a loose-but-working type). Before writing a call site against ANY generated OpenAPI client for a Zod-validated service, grep the real `schema.ts` for that operation's `parameters.query` (and `requestBody`) directly — don't assume from another service's or session's disclosed gap that the shape is the same, and don't trust the generated client's own header comments (they described the money-service gap as body-only; it wasn't). Work around a `never` query type with one narrowly-scoped cast per call site (e.g. append the query string to a literal path) rather than casting individually everywhere.
- Source: Session 7-1 (promoted 2026-08-12); query-parameter variant found Session 7-2 (2026-08-20), order Deviation 5 · Status: ACTIVE
...
- Recurrence (Session 8-2, 2026-08-24): a schema-resolution variant, not a grant variant.
  `railway-gateway`'s first-ever production write to `market_data_v6` (created and verified once,
  Session 4B-12, then never touched again for 3 weeks) failed with `The table public.market_data_v6
does not exist`, even connecting via the byte-identical `DATABASE_URL` `operation-service` itself
  uses. Root cause: Prisma's generated client always fully qualifies with `public` (no `@@schema`
  declared), but the connecting role's own default `search_path` resolves an unqualified
  `CREATE TABLE` to a different schema first — confirmed by running the identical unqualified DDL
  again, which returned `relation already exists`. The table was never missing; it was never in
  `public` to begin with, matching `DECISION-LOG.md` F70's own already-registered, still-open
  question ("which DB role reads `market_data_v6` — `core_app` has no market-data grant"). Rule
  extension: a "table does not exist" error from a Prisma client is not proof the table is
  missing — it can mean the connecting role's `search_path` puts a different schema first. Verify
  with an UNQUALIFIED raw DDL/query on the exact same connection before concluding data was lost.
...
### L45 — BullMQ's `removeOnComplete` can purge a job's data before an async `QueueEvents` listener re-reads it; separately, `docker compose up -d <service>` without a forced recreate can silently keep old env vars

- Symptom (Session 14-1): a live WSS smoke test against the newly-deployed chat stack connected and enqueued a job (`bot_typing:true` arrived) but no reply ever came back — total silence, no error either. Root cause: the Socket.IO server's job-completion handler called `Job.fromId(queue, jobId)` to recover the target `socketId`, but `removeOnComplete: true` had already deleted that job's stored data by the time the async `QueueEvents` `'completed'` listener ran, so the lookup silently returned `undefined` and the handler no-opped. Fixing this (carry `socketId` inside the job's own return value instead of re-fetching it) unmasked a second, previously-invisible bug in the same path — L11's pattern exactly, the error message changed shape (silence → a real `chat_error`) rather than disappearing, which was progress, not a new failure.
- Second, separate gotcha in the same episode: an `.env` edit on the VPS (`LLM_MODEL`) had no effect after `docker compose up -d bot_worker` reported "Restarted -> Started" — a restart of an existing container does not reload interpolated `.env` values, only an actual recreate does (`Recreate -> Recreated -> Started` in the compose output is the tell). Two apply attempts looked identical and failed identically until one actually triggered a recreate.
- Rule: for any BullMQ consumer whose result needs to reach something keyed by the ORIGINAL job data (a socket ID, a request ID), put that key inside the return value itself — never re-fetch `job.data` from an async completion listener when `removeOnComplete`/`removeOnFail` might have already purged it. Separately, when an `.env` change on a running Compose stack doesn't seem to take effect, check the compose output for `Recreated` vs merely `Restarted` before assuming the fix itself is wrong — `docker compose up -d --force-recreate <service>` guarantees the reload; a plain `up -d` does not.
- Source: Session 14-1 (2026-08-30) · Status: ACTIVE
