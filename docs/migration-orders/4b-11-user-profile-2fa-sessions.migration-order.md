...
- [ ] Feature flags set on Vercel production + live smoke test verified — **NOT done this session, per the order's own Step 5 checkpoint.** Awaiting Davin's separate, explicit live approval (`EXECUTOR-PROTOCOL.md` §7) before touching any `MIGRATE_USER_*` Vercel production env var.
