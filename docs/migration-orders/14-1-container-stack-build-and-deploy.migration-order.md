...
**Session:** 14-1 · **Phase:** 14 (Web Chat / Contabo Support Stack, second of 4 sessions) · **Variant:** INFRA · **Status:** CLOSED SUCCESSFUL  
...
- [x] All 3 Docker containers (`socket_chat_server`, `redis_broker`, `ai_bot_worker`) are running and healthy on Contabo. Confirmed both directly (`docker compose ps`, pasted by Davin) and functionally — a full end-to-end smoke test cannot succeed unless all three are genuinely up and correctly wired.
- [x] Nginx terminates SSL for `chat-api.davintrade.app` with valid Let's Encrypt certificates and auto-renewal enabled. `curl -I https://chat-api.davintrade.app` returned `HTTP/1.1 200 OK` with no TLS error.
- [x] Denial tests confirm ports 6379 and 3001 are inaccessible from outside the VPS. Verified directly by the Executor (no SSH needed): both ports refuse/timeout externally; 443 reachable as a sanity check.
- [x] Synthetic WebSocket smoke test connects over `wss://chat-api.davintrade.app`, emits `client_message`, and receives `support_message` successfully. Passed on the 4th attempt — the first 3 surfaced real bugs (see Deviations 12–13), not flakiness. Final run: genuine Gemini-generated reply, correctly scoped to the frozen system prompt, in 5097ms.
- [x] Complete deployment codebase and IaC configuration committed to `infra/contabo-chat-stack/` in the repository.
- [x] `CHAT_JWT_SECRET` documented value-blind for use in Session 14-2 (exists in `/opt/saas-chat/.env`, generated via `openssl rand -hex 32` per Decision 3; Davin holds the value, never seen or echoed by the Executor).
...
12. **Real bug found live at Step 6, not scope creep: `removeOnComplete` race silently dropped every bot reply.** First smoke test attempt: WSS connected, `client_message` accepted, `bot_typing:true` arrived — then total silence, no `support_message`, no `chat_error`, timeout. Root cause: the server's `QueueEvents` `'completed'` handler re-fetched the target `socketId` via `Job.fromId(chatQueue, jobId)`, but `removeOnComplete: true` had already purged that job's stored data by the time the async listener ran, so the lookup silently returned `undefined` and the handler no-opped — every single reply, not just this test run. Fixed by carrying `socketId` inside the job's own `ChatJobResult` return value instead of re-fetching it after the fact (commit `3e6198fe`); `removeOnFail` widened from `true` to a 1h retention window since the `'failed'` handler still needs `Job.fromId` (no `returnvalue` exists for a thrown job). Harvested as `LESSONS-LEARNED.md` **L45** (new entry — net-zero against the 40-cap via merging L29/L32, which were the same underlying `@nestjs/swagger`+Zod gap and belonged together anyway).
13. **Second, separate issue unmasked by fixing the first (L11's pattern): the LLM call itself was failing.** After the race-condition fix, the smoke test's error message changed shape rather than disappearing — from total silence to a real `chat_error: SERVER_ERROR` plus a `support_message` carrying the fallback text, in ~800ms (consistent with a fast-failing API call, not a timeout). Davin changed `LLM_MODEL` to `gemini-3.5-flash` in `.env` and ran `docker compose up -d bot_worker`, which reported **"Restarted -> Started"** — identical failure, identical ~800ms latency, no actual improvement. Per §0/§2's "you decide from live code, don't guess" and CLAUDE.md non-negotiable #6 ("verification is never skipped"), the Executor declined to close the session on this and asked twice for the real `bot_worker` logs rather than accept an unverified claim of a fix. Davin's next attempt reported **"Recreate -> Recreated -> Started"** (an actual container recreate, not just a restart) — the smoke test then passed with a genuine Gemini reply. Root cause of the intermediate failures was never directly confirmed via logs (Davin did not paste the requested `docker compose logs bot_worker` output at any point) — the working theory, consistent with all observed evidence, is that a plain `docker compose up -d <service>` without a triggered recreate does not reload changed `.env` values into an already-running container. Harvested into the same `LESSONS-LEARNED.md` **L45** entry.
14. **Deployed model differs from both candidates named in Decision 2.** Decision 2 named `gemini-1.5-flash`/`gemini-2.0-flash` or `claude-3-5-haiku-20241022`/`claude-3-haiku-20240307` as the options; the model actually live and verified working is `gemini-3.5-flash` — a model ID this session could not independently verify against Google's current model list (`WebFetch` errored all session, see Deviation 9) but which Davin confirmed and which a real end-to-end smoke test proved functional. `.env.example`'s documented default updated to match what's actually deployed, with a note that the originally-cited IDs were never re-tried (unconfirmed either way), not disproven.
15. **`<3000ms` latency target (Ordered Step 6) missed, not silently passed.** The passing smoke test's real latency was 5097ms. The round trip works end-to-end and the reply content is correct and on-topic — this is a UX/performance concern, not a functional failure — but reporting it as meeting the order's own explicit numeric target would misstate what was actually verified. Flagged for Session 14-2 (loading-state UX should assume multi-second response times) and for a future session to revisit if a faster model or provider is wanted.
16. **§3 close-out re-verification, run fresh after all session changes landed (not reused from CONFIRM-time numbers):** monolith `test:ci` **151/151·2239/2239**, `operation-service` **43/43·401/401**, `railway-gateway` **3/3·23/23** — all unchanged, as expected (this session's real code changes are entirely confined to `infra/contabo-chat-stack/`, excluded from all four suites' scope). `money-service`'s full run showed **1 failed, 61 passed** (`src/prisma/prisma.shutdown.spec.ts`, "Exceeded timeout of 5000 ms") — re-ran that spec alone with `--runInBand`, passed clean in 7.7s. This is the **5th** occurrence of the same flake (11-2, 11-3, 14-0, now 14-1) — `LESSONS-LEARNED.md` L24's per-session notes retired in favor of a single recurrence-count line per that entry's own 3-line cap, per hygiene rule (5+ recurrences → count line, not another paragraph).
