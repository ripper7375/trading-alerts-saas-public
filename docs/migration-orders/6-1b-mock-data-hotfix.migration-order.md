...
3. **`components/billing/subscription-card.tsx` NOT mounted, despite the order's own text naming it** — reading the component before wiring it found a real, pre-existing correctness bug: its `handleUndoCancel` only clears local optimistic state and never calls a reactivation API, while `handleOptimisticCancel` already `await`s the real `onCancel()` and does not roll it back on "Undo." Wiring `POST /api/subscription/cancel` directly into this component's own cancel button would mean a user who clicks Cancel then Undo within its 5s window sees the UI silently revert to "still PRO" while their subscription was, in fact, already cancelled server-side — a real, money-adjacent, misleading-state bug. Fixing `subscription-card.tsx` itself is out of this session's scope (not one of the 4 target files; a drive-by fix to a shared component is exactly the scope creep `EXECUTOR-PROTOCOL.md` §2 prohibits). Kept the existing hand-rolled Card + `AlertDialog` cancel-confirmation flow instead (rewired to live `/api/subscription` data and a real `POST /api/subscription/cancel` call), which correctly satisfies the file's own Invariant ("must not regress the existing dialog's confirmation copy"). `components/billing/invoice-list.tsx` mounted as specified — no correctness issue found in it. Flagged for a future session to either fix `subscription-card.tsx`'s undo bug or retire it if it stays unused.
4. **File 3's `GET /api/admin/fraud-alerts` call uses `pageSize=10`, not the order's own suggested `pageSize=5`** — the route's real `querySchema` (`z.coerce.number().min(10).max(100).default(20)`) rejects anything below 10; found by reading the route before calling it, not by a live 400. Fetches the minimum allowed page and trims to the 5 most recent client-side (`.slice(0, 5)`) — same visible result as the order intended, no behavior change to the endpoint itself, no scope change.
