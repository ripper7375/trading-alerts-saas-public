...
7. **Found, not fixed (at the time):** `app/(marketing)/help/page.tsx` (and possibly `app/settings/help/`) still shows `support@davintrade.com` in its rendered copy, not `.app` — a pre-existing leftover from Session 9-0/14-0's domain correction that never propagated to this specific page's content. Out of scope for a PORT session touching only the chat widget.
   **Addendum (same day, ad-hoc follow-up):** Davin asked for this fixed. A live grep found the true scope was far wider than either page named above — 6 `.com` email occurrences (`support@`/`legal@`/`privacy@`/`careers@`) across 7 `app/` files, plus a stale test assertion. All fixed; see `frontend-swap-route-map.md` gap-inventory row 6d (now marked RESOLVED for main-repo scope) and `CLAUDE.md`'s matching ad-hoc entry for full detail.
