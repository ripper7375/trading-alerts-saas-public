...
- **Addendum (session close, Davin asked to see the actual captured evidence):** the `logger.info` spy originally used `.mockImplementation(() => {})`, which suppressed the real console output — sufficient for the assertion but not for visible proof. Changed to a plain `jest.spyOn(logger, 'info')` (calls through), matching this repo's other spec files' convention of not swallowing real log output. Re-ran; the actual printed line, captured verbatim:
  ```
  [2026-07-26T13:16:42.771Z] [INFO] PrismaService disconnected cleanly on module shutdown
  ```
  Full suite re-verified green after the change (29/29, 288/288 — see Step 4's own count, unaffected by this visibility-only tweak).
...
- **Addendum (session close, Davin asked for the literal dLocal-route before/after replay):** attempted a direct empirical before/after on the real route itself — temporarily commented out `@Throttle(...)` on the live controller, intending to re-run the burst test against `/webhooks/dlocal` directly (expecting 429s to reappear), then restore it. The harness's own auto-mode safety classifier blocked the test run while the file sat in that weakened state ("Blocked by classifier" — a rate-limit control removed from a live money-route file, even locally/uncommitted). Restored `@Throttle(...)` immediately; `git diff` confirmed the file matched its committed state exactly, zero residual change. Did not attempt to work around the block. The control-route test already in this file (same global default, same 150-request burst, no `@Throttle()` override) is the safe substitute — it isolates the identical variable (presence/absence of the route-level override) without ever disabling a live route's real rate limit, and is arguably cleaner evidence for that reason (same run, same moment, same code, only the decorator differs between the two routes). If Davin specifically wants the literal live-file removal-and-restore repeated, it needs an explicit permission change on this session's Bash tool.
