...
5. **A real, empirically-verified Express 5 / path-to-regexp v8 breaking change found before it could bite Step 4.** The bare `'*'` wildcard (the obvious choice for "match every route" in `MiddlewareConsumer.forRoutes()`) is REMOVED in path-to-regexp v8 (the version this repo's installed `express@5.2.1` actually uses) — confirmed by calling the real installed `pathToRegexp()` directly in a throwaway script, which threw `"Missing parameter name at index 1: *"`. The documented replacement, `'/{*splat}'`, was verified the same way to match every path including bare `/`. Neither this repo nor either service had any prior middleware registration to copy this convention from (grepped first, found nothing) — this is the first `forRoutes()` call in either service's history. Harvested as `LESSONS-LEARNED.md` **L42** (Davin's explicit direction to exceed the stated cap for this one, 2026-08-01).
...
**LESSONS-LEARNED.md is past its stated ~40-lesson cap** (already flagged at 4B-3's close, consolidation pass still not run). Deviation #5's path-to-regexp finding was harvested as **L42** at Davin's explicit direction (2026-08-01) — the one exception to "don't add without direction" this session. Deviation #11's taskkill incident stays an unpromoted candidate, described in `LESSONS-LEARNED.md`'s own header note for the next consolidation pass rather than given a fresh `Lxx` number.
