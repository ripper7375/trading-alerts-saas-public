...
| `test:e2e` and its variants (`test:e2e:ui/headed/debug/report/group-a/group-b/ci`) — Playwright | **Broken, not just unattempted.** The script runs `playwright test --config=e2e/playwright.config.ts`, but that file does not exist on disk — only `e2e/archive/playwright.config.ts` does (looks like a stale reference left over from an archive/reorg). Attempted once to observe the actual failure mode: the command does **not** fail fast on the missing config — it was still running after 20s and had to be terminated to avoid a hung process. Root cause not diagnosed further (out of scope for a read-only session); worth a fix in a later session. |
