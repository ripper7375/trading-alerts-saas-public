...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| `NEXTAUTH_SECRET`                               | NextAuth JWT/session signing — `lib/auth/auth-options.ts`; also read by `operation-service`'s `JwtAuthGuard` (Session 3-1, F7) to decrypt the same NextAuth-issued JWE — **not yet granted to the Railway service as of Session 3-1's deploy**, Davin sets directly (Railway variable-set, never via an agent/CLI arg/printed output) | `.env.example`, `.env`, `.env.local`, `docker-compose.yml` (web) |
...
| Name                                           | Consumed by                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Found in                                                                                                    |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `DATABASE_URL`                                 | Primary Postgres via Prisma — used app-wide; `operation-service`'s `PrismaService` also needs it (Session 3-1) — **not yet set correctly as of the Session 3-1 deploy**: `pgbouncer` (the Railway service in front of Postgres) has no native `DATABASE_URL` to reference (it's a bespoke deploy, Session 1-3b, not a Railway-managed template) — the Postgres service's own `CORE_APP_DB_PASSWORD` (paired with `MONEY_SVC_DB_PASSWORD` for money-service) is the scoped credential meant for this, combined with `pgbouncer.railway.internal` into a real pooled connection string. Davin sets it directly. | `.env.example`, `.env`, `.env.local`, `docker-compose.yml` (web, mt5-service, alert-worker)                 |
