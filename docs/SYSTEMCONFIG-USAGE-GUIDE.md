...
| Admin MRR                                                                               | monthly PRO × `affiliate_base_price` + annual PRO × `affiliate_annual_price` ÷ 12 | `lib/admin/analytics/mrr.ts` (used by `/api/admin/analytics` and `revenue.ts`), money-service `admin-mrr.ts` |
