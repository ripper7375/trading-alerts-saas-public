/**
 * Part 20 - Phase 04e: Symbols API Route
 *
 * GET /api/symbols
 * Returns list of symbols accessible by the authenticated user's tier.
 * Uses the Phase 04c tier validation utilities.
 *
 * @see docs/sqlite-and-mt5service/part-20-sqlite-sync-postgresql-openapi.yaml
 */

import { NextResponse } from 'next/server';
import { getServerSession } from 'next-auth';

import { authOptions } from '@/lib/auth/auth-options';
import { getAccessibleSymbols, type Tier } from '@/lib/tier/validation';

interface SymbolsResponse {
  success: boolean;
  tier: Tier;
  symbols: readonly string[];
  total_count: number;
}

interface ErrorResponse {
  success: boolean;
  error: string;
  message: string;
}

/**
 * GET /api/symbols
 *
 * Returns list of symbols accessible by the requesting user's tier.
 *
 * @returns 200: List of accessible symbols
 * @returns 401: Unauthorized (not logged in)
 * @returns 500: Internal server error
 *
 * @example Response (FREE tier):
 * {
 *   "success": true,
 *   "tier": "FREE",
 *   "symbols": ["BTCUSD", "EURUSD", "USDJPY", "US30", "XAUUSD"],
 *   "total_count": 5
 * }
 */
export async function GET(): Promise<NextResponse> {
  try {
    // Get user session
    const session = await getServerSession(authOptions);

    if (!session?.user?.id) {
      const errorResponse: ErrorResponse = {
        success: false,
        error: 'Unauthorized',
        message: 'You must be logged in to view available symbols',
      };
      return NextResponse.json(errorResponse, { status: 401 });
    }

    // Get user tier (default to FREE if not set)
    const userTier = (session.user.tier as Tier) || 'FREE';

    // Get accessible symbols for user's tier
    const symbols = getAccessibleSymbols(userTier);

    const response: SymbolsResponse = {
      success: true,
      tier: userTier,
      symbols,
      total_count: symbols.length,
    };

    return NextResponse.json(response, { status: 200 });
  } catch (error) {
    console.error('GET /api/symbols error:', {
      error: error instanceof Error ? error.message : 'Unknown error',
      stack: error instanceof Error ? error.stack : undefined,
      timestamp: new Date().toISOString(),
    });

    const errorResponse: ErrorResponse = {
      success: false,
      error: 'Failed to fetch symbols',
      message:
        'An error occurred while fetching available symbols. Please try again.',
    };

    return NextResponse.json(errorResponse, { status: 500 });
  }
}
