...
const PATH_TARGETS = {
  ...
  "Path F (Alerts)": { pattern: /(lib\/jobs|app\/api\/(alerts|notifications))/, target: 10, tier: 2 },
  "Path G (Indicators)": { pattern: /app\/api\/indicators/, target: 10, tier: 2 },
...
function main() {
  ...
  if (!fs.existsSync(coveragePath)) {
    ...
    console.error("   npm test -- --coverage --coverageReporters=json-summary\n");
    process.exit(1);
  }
  ...
  console.log("├" + "─".repeat(15) + "┬" + "─".repeat(12) + "┬" + "─".repeat(18) + "┬" + "─".repeat(12) + "┬" + "─".repeat(16) + "┤");
  console.log("│ Metric        │ Current    │ Min Threshold    │ Status     │ Margin         │");
  console.log("├" + "─".repeat(15) + "┼" + "─".repeat(12) + "┼" + "─".repeat(18) + "┼" + "─".repeat(12) + "┼" + "─".repeat(16) + "┤");
  ...
  for (const metric of ["statements", "branches", "functions", "lines"]) {
    ...
    globalResults[metric] = { current, threshold, pass, margin: parseFloat(margin) };
  ...
  console.log("├" + "─".repeat(25) + "┬" + "─".repeat(8) + "┬" + "─".repeat(10) + "┬" + "─".repeat(10) + "┬" + "─".repeat(10) + "┬" + "─".repeat(11) + "┤");
  console.log("│ Path                    │ Tier   │ Target   │ Actual   │ Status   │ Margin    │");
  console.log("├" + "─".repeat(25) + "┼" + "─".repeat(8) + "┼" + "─".repeat(10) + "┼" + "─".repeat(10) + "┼" + "─".repeat(10) + "┼" + "─".repeat(11) + "┤");
  ...
  for (const [pathName, config] of Object.entries(PATH_TARGETS)) {
    ...
    const actual = totalStatements > 0
      ? (coveredStatements / totalStatements * 100).toFixed(2)
      : "N/A";
    const pass = actual !== "N/A" && parseFloat(actual) >= config.target;
    const margin = actual !== "N/A" ? (parseFloat(actual) - config.target).toFixed(2) : "N/A";
    const marginStr = margin !== "N/A" ? (margin >= 0 ? `+${margin}%` : `${margin}%`) : "N/A";
  ...
  const globalStatus = allGlobalPass ? "✅ ALL GLOBAL THRESHOLDS MET" : "❌ SOME GLOBAL THRESHOLDS NOT MET";
  const pathStatus = allPathsPass ? "✅ ALL PATH TARGETS MET" : "❌ SOME PATH TARGETS NOT MET";
  ...
  // Count stats
  const tier1Pass = Object.values(pathResults).filter(r => r.tier === 1 && r.pass).length;
  const tier1Total = Object.values(pathResults).filter(r => r.tier === 1).length;
  const tier2Pass = Object.values(pathResults).filter(r => r.tier === 2 && r.pass).length;
  const tier2Total = Object.values(pathResults).filter(r => r.tier === 2).length;
  const tier3Pass = Object.values(pathResults).filter(r => r.tier === 3 && r.pass).length;
  const tier3Total = Object.values(pathResults).filter(r => r.tier === 3).length;

  console.log("│ Tier 1 (Critical 25%): " + `${tier1Pass}/${tier1Total} paths passing`.padEnd(54) + " │");
  console.log("│ Tier 2 (Feature 10%):  " + `${tier2Pass}/${tier2Total} paths passing`.padEnd(54) + " │");
  console.log("│ Tier 3 (Utility 2%):   " + `${tier3Pass}/${tier3Total} paths passing`.padEnd(54) + " │");
