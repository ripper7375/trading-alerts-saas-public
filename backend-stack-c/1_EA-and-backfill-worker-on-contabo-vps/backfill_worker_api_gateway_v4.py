#!/usr/bin/env python3
"""
MT5 Data Backfill Worker v4 - API Gateway Version
Recovers data from SQLite backups and syncs to API Gateway
Runs continuously, checking every 5 minutes (configurable)

v4 Changelog (from v4):
- SCHEMA UPDATE (61 columns): Supports new "symbol" column at position 2
  (after timestamp). This column is automatically added when sending to API.
- COLUMN RENAME: Handles renamed "ema_26" → "ema" column for consistency
  with other moving averages (tema, hrma, smma). No code changes needed as
  columns are read dynamically from SQLite schema.
...
# Schema changes in v2.27 EA / v4 backfill worker:
# - v2.27 adds "symbol" column at position 2 (always added by worker on line 285)
# - v2.27 renames "ema_26" → "ema" (handled automatically via dynamic column reading)
# - v2.26 added these columns (may be absent in v2.26 or earlier backups):
V4_NEW_COLUMNS = {'dual_tema_high', 'dual_tema_low', 'pinbar'}
...
def create_http_session() -> requests.Session:
    ...
    session.headers.update({
        ...
        'X-EA-Version': 'backfill_worker_v4.py'
    })
...
def row_to_dict(row, columns: List[str]) -> Dict:
    ...
    Handles both v2.26 rows (no dual_tema_high/dual_tema_low/pinbar columns)
    and v2.27+ rows that include these new columns. Missing v4 columns are
...
def has_v4_columns(columns: List[str]) -> bool:
    ...
    Returns True if all three new columns (dual_tema_high, dual_tema_low, pinbar) are present.
    Note: v2.27 changes (symbol column, ema rename) are handled automatically.
    """
    return V4_NEW_COLUMNS.issubset(set(columns))
...
def backfill_symbol(session: requests.Session, symbol: str) -> Tuple[int, int, bool]:
    ...
    Handles databases from all versions:
    - v2.26 (60 columns, no dual_tema_high/dual_tema_low/pinbar, has ema_26)
    - v2.27+ (61 columns, with symbol column, ema renamed from ema_26)

    The row dict is built generically from whatever columns exist in SQLite.
    Symbol is always added to the payload before sending to API (line 285).
    ...
    try:
        ...
        # Get column names (detects whether v4 columns are present)
        columns = get_all_columns(cursor, symbol)
        db_has_v4 = has_v4_columns(columns)

        if not db_has_v4:
            logger.debug(
                f"ℹ️  {symbol}: database schema is pre-v2.26 "
                f"(missing: {V4_NEW_COLUMNS - set(columns)})"
            )
...
def main():
    ...
    logger.info("🚀 Starting MT5 Data Backfill Worker v4 (API Gateway Version)")
