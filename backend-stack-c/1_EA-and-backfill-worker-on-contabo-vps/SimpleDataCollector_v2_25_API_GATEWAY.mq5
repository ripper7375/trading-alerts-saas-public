//+------------------------------------------------------------------+
//|                          SimpleDataCollector_v2.25_API_GATEWAY.mq5|
...
// Version 2.25-API-Gateway Changelog (from v2.24):
// - 5-MINUTE TIMER: Periodic data collection every 5 minutes via OnTimer()
//   ensures bars are captured even during low-tick/low-liquidity periods
// - TICK + TIMER HYBRID: OnTick() still catches bars instantly, OnTimer()
//   acts as a safety net sweep every 5 minutes
// - CIRCUIT BREAKER: After N consecutive API failures, temporarily skip
//   API Gateway and write directly to SQLite to avoid hammering a dead API
// - BACKFILL FILE FIX: MarkForBackfill now uses FILE_READ|FILE_WRITE to
//   append properly instead of truncating the file (was a data loss bug)
// - WEBREQUEST TIMEOUT: Increased from 2s to 3s for better reliability
// - COLLECTED_AT IN PAYLOAD: API Gateway now receives collected_at timestamp
// - INDICATOR READINESS: Checks indicator data availability before collecting
// - HOURLY STATS: Still logged every hour via timer counter
// - All other v2.24 features preserved (multi-symbol, SQLite fallback, etc.)
...
// Indicator parameters (same as v2.24)
...
{
   Print("========================================");
   Print("SimpleDataCollector v2.25-API-Gateway");
   Print("========================================");
...
{
   // Build complete JSON payload with all 57 columns + collected_at
                       ...
                       double zigzag_peak, double zigzag_bottom, double ema_26)
{
   ...
   // Build complete INSERT statement with all 57 columns
   string insertSQL = StringFormat(
      ...
      "sr_support_4, sr_support_3, sr_support_2, sr_support_1, "
      "sr_resistance_1, sr_resistance_2, sr_resistance_3, sr_resistance_4, "
      "zigzag_peak, zigzag_bottom, ema_26, collected_at) "
