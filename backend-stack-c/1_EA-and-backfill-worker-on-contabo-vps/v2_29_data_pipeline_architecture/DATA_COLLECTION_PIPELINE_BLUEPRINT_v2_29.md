...
| --- | -------------------------------------------------------------------------- | ------------------------------------------------ | -------------------------------------------- | ----------- |
| 1   | `SimpleDataCollector_v2_29_ASYNC_SOCKET.mq5`                               | Data producer EA (v2.29, new indicator set)      | ✅ Updated; not part of v5 data flow (§12.7) | §4.1        |
| 2   | `backfill_worker_api_gateway_v5.py`                                        | SQLite → gateway recovery/push worker            | ⚠️ Needs v5 rework (§12.6)                   | §4.3        |
| 3   | `mt5_api_relay_for_v2_29.py`                                               | Local async upload relay (schema-agnostic)       | ✅ No change needed                          | §4.2        |
| 4   | `sqlite_schema_v6_xauusd.sql`                                              | v5 staging / validation / market-data schema     | ✅ Created, verified                         | §4.5, §12.3 |
| 5   | `export_collector_validator_v2.py`                                         | Ingest 12 export files → validate → promote      | ✅ Built, tested on mock data                | §4.6, §12   |
...
│  │ SQLite backups      │   ┌──────────────────────────────────────────┐              │
│  │ C:/Scripts/database │   │ backfill_worker_api_gateway_v5.py        │  HTTPS POST  │
...
### 4.1 EA — `SimpleDataCollector_v2_29_ASYNC_SOCKET.mq5` (internal version 2.29)
...
### 4.2 Relay — `mt5_api_relay_for_v2_29.py`
...
### 4.3 Backfill worker — `backfill_worker_api_gateway_v5.py`
...
### 4.5 v5 SQLite schema — `sqlite_schema_v6_xauusd.sql`
...
### 4.6 Export collector + validator — `export_collector_validator_v2.py`
...
- **Mock mode** (regression check, runs against the repo's mock data):
  `python3 export_collector_validator_v2.py --export-dir mock-data-from-indicators/time_series_data --db /tmp/test.db --timeframes M5 --once --no-market-hours --no-completeness`
...
```bat
nssm install MT5Relay "C:\Python311\python.exe" "C:\Scripts\relay\mt5_api_relay_for_v2_29.py"
...
│  └──────────────────────────────┘   ┌────────────────────────────────────────┐    │
│                                     │ export_collector_validator_v2.py (§4.6)│    │
...
│                                     │  CALCULATE→ Python calc stack (9-12)    │    │
...
### 12.3 v5 SQLite schema — `sqlite_schema_v6_xauusd.sql`
...
| Relay (`mt5_api_relay_for_v2_29.py`)                              | ✅     | Schema-agnostic (forwards JSON as-is; only reads `terminal_id`). No functional change needed                                                                                                                                                                                                                                                           |
| v5 schema (`sqlite_schema_v6_xauusd.sql`)                         | ✅     | Staging/validation/promote design + `synced_at` outbox column on `market_data`; executes cleanly                                                                                                                                                                                                                                                       |
...
| Export collector / validator (`export_collector_validator_v2.py`) | ✅     | Collect→adjust→validate→promote implemented incl. market-hours gate (Python port), ±0.01 close tolerance, zigzag subset rule, empty→NULL, bounded reject→re-request. Verified on mock data: strict mode correctly rejects stale exports; promote path fills `market_data` with NULL-correct LEFT-JOIN semantics                                        |
...
~~1. Export collector + validator~~ — **done**: `export_collector_validator_v2.py`
...
4. **Backfill worker v5** (rework of `backfill_worker_api_gateway_v5.py`) —
...
### 12.8 MQL5 → Python calculation shift (decided June 2026 — in progress)

**Decision:** MQL5 keeps the admin-configured input layer (OHLCV, fractal
high/low maps, SSA / ema_ssa / crossing, zigzag pivot detection); Python owns
all derived calculations with **user-configurable parameters** (window
dates, centroid in/exclusion, min EDT touches, tolerances, thresholds). This
enables the on-demand and user-configurable services and collapses the 6
centroid variants into one parameterized engine. Pipeline gains a CALCULATE
stage: collect → adjust → validate → **calculate** → promote; the same Python
modules double as the on-demand engine (user params, cached by param-hash).

**Stack:** `backend-stack-c/2_python-calc-stack/` (see its README for porting
rules and phase table). Status: ALL THREE PHASES PORTED — Phase 1 (z-score
candle, zigzag segment metrics; golden vs mock exports), Phase 2
(fractal/single-best lines), Phase 3 (centroid engine, six variants as one
parameterized module). Test suite 93/93. Full golden certification runs at
cutover against fresh 3000-row MQL5 exports (price-valued outputs within
1e-5 on identical windows).

### 12.9 v2.29/v6 integrated release (June 2026) — the current deployment set

The calc stack is wired into the pipeline; every component renamed and
updated in one coordinated release:

| Old name                       | New name                                      | Change                                                                                                                                                                                                               |
| ------------------------------ | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SimpleDataCollector*v2_28*…    | `SimpleDataCollector_v2_29_ASYNC_SOCKET.mq5`  | iCustom refs → `*_v2_29` indicator names; role note (charts-alive; not in v6 data flow)                                                                                                                              |
| backfill_worker_api_gateway_v4 | `backfill_worker_api_gateway_v5.py`           | REDESIGNED: pushes `market_data WHERE synced_at IS NULL`, stamps `synced_at` on 200/201 (never deletes); 400s quarantined + stamped so poison rows can't block the outbox                                            |
| mt5_api_relay_for_v2_28        | `mt5_api_relay_for_v2_29.py`                  | Version strings only (schema-agnostic)                                                                                                                                                                               |
| sqlite_schema_v5_xauusd        | `sqlite_schema_v6_xauusd.sql`                 | Staging slimmed to the ADMIN LAYER (keys + OHLCV + centroid maps/ssa/ema_ssa/crossing + zigzag pivots); `market_data` columns marked calculated; `calculated_at` added                                               |
| export_collector_validator_v1  | `export_collector_validator_v2.py`            | CALCULATE stage added (imports the four calc modules); HEADER-NAME-BASED parsing accepts both full (current) and reduced (future) exports                                                                            |
| 12 indicators `*.mq5`          | `*_v2_29.mq5`                                 | Renamed (compiled names change → EA updated). Exports stay FULL until golden certification — the MQL5 line columns ARE the golden data for certifying the calc stack; slimming to the reduced contract happens after |
| BLUEPRINT_v2_28.md             | `DATA_COLLECTION_PIPELINE_BLUEPRINT_v2_29.md` | This document                                                                                                                                                                                                        |
| schema_v5 preview              | `sqlite_schema_v6_xauusd_preview.txt`         | Regenerated from the v6 chain (shows CALCULATE-stage columns)                                                                                                                                                        |

The four calc modules keep their names (versioned by the stack, not the file):
`zscore_candle.py`, `zigzag_metrics.py`, `fractal_lines.py`,
`centroid_regression.py`.

**v6 data flow (verified end-to-end on mock data):** 12 indicator exports →
collector v2 stages admin-layer columns → cross-source validation →
CALCULATE (calc stack computes z-score candle set, zigzag metrics,
fractal/resistance/support lines, six centroid baselines + EDTs) → PROMOTE
into `market_data` (`calculated_at` stamped) → push worker v5 syncs the
outbox to the API gateway.

**Remaining before production cutover:**

1. Golden certification — fresh 3000-row manual exports from the 12
   indicators; compare Python CALCULATE output vs the MQL5 full-export line
   columns within 1e-5 (then optionally slim the auto-exports).
2. Gateway migration — accept the `market_data` field set (nullable derived
   columns) with idempotent upsert on `(symbol, timeframe, timestamp)`.
3. One open data-contract assumption to confirm at certification: the
   crossing PRICE is taken as the SSA value on the crossing bar (the MQL5
   cross buffer holds the SSA price at the cross).
...
| v5 schema (`sqlite_schema_v6_xauusd.sql`) | Staging + validation + `market_data`                | Export collector/validator + worker v5 (both to be built — §12.6)                                            |
