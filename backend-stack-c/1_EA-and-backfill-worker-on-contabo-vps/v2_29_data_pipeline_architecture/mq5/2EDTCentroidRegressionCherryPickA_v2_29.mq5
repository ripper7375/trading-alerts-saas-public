...
#property description "DavinTrade V3.896_1: Cherry-Pick Centroid Exclusion"
...
#define EXPORT_BUTTON_NAME "V3896_1_ExportButton"
...
input int               InpRegCentroids = 6; // Centroids IN USE
input string            InpExcludedCentroids = ""; // Excluded centroids (e.g. "1,3,5")
...
input color             InpBaseLineColor = clrSteelBlue;
input color             InpEDTColor = clrDarkBlue;
...
input string            InpExportFileName = "Cherry-Pick-A";
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DavinTrade SSA EDT V3.896_1");
   CreateExportButton();
...
void OnDeinit(const int reason)
{
   ...
   ObjectsDeleteAll(0, "ClusterHull_V896_1_");
   ObjectsDeleteAll(0, "ClusterCentroidStar_V896_1_");
...
void CreateExportButton()
{
   ...
   ObjectSetString(0, EXPORT_BUTTON_NAME, OBJPROP_TEXT, "ChrPic-A");
...
void ExpandCluster(const ClusterPoint &data[], int p_idx, int &neighbors[], int cluster_id, double eps, int min_pts, bool &visited[], int &assignments[]) {
   assignments[p_idx] = cluster_id; int i = 0;
...
void PerformClusteringAndEDT(const int rates_total, const datetime &time[], const double &close_arr[])
{
   ...
   if(p_count < InpMinPts) {
      if(InpShowComments) Comment("--- DavinTrade V3.896_1 ---\nAwaiting more data: Points < MinPts");
      return;
   }
   ...
   for(int k = 0; k < total_clusters; k++) {
      ...
      if(filtered_count >= 3) { 
         ...
         for(int h = 0; h < h_count; h++) {
            ...
            string line_name = "ClusterHull_V896_1_" + IntegerToString(k) + "_" + IntegerToString(h);
         ...
         string star_name = "ClusterCentroidStar_V896_1_" + IntegerToString(k);
   ...
   while(valid_count < user_target_reg && chrono_idx < centroid_count) {
       ...
       for(int e = 0; e < ArraySize(g_excluded_indices); e++) {
           if(g_excluded_indices[e] == chrono_idx) { is_excluded = true; break; }
       }
   ...
   for(int c = 0; c < MathMin(12, chrono_idx); c++) {
       bool is_excluded = false;
       for(int e = 0; e < ArraySize(g_excluded_indices); e++) {
           if(g_excluded_indices[e] == c) { is_excluded = true; break; }
       }
       if(!is_excluded) {
   ...
   if(leftmost_bar <= rightmost_bar && leftmost_bar != rates_total) {
      double sum_crossings = 0; double sum_close = 0;
      for(int i = leftmost_bar; i <= rightmost_bar; i++) {
          if(ExtSSACross[i] != EMPTY_VALUE && ExtSSACross[i] != 0.0) {
             sum_crossings += ExtSSACross[i]; n_crossings++;
          }
   ...
   if(InpShowComments) {
      string comment_text = StringFormat(
          "--- DavinTrade V3.896_1 A/B Statistical Pipeline ---\n" +
          ...
          "Skewness   : %-16.2f %.2f\n" +
          "Kurtosis   : %-16.2f %.2f",
          valid_count, (StringLen(InpExcludedCentroids)>0 ? InpExcludedCentroids : "None"),
          InpSSAMathLookback,
...
bool ExportData(bool silent = false)
{
   ...
   if(!InpCalcCentroidRegressionEDT) {
      ...
      FileWrite(fh_stat, "Regression Centroids (Box B): 0");
      FileWrite(fh_stat, "Excluded Centroids: None");
      FileWrite(fh_stat, "Math Search Window (Bars): " + IntegerToString(InpSSAMathLookback));
      FileWrite(fh_stat, "Visual EDT Window (Bars): 0");
      FileWrite(fh_stat, "Observation Window (Box B Bars): 0");
   ...
   // Refactored Export Header (Removed CEN columns, Renamed EDT columns to Cherry)
   FileWrite(fh_data, "Cherry_A_timestamp\tCherry_A_symbol\tCherry_A_timeframe\tCherry_A_close\tCherry_A_Base_FL\tCherry_A_UOEDT\tCherry_A_LOEDT\tCherry_A_horiz_high_map\tCherry_A_horiz_low_map\tCherry_A_ssa\tCherry_A_ema_ssa\tCherry_A_crossing");
   ...
   for(int i = start_idx; i < g_rates_total; i++) {
      ...
      // Data bounds handling (Tabs print empty if out of bounds or zero)
