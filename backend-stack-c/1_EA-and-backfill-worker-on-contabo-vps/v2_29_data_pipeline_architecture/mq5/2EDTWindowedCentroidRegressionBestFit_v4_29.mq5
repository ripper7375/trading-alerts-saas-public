//+------------------------------------------------------------------+
//|                        2EDTWindowedCentroidRegression_v3_00.mq5  |
...
#define EXPORT_BUTTON_NAME "V3_00_ExportButton"
...
input string            InpExportFileName = "Centriod_Windowed";
...
int g_stat_window_start_idx = 0;
int g_stat_window_end_idx = 0;
...
void CreateExportButton()
{
   ...
   ObjectSetString(0, EXPORT_BUTTON_NAME, OBJPROP_TEXT, "Export Windowed");
...
void PerformClusteringAndCFL(const int rates_total, const datetime &time_arr[], const double &close_arr[])
{
   ...
   for(int i = window_start_idx; i <= window_end_idx; i++) {
      if(ExtSSACross[i] != EMPTY_VALUE && ExtSSACross[i] != 0.0) {
         ...
         points[sz].bar = i; points[sz].time = time_arr[i]; points[sz].price = ExtSSACross[i];
      }
   }
   ...
   for(int k = 0; k < total_clusters; k++) {
      ...
      if(filtered_count >= 3) { 
         ...
         for(int h = 0; h < h_count; h++) {
            ...
            string line_name = "ClusterHull_V3_" + IntegerToString(k) + "_" + IntegerToString(h);
         ...
         datetime centroid_time = time_arr[time_index];
         
         string star_name = "ClusterCentroidStar_V3_" + IntegerToString(k);
...
bool ExportData(bool silent = false)
{
   ...
   FileWrite(fh_stat, "Window Start TS (UTC): " + IntegerToString(g_stat_window_start_ts));
   FileWrite(fh_stat, "Window End TS (UTC): " + IntegerToString(g_stat_window_end_ts));
   ...
   // Export from start of the window up to live price to capture both In-Sample and Out-Of-Sample data
   for(int i = g_stat_window_start_idx; i < g_rates_total; i++) {
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   for(int i = 0; i < len; i++) vecClose[i] = close[ssa_start_idx + i];
   ...
   if(prev_calculated == 0) {
      ...
      if(trend.Size() == len) {
         ...
         for(int i = 0; i < len; i++) ExtSSATrend[ssa_start_idx + i] = vecTrend[i];
         
         ExtSSASignal[ssa_start_idx] = vecTrend[0]; ExtSSACross[ssa_start_idx] = EMPTY_VALUE;
         for(int i = 1; i < len; i++) {
            int idx = ssa_start_idx + i;
