# 15th Indicator Specification: `S-R-AutoCalibration_v2_29`

**Document Version:** 1.0.0  
**Target System:** DavinTrade App / Backend Stack C (Data Pipeline v2.29)  
**Source Baseline:** [`SupportAndResistantAutoCalibration_v2_29.mq5`](SupportAndResistantAutoCalibration_v2_29.mq5) (14th Indicator)  
**Implementation File:** [`S-R-AutoCalibration_v2_29.mq5`](S-R-AutoCalibration_v2_29.mq5)

---

## 1. Executive Summary

`S-R-AutoCalibration_v2_29` is replicated from `SupportAndResistantAutoCalibration_v2_29` to serve as the **15th Indicator** in the DavinTrade App ecosystem.

While the 14th indicator exports price level slots **`sr_1` through `sr_8`**, this 15th indicator is mapped to export slots **`sr_9` through `sr_16`**, allowing independent calibration parameters (such as different date anchors `InpStartDateTime`/`InpEndDateTime`, lookback modes, or timeframe clustering) while coexisting in the same data pipeline.

---

## 2. Slot Mapping: Indicator 14 vs. Indicator 15

| Price Category   | Relative Position to Close           | Indicator 14 (`SupportAndResistantAutoCalibration`) | Indicator 15 (`S-R-AutoCalibration`) |
| :--------------- | :----------------------------------- | :-------------------------------------------------: | :----------------------------------: |
| **Support 1**    | Closest Support below `close`        |                       `sr_1`                        |              **`sr_9`**              |
| **Support 2**    | 2nd closest Support below `close`    |                       `sr_2`                        |             **`sr_10`**              |
| **Support 3**    | 3rd closest Support below `close`    |                       `sr_3`                        |             **`sr_11`**              |
| **Support 4**    | Furthest Support below `close`       |                       `sr_4`                        |             **`sr_12`**              |
| **Resistance 1** | Closest Resistance above `close`     |                       `sr_5`                        |             **`sr_13`**              |
| **Resistance 2** | 2nd closest Resistance above `close` |                       `sr_6`                        |             **`sr_14`**              |
| **Resistance 3** | 3rd closest Resistance above `close` |                       `sr_7`                        |             **`sr_15`**              |
| **Resistance 4** | Furthest Resistance above `close`    |                       `sr_8`                        |             **`sr_16`**              |

---

## 3. Data Contract Specification

### 3.1 TimeSeries Export Contract (TSV)

- **File Naming Pattern:** `{InpExportFileName}_{Symbol}_{Timeframe}.txt`
  - Default: `S_R_Levels_XAUUSD_M5.txt` and `S_R_Levels_XAUUSD_M15.txt`
- **Output Directory:** `MQL5/Files/`
- **Cadence:** Auto-export every minute at second `:59` via timer loop (`InpAutoExport=true`, `InpExportSecond=59`).
- **Depth:** 3,000 bars (`InpExportBars=3000`).
- **TSV Header (Exactly 12 Columns):**
  ```tsv
  timestamp\tsymbol\ttimeframe\tclose\tsr_9\tsr_10\tsr_11\tsr_12\tsr_13\tsr_14\tsr_15\tsr_16\r\n
  ```
- **Null Safety Contract:** Inactive/unresolved slots export as empty string tabs `""` (staged cleanly as SQL `NULL`, avoiding `0.00` sentinels).

### 3.2 Companion Statistic File Contract

- **File Naming Pattern:** `S_R_Levels_{Symbol}_{Timeframe}_Statistic.txt`
- **Structure:**

  ```ini
  [SUPPORT-RESISTANCE AUTO-CALIBRATION - PARAMETERS]
  Calculation Mode: Freedman-Diaconis IQR (Auto-Calibrated)
  Window Mode: Fixed Window
  Window Start TS (UTC): ...
  Window End TS (UTC): ...
  Window Bars: 142
  ...
  Total Macro Clusters: ...
  Pipeline 1:1 Slots: 8 (sr_9..sr_16)
  sr_9 (Support 1): 4275.36
  sr_10 (Support 2): 4260.71
  sr_11 (Support 3): NULL
  sr_12 (Support 4): NULL
  sr_13 (Resistance 1): 4285.60
  sr_14 (Resistance 2): 4297.28
  sr_15 (Resistance 3): 4311.48
  sr_16 (Resistance 4): 4317.52
...
## 4. Multi-Indicator Coexistence Isolation

To prevent conflicts when running both Indicator 14 (`SupportAndResistantAutoCalibration`) and Indicator 15 (`S-R-AutoCalibration`) on the same chart or terminal:

1. **Object Prefix Isolation:**
   - Indicator 14: `MQLTA-SR-Auto-v2_29`
   - Indicator 15: `MQLTA-S-R-Auto-v2_29`
   - Prevents `CleanChart()` and `CleanLines()` from deleting each other's on-chart ray lines and tags.
2. **Button Identifier Namespace:**
   - Indicator 14: `SRAutoExportButton_v2_29`, `SRAutoBackfillButton_v2_29`
   - Indicator 15: `S_R_2_AutoExportButton_v2_29`, `S_R_2_AutoBackfillButton_v2_29`
3. **On-Chart GUI Button Layout:**
   - Indicator 14 buttons are positioned at `x_margin = 200`
   - Indicator 15 buttons are positioned at `x_margin = 390` (width 180 + 10px padding)
   - Both indicators can be added to the exact same chart window without button overlap.
4. **Visual Color Distinctiveness:**
   - Indicator 14: Red (Resistances), Green (Supports), Blue/Green Buttons
   - Indicator 15: Magenta (Resistances `sr_13..sr_16`), Aqua (Supports `sr_9..sr_12`), Purple/Amber Buttons
5. **Export File Separation:**
   - Indicator 14: `SR_Levels_*.txt`
   - Indicator 15: `S_R_Levels_*.txt`

---

## 5. Complete MQL5 Source Code

The full implementation is committed at [`S-R-AutoCalibration_v2_29.mq5`](S-R-AutoCalibration_v2_29.mq5).

```mql5
//+------------------------------------------------------------------+
//|                                     S-R-AutoCalibration_v2_29.mq5 |
//|                                  Copyright 2026, MetaQuotes Ltd. |
//|                                             https://www.mql5.com |
//| Description: Dynamic S/R Levels with Freedman-Diaconis IQR       |
//|              Auto-Calibration & Fixed Date Anchors (v2.29)       |
//|              (15th Indicator - Slots sr_9..sr_16)                |
//+------------------------------------------------------------------+
#property copyright   "Copyright 2026, MetaQuotes Ltd."
#property link        "https://www.mql5.com"
#property version     "2.29"
#property description "Support & Resistance with Freedman-Diaconis IQR Auto-Calibration (sr_9..sr_16) & Stack C Pipeline"
#property strict
#property indicator_chart_window

#property indicator_buffers 8
#property indicator_plots   8
#property indicator_label1  "sr_12"
#property indicator_label2  "sr_11"
#property indicator_label3  "sr_10"
#property indicator_label4  "sr_9"
#property indicator_label5  "sr_13"
#property indicator_label6  "sr_14"
#property indicator_label7  "sr_15"
#property indicator_label8  "sr_16"

//--- Export Button name constants (namespaced for Indicator 15)
#define EXPORT_BUTTON_NAME   "S_R_2_AutoExportButton_v2_29"
#define BACKFILL_BUTTON_NAME "S_R_2_AutoBackfillButton_v2_29"
...
input color           ResistanceColor  = clrMagenta;           // Resistance Color (sr_13..sr_16)
input color           SupportColor     = clrAqua;              // Support Color (sr_9..sr_12)
input bool            InpShowSlotTags  = true;                 // Show sr_9..sr_16 Slot Tags on Chart
...
input string InpExportFileName       = "S_R_Levels";          // Export file prefix ({Prefix}_{Symbol}_{TF}.txt)
...
input string IndicatorName           = "MQLTA-S-R-Auto-v2_29"; // Object Prefix / Indicator Name
...
// 8 indicator plot buffers (Indicator 15: sr_9..sr_16)
double BufferZero[];   // Support 4 (furthest below: sr_12)
double BufferOne[];    // Support 3 (sr_11)
double BufferTwo[];    // Support 2 (sr_10)
double BufferThree[];  // Support 1 (closest below: sr_9)
double BufferFour[];   // Resistance 1 (closest above: sr_13)
double BufferFive[];   // Resistance 2 (sr_14)
double BufferSix[];    // Resistance 3 (sr_15)
double BufferSeven[];  // Resistance 4 (furthest above: sr_16)
...
void CreateExportButton()
{
   ...
   int x_margin      = 390; // Placed alongside Indicator 14 button (x=200, width=180 + 10px padding)
   int y_margin      = 55;
   ...
   ObjectSetString(0, EXPORT_BUTTON_NAME, OBJPROP_TEXT, "Auto-S-R-2 Export");
   ...
   ObjectSetInteger(0, EXPORT_BUTTON_NAME, OBJPROP_BGCOLOR, C'110,60,180');
   ObjectSetInteger(0, EXPORT_BUTTON_NAME, OBJPROP_BORDER_COLOR, C'90,40,150');
...
void CreateBackfillButton()
{
   ...
   int button_width  = 180;
   int button_height = 35;
   int x_margin      = 390; // Placed alongside Indicator 14 button
   int y_margin      = 15;
   ...
   ObjectSetString(0, BACKFILL_BUTTON_NAME, OBJPROP_TEXT, "Auto-S-R-2 Backfill");
   ...
   ObjectSetInteger(0, BACKFILL_BUTTON_NAME, OBJPROP_BGCOLOR, C'180,90,20');
   ObjectSetInteger(0, BACKFILL_BUTTON_NAME, OBJPROP_BORDER_COLOR, C'150,70,10');
...
void WriteSRStatFile(string clean_symbol, string tf_str)
{
   ...
   FileWriteString(fh, "Pipeline 1:1 Slots: 8 (sr_9..sr_16)\r\n");
   FileWriteString(fh, "sr_9 (Support 1): "       + (IsValidSRLevel(BufferThree[0]) ? SRPriceToString(BufferThree[0]) : "NULL") + "\r\n");
   FileWriteString(fh, "sr_10 (Support 2): "      + (IsValidSRLevel(BufferTwo[0])   ? SRPriceToString(BufferTwo[0])   : "NULL") + "\r\n");
   FileWriteString(fh, "sr_11 (Support 3): "      + (IsValidSRLevel(BufferOne[0])   ? SRPriceToString(BufferOne[0])   : "NULL") + "\r\n");
   FileWriteString(fh, "sr_12 (Support 4): "      + (IsValidSRLevel(BufferZero[0])  ? SRPriceToString(BufferZero[0])  : "NULL") + "\r\n");
   FileWriteString(fh, "sr_13 (Resistance 1): "   + (IsValidSRLevel(BufferFour[0])  ? SRPriceToString(BufferFour[0])  : "NULL") + "\r\n");
   FileWriteString(fh, "sr_14 (Resistance 2): "   + (IsValidSRLevel(BufferFive[0])  ? SRPriceToString(BufferFive[0])  : "NULL") + "\r\n");
   FileWriteString(fh, "sr_15 (Resistance 3): "   + (IsValidSRLevel(BufferSix[0])   ? SRPriceToString(BufferSix[0])   : "NULL") + "\r\n");
   FileWriteString(fh, "sr_16 (Resistance 4): "   + (IsValidSRLevel(BufferSeven[0]) ? SRPriceToString(BufferSeven[0]) : "NULL") + "\r\n");
...
bool ExportSRData(bool is_backfill = false)
{
   ...
   // 1. TSV Header (Indicator 15: sr_9..sr_16 strictly conforming to Stack C schema)
   if(InpIncludeHeader)
   {
      string header = "timestamp\tsymbol\ttimeframe\tclose\tsr_9\tsr_10\tsr_11\tsr_12\tsr_13\tsr_14\tsr_15\tsr_16\r\n";
   ...
   for(int shift = export_limit; shift >= 0; shift--)
   {
      ...
      // Evaluate levels relative to THIS bar's close price (sr_9..sr_16)
      double sr_support_9 = 0, sr_support_10 = 0, sr_support_11 = 0, sr_support_12 = 0;
      double sr_resistance_13 = 0, sr_resistance_14 = 0, sr_resistance_15 = 0, sr_resistance_16 = 0;
      ...
      for(int i = 0; i < ArraySize(ArrayLevels) && r_count < 4; i++)
      {
         if(IsValidSRLevel(ArrayLevels[i]) && ArrayLevels[i] > bar_close)
         {
            switch(r_count)
            {
               case 0: sr_resistance_13 = ArrayLevels[i]; break;
               case 1: sr_resistance_14 = ArrayLevels[i]; break;
               case 2: sr_resistance_15 = ArrayLevels[i]; break;
               case 3: sr_resistance_16 = ArrayLevels[i]; break;
            }
            r_count++;
         }
      }
      ...
      for(int i = ArraySize(ArrayLevels) - 1; i >= 0 && s_count < 4; i--)
      {
         if(IsValidSRLevel(ArrayLevels[i]) && ArrayLevels[i] < bar_close)
         {
            switch(s_count)
            {
               case 0: sr_support_9  = ArrayLevels[i]; break;
               case 1: sr_support_10 = ArrayLevels[i]; break;
               case 2: sr_support_11 = ArrayLevels[i]; break;
               case 3: sr_support_12 = ArrayLevels[i]; break;
            }
            s_count++;
         }
      }
      ...
      string support_9_str  = SRPriceToString(sr_support_9);
      string support_10_str = SRPriceToString(sr_support_10);
      string support_11_str = SRPriceToString(sr_support_11);
      string support_12_str = SRPriceToString(sr_support_12);

      string resist_13_str  = SRPriceToString(sr_resistance_13);
      string resist_14_str  = SRPriceToString(sr_resistance_14);
      string resist_15_str  = SRPriceToString(sr_resistance_15);
      string resist_16_str  = SRPriceToString(sr_resistance_16);
...
//+------------------------------------------------------------------+
//| Draw horizontal lines on chart (Strict 1:1 with exported sr_9..16)|
//+------------------------------------------------------------------+
void DrawLines(double currentClose)
{
   CleanLines();
   ...
   // Supports:   sr_9=BufferThree[0], sr_10=BufferTwo[0], sr_11=BufferOne[0], sr_12=BufferZero[0]
   // Resistances: sr_13=BufferFour[0], sr_14=BufferFive[0], sr_15=BufferSix[0], sr_16=BufferSeven[0]

   double levels[8];
   string names[8] = {"sr_9", "sr_10", "sr_11", "sr_12", "sr_13", "sr_14", "sr_15", "sr_16"};
   bool   is_res[8] = {false, false, false, false, true, true, true, true};

   levels[0] = BufferThree[0]; // Support 1 (closest below: sr_9)
   levels[1] = BufferTwo[0];   // Support 2 (sr_10)
   levels[2] = BufferOne[0];   // Support 3 (sr_11)
   levels[3] = BufferZero[0];  // Support 4 (furthest below: sr_12)
   levels[4] = BufferFour[0];  // Resistance 1 (closest above: sr_13)
   levels[5] = BufferFive[0];  // Resistance 2 (sr_14)
   levels[6] = BufferSix[0];   // Resistance 3 (sr_15)
   levels[7] = BufferSeven[0]; // Resistance 4 (furthest above: sr_16)
...
void DisplayDistanceComments()
{
   ...
   string commentText = "Support & Resistance Auto-Calibration (v2.29 - Indicator 15) HUD:\n";
   ...
   commentText += "Pipeline 1:1 Alignment: Strict (Max 8 Slots: sr_9..sr_16)\n";

   if(DistanceFromResistance != INT_MAX)
      commentText += "Distance to Resistance (sr_13): " + IntegerToString(DistanceFromResistance) + " points (" + DoubleToString(LevelAbove, _Digits) + ")\n";
   else
      commentText += "No resistance level (sr_13) found\n";

   if(DistanceFromSupport != INT_MAX)
      commentText += "Distance to Support (sr_9): " + IntegerToString(DistanceFromSupport) + " points (" + DoubleToString(LevelBelow, _Digits) + ")\n";
   else
      commentText += "No support level (sr_9) found\n";
   ...
   commentText += StringFormat("Exported Active Slots: %d Supports (sr_9-sr_%d) | %d Resistances (sr_13-sr_%d)",
                               act_sup, act_sup > 0 ? (8 + act_sup) : 0,
                               act_res, act_res > 0 ? (12 + act_res) : 0);
...
void FillBuffers(double currentClose)
{
   ...
   // Fill resistance levels (above current close: sr_13..sr_16)
   int j = 0;
   for(int i = 0; i < ArraySize(ArrayLevels); i++)
   {
      if(IsValidSRLevel(ArrayLevels[i]) && ArrayLevels[i] > currentClose)
      {
         switch(j)
         {
            case 0: BufferFour[0]  = NormalizeDouble(ArrayLevels[i], _Digits); break; // sr_13
            case 1: BufferFive[0]  = NormalizeDouble(ArrayLevels[i], _Digits); break; // sr_14
            case 2: BufferSix[0]   = NormalizeDouble(ArrayLevels[i], _Digits); break; // sr_15
            case 3: BufferSeven[0] = NormalizeDouble(ArrayLevels[i], _Digits); break; // sr_16
         }
         j++;
         if(j == 4) break;
      }
   ...
   // Fill support levels (below current close - reverse order for closest first: sr_9..sr_12)
   j = 0;
   for(int i = ArraySize(ArrayLevels) - 1; i >= 0; i--)
   {
      if(IsValidSRLevel(ArrayLevels[i]) && ArrayLevels[i] < currentClose)
      {
         switch(j)
         {
            case 0: BufferThree[0] = NormalizeDouble(ArrayLevels[i], _Digits); break; // sr_9
            case 1: BufferTwo[0]   = NormalizeDouble(ArrayLevels[i], _Digits); break; // sr_10
            case 2: BufferOne[0]   = NormalizeDouble(ArrayLevels[i], _Digits); break; // sr_11
            case 3: BufferZero[0]  = NormalizeDouble(ArrayLevels[i], _Digits); break; // sr_12
         }
         j++;
         if(j == 4) break;
      }
                ...
                const int &spread[])
{
   ...
   // ================================================================
   // Update buffers with closest levels (sr_9..sr_16)
   FillBuffers(close[0]);
   ...
   if (level_pair_changed || new_tag_bar)
   {
      ...
      {
         Print(StringFormat(
            "S&R crossing at %s: close=%s  sr_13(above)=%s  sr_9(below)=%s",
...
void OnTimer()
{
   ...
   if(time_struct.sec == InpExportSecond && time_struct.min != last_trigger_min)
   {
      ...
      if(ExportSRData())
         Print("SUCCESS: [Auto Export] S-R Indicator 15 data exported");
      else
         Print("ERROR: [Auto Export] Failed to export S-R Indicator 15 data.");
   }
}
                  ...
                  const string &sparam)
{
   ...
   {
      if(ExportSRData())
         Print("SUCCESS: [Export All] S-R Indicator 15 exported");
      else
         Print("ERROR: [Export All] Failed to export S-R Indicator 15 levels.");
      return;
   }
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      {
         if(ExportSRData())
            Print("SUCCESS: [Manual Export] S-R Indicator 15 levels exported successfully");
         else
            Print("ERROR: [Manual Export] Failed to export S-R Indicator 15 levels.");
