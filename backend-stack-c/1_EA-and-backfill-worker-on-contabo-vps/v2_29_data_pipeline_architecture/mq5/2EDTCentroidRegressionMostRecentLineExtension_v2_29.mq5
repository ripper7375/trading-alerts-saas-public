...
#property version   "3.891_3"
#property description "DavinTrade V3.891_3: Refactored with Outermost EDTs Only"
...
#define EXPORT_BUTTON_NAME "V3891_3_ExportButton"
...
input string            InpExportFileName = "Most-Recent";
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DavinTrade SSA EDT V3.891_3");
   CreateExportButton();
...
void OnDeinit(const int reason)
{
   ...
   ObjectsDeleteAll(0, "ClusterHull_V891_3_");
   ObjectsDeleteAll(0, "ClusterCentroidStar_V891_3_");
...
void CreateExportButton()
{
   ...
   ObjectSetInteger(0, EXPORT_BUTTON_NAME, OBJPROP_YDISTANCE, 150);
   ObjectSetInteger(0, EXPORT_BUTTON_NAME, OBJPROP_XSIZE, 160);
   ObjectSetInteger(0, EXPORT_BUTTON_NAME, OBJPROP_YSIZE, 30);
   ObjectSetString(0, EXPORT_BUTTON_NAME, OBJPROP_TEXT, "Most-Recent");
   ObjectSetInteger(0, EXPORT_BUTTON_NAME, OBJPROP_COLOR, clrWhite);
   ObjectSetInteger(0, EXPORT_BUTTON_NAME, OBJPROP_BGCOLOR, clrDarkViolet);
...
void PerformClusteringAndEDT(const int rates_total, const datetime &time[], const double &close_arr[])
{
   ...
   if(p_count < InpMinPts) {
      if(InpShowComments) Comment("--- DavinTrade V3.891_3 ---\nAwaiting more data: Points < MinPts");
      return;
   }
   ...
   for(int k = 0; k < total_clusters; k++) {
      ...
      if(filtered_count >= 3) { 
         ...
         for(int h = 0; h < h_count; h++) {
            ...
            string line_name = "ClusterHull_V891_3_" + IntegerToString(k) + "_" + IntegerToString(h);
         ...
         string star_name = "ClusterCentroidStar_V891_3_" + IntegerToString(k);
   ...
   if(InpShowComments) {
      string comment_text = StringFormat(
          "--- DavinTrade V3.891_3 A/B Statistical Pipeline ---\n" +
...
bool ExportData(bool silent = false)
{
   ...
   if(!InpCalcCentroidRegressionEDT) {
      ...
      FileWrite(fh_stat, "Regression Centroids: 0");
      ...
      FileWrite(fh_stat, "Regression Centroids: " + IntegerToString(g_stat_centroids));
   ...
   // Refactored Export Header (Removed CEN columns, Renamed EDT columns to Most_Recent)
   FileWrite(fh_data, "Most_Recent_timestamp\tMost_Recent_symbol\tMost_Recent_timeframe\tMost_Recent_close\tMost_Recent_Base_FL\tMost_Recent_UOEDT\tMost_Recent_LOEDT\tMost_Recent_horiz_high_map\tMost_Recent_horiz_low_map\tMost_Recent_ssa\tMost_Recent_ema_ssa\tMost_Recent_crossing");
