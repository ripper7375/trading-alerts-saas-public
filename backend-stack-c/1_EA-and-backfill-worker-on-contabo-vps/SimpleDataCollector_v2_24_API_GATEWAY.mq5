//+------------------------------------------------------------------+
//|                          SimpleDataCollector_v2.24_API_GATEWAY.mq5|
...
// Version 2.24-API-Gateway Changelog:
// - MULTI-SYMBOL: Collect 3-5 symbols per terminal
// - API GATEWAY PRIMARY: Data validation before queuing
// - SQLITE FALLBACK: Safety net only when API Gateway fails
// - SYMBOL DETECTION: Auto-detect Eightcap suffixes (.i for forex)
// - SINGLE FILE PER SYMBOL: Separate SQLite database per symbol
// - BACKFILL TRACKING: Marks failed API publishes for recovery
// - STATISTICS: Real-time monitoring of success/failure rates
// - ENHANCED ERROR HANDLING: 400, 401, 429 response handling
...
// API Gateway configuration (UPDATED for API Gateway)
...
input bool EnableAPIGateway = true;  // Enable API Gateway publishing (disable for SQLite-only testing)
...
// Collection statistics (UPDATED for API Gateway)
...
{
   Print("========================================");
   Print("SimpleDataCollector v2.24-API-Gateway");
   Print("========================================");
   ...
   // Set up timer for statistics (1 hour)
   EventSetTimer(3600);
...
void OnDeinit(const int reason)
{
   ...
   // Close all database connections (SQLite handles cleanup automatically)
   // Database connections are closed automatically when EA is removed
...
//+------------------------------------------------------------------+
//| Timer function (called every hour)                                |
//+------------------------------------------------------------------+
void OnTimer()
{
   ...
   for(int symbolIndex = 0; symbolIndex < symbolCount; symbolIndex++)
   {
      for(int tfIndex = 0; tfIndex < 9; tfIndex++)
      {
         ...
         if(barTime != lastProcessed[symbolIndex][tfIndex])
         {
            ...
            InsertCandle(symbolIndex, tfIndex);
...
bool InsertCandle(int symbolIndex, int tfIndex)
{
   ...
   if(EnableAPIGateway)
   {
      ...
      // ❌ API Gateway failed (network issue, validation error, or rate limit)
      // Will fall through to SQLite backup for recovery
      ...
      Print("⚠️ API Gateway failed for ", sanitizedName, " ", EnumToString(tf), 
            " - backing up to SQLite");
   }
   
   // ============================================================
   // FALLBACK PATH: SQLite backup (only runs if API Gateway failed or disabled)
   // ============================================================
...
{
   // Build complete JSON payload with all 57 columns
   ...
   int httpCode = WebRequest(
      ...
      url,
      headers,
      2000,  // 2 second timeout (fail fast for better fallback)
      postData,
      resultData,
      resultHeaders
   );
