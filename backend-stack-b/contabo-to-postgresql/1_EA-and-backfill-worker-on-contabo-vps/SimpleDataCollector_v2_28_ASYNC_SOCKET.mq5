...
//
// v2.28 revision (reliability hardening):
// - SocketTimeoutMs default raised 5ms -> 50ms (avoids spurious circuit-breaker trips under VPS load)
// - SocketSend now requires the FULL payload to be written (partial send counted as failure)
// - MarkForBackfill opens backfill_queue.csv with FILE_SHARE_READ (no lost rows while worker reads it)
// - Companion relay (mt5_api_relay_for_v2_28.py) gained a persistent retry queue + EOF-safe reads
