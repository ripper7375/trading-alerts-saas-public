#!/usr/bin/env python3
"""
MT5 Data Backfill Worker v3 - API Gateway Version
...
- DUAL TEMA HIGH/LOW: Supports new dual_tema_high and dual_tema_low columns
  collected from Dual_TEMA_High_Low indicator (buffer 0 = high, buffer 1 = low)
- PINBAR DETECTION: Supports new pinbar column (1 = pinbar detected, 0 = none)
  collected from Pinbar Detector_Diamond Symbol_V17 indicator
- SCHEMA MIGRATION: Auto-detects and handles databases with or without new columns
  (backward compatible with v2.25 SQLite backups missing the new columns)
- EA VERSION HEADER: Updated to backfill_worker_v3.py
...
# New columns added in v2.26 EA / v3 backfill worker
# These columns may be absent in older SQLite backups created by v2.25 or earlier.
# The worker handles both cases: if present, they are sent to the API; if absent,
# the row dict will simply not contain them (API should treat as null/0).
V3_NEW_COLUMNS = {'dual_tema_high', 'dual_tema_low', 'pinbar'}
...
def create_http_session() -> requests.Session:
    ...
    session.headers.update({
        ...
        'X-EA-Version': 'backfill_worker_v3.py'
    })
...
def row_to_dict(row, columns: List[str]) -> Dict:
    ...
    Handles both v2.25 rows (no dual_tema_high/dual_tema_low/pinbar columns)
    and v2.26+ rows that include these new columns. Missing v3 columns are
    omitted from the dict — the API Gateway should treat absent fields as null.
    """
    data = {}
...
def has_v3_columns(columns: List[str]) -> bool:
    """
    Check whether this table already has the v2.26 / v3 columns.
    Returns True if all three new columns are present.
    """
    return V3_NEW_COLUMNS.issubset(set(columns))
...
def backfill_symbol(session: requests.Session, symbol: str) -> Tuple[int, int, bool]:
    ...
    Handles both v2.25 databases (without dual_tema_high/dual_tema_low/pinbar)
    and v2.26+ databases (with these columns). The row dict is built generically
    from whatever columns exist — the API receives exactly what is stored.
    ...
    try:
        ...
        # Get column names (detects whether v3 columns are present)
        columns = get_all_columns(cursor, symbol)
        db_has_v3 = has_v3_columns(columns)

        if not db_has_v3:
            logger.debug(
                f"ℹ️  {symbol}: database schema is pre-v2.26 "
                f"(missing: {V3_NEW_COLUMNS - set(columns)})"
            )
...
def main():
    ...
    logger.info("🚀 Starting MT5 Data Backfill Worker v3 (API Gateway Version)")
    ...
    logger.info(f"   v3 new columns: {', '.join(sorted(V3_NEW_COLUMNS))}")
    logger.info("=" * 60)
