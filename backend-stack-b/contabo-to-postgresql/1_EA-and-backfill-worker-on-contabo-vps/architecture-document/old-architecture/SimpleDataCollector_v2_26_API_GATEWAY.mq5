//+------------------------------------------------------------------+
//|                          SimpleDataCollector_v2.26_API_GATEWAY.mq5|
...
// Version 2.26-API-Gateway Changelog (from v2.25):
// - DUAL TEMA HIGH/LOW: Added data collection from Dual_TEMA_High_Low indicator
//   Buffer 0 = dual_tema_high (TEMA of bar highs)
//   Buffer 1 = dual_tema_low  (TEMA of bar lows)
// - PINBAR DETECTION: Added data collection from Pinbar Detector_Diamond Symbol_V17
//   Buffer 0 = BullishPinbars, Buffer 1 = BearishPinbars
//   Combined into single pinbar column: 1 if any pinbar detected, 0 otherwise
//   DisplayMode=INDICATOR_BUFFERS(0) forced for reliable buffer access
// - NEW COLUMNS: dual_tema_high, dual_tema_low, pinbar added to schema
// - SCHEMA MIGRATION: Auto-adds new columns to existing databases on startup
// - All v2.25 features preserved (circuit breaker, timer, multi-symbol, etc.)
...
input string APIGatewayURL = "https://your-api.railway.app";  // API Gateway URL
input string APIKey = "YOUR_API_KEY_HERE";  // API Key for authentication
...
input bool EnableAPIGateway = true;  // Enable API Gateway publishing

// Timer & circuit breaker settings
input int DataCollectionIntervalSec = 300;  // Data collection interval in seconds (300 = 5 min)
input int CircuitBreakerThreshold = 10;     // Consecutive API failures before circuit opens
input int CircuitBreakerCooldownSec = 300;  // Seconds to wait before retrying API after circuit opens

// Indicator parameters (same as v2.25)
...
input int InpDualTEMA_Period = 9;  // Dual TEMA EMA period (for high/low lines)

// --- Pinbar Detector parameters ---
// StringencyLevel: 0=Less Stringent, 1=Baseline, 2=More Stringent
input int    InpPinbar_StringencyLevel    = 1;     // Pinbar stringency (0=less, 1=baseline, 2=more)
input int    InpPinbar_CountBars          = 500;   // Pinbar lookback bars
input bool   InpPinbar_UseManualSettings  = true;  // Override pinbar automatic scheme
input double InpPinbar_MinWickSize        = 0.75;  // Pinbar min wick size ratio
input double InpPinbar_MaxBodySize        = 0.25;  // Pinbar max body size ratio
input double InpPinbar_BodyPosition       = 0.75;  // Pinbar body position threshold
input double InpPinbar_Protruding         = 0.40;  // Pinbar min nose protrusion ratio
...
struct CircuitBreakerState
{
   ...
   void RecordFailure(int threshold, int cooldownSec)
   {
      ...
      if(consecutiveFailures >= threshold && !isOpen)
      {
         ...
         Print("🔴 CIRCUIT BREAKER OPEN: ", consecutiveFailures,
               " consecutive API failures. Skipping API for ", cooldownSec, "s");
      }
   }
   ...
   bool ShouldSkipAPI(int cooldownSec)
   {
      ...
      {
         Print("🟡 CIRCUIT BREAKER HALF-OPEN: Cooldown elapsed, retrying API...");
         isOpen = false;
         consecutiveFailures = 0;
         return false;
      }

      return true;  // Still in cooldown, skip API
   }
};
...
{
   Print("========================================");
   Print("SimpleDataCollector v2.26-API-Gateway");
   Print("========================================");

   // Verify API Gateway configuration
   if(EnableAPIGateway)
   {
      if(StringLen(APIGatewayURL) < 10 || StringFind(APIGatewayURL, "your-api") >= 0)
      {
         Print("❌ ERROR: Please configure APIGatewayURL");
         Print("   Example: https://your-api.railway.app");
         return INIT_FAILED;
      }

      if(StringLen(APIKey) < 20 || StringFind(APIKey, "YOUR_API_KEY") >= 0)
      {
         Print("❌ ERROR: Please configure APIKey");
         Print("   Format: mt5_terminal_xxx_xxxxxxxxxx");
         return INIT_FAILED;
      }
   ...
   // This ensures bars are captured even during low-tick periods
   ...
   Print("   New indicators: Dual TEMA H/L + Pinbar Detector");
   Print("========================================");
...
void OnDeinit(const int reason)
{
   ...
   Print("API Gateway Success: ", g_stats.apiSuccessCount);
   Print("API Gateway Failures: ", g_stats.apiFailureCount);
...
void OnTimer()
{
   ...
   // --- Periodic data collection sweep (every timer tick = every 5 min) ---
   int barsCollected = 0;
...
bool ParseSymbolsList()
{
   ...
   for(int i = 0; i < tokenCount; i++)
   {
      ...
      // Try to find actual symbol name (with potential suffix)
...
bool InitializeIndicatorsForSlot(int slotIndex, string sym, ENUM_TIMEFRAMES tf)
{
   ...
   // Buffer 0 = TEMAHighBuffer (TEMA of bar highs)
   // Buffer 1 = TEMALowBuffer  (TEMA of bar lows)
   ...
   // 12. Pinbar Detector
   // Buffer 0 = BullishPinbars, Buffer 1 = BearishPinbars
   // DisplayMode=0 (INDICATOR_BUFFERS) is required for CopyBuffer() to work.
   // The indicator's default DisplayMode=1 (DRAWING_OBJECTS) does not populate buffers.
   tfIndicators[slotIndex].h_pinbar = iCustom(
      sym, tf, "Pinbar Detector_Diamond Symbol_V17",
      InpPinbar_StringencyLevel,   // StringencyLevel (0=less, 1=baseline, 2=more)
      0,                           // DisplayMode = INDICATOR_BUFFERS (required for buffer access)
      InpPinbar_CountBars,         // CountBars
      5,                           // DisplayDistance
      false,                       // ShowStringencyInfo
      "=== LEFT EYE ANALYSIS ===", // LeftEyeSection (separator)
      false,                       // UseLeftEyeAnalysis
      false,                       // ShowLeftEyeMode
      false,                       // EnableDebugMode
      "=== PERMANENT OBJECTS ===", // ObjectSection (separator)
      false,                       // DeleteOldObjects
      200,                         // MaxObjectsToKeep
      clrLimeGreen,                // BullishColor
      clrRed,                      // BearishColor
      2,                           // ArrowSize
      "=== MANUAL OVERRIDE ===",   // ManualSection (separator)
      InpPinbar_UseManualSettings, // UseManualSettings
      InpPinbar_MinWickSize,       // ManualMinWickSize
      InpPinbar_MaxBodySize,       // ManualMaxNoseBodySize
      InpPinbar_BodyPosition,      // ManualNoseBodyPosition
      InpPinbar_Protruding,        // ManualNoseProtruding
      0.15,                        // ManualLeftEyeMinBodySize
      "=== ADVANCED FILTERS ===",  // AdvancedSection (separator)
      false,                       // LeftEyeOppositeDirection
      false,                       // NoseSameDirection
      false,                       // NoseBodyInsideLeftEyeBody
      1.0,                         // NoseBodyToLeftEyeBody
      0.0,                         // NoseLengthToLeftEyeLength
      0.1,                         // LeftEyeDepth
      1                            // MinimumNoseLength
   );
...
bool InitializeDatabases()
{
   ...
   for(int i = 0; i < symbolCount; i++)
   {
      ...
      // Migrate existing databases to add v2.26 columns if missing
...
//+------------------------------------------------------------------+
//| Migrate existing table to add v2.26 columns if missing            |
//+------------------------------------------------------------------+
void MigrateSymbolTable(int dbIndex)
{
   string tableName = symbolDatabases[dbIndex].sanitizedName;

   // Add dual_tema_high if it doesn't exist
   string addHighSQL = StringFormat(
      "ALTER TABLE [%s] ADD COLUMN dual_tema_high REAL", tableName
   );
   symbolDatabases[dbIndex].db.Exec(addHighSQL);  // Silently ignore if column already exists

   // Add dual_tema_low if it doesn't exist
   ...
   // Add pinbar if it doesn't exist
...
bool InsertCandle(int symbolIndex, int tfIndex)
{
   ...
   // --- v2.26: Dual TEMA High/Low ---
   // Buffer 0 = TEMAHighBuffer, Buffer 1 = TEMALowBuffer
   ...
   // --- v2.26: Pinbar Detection ---
   // Buffer 0 = BullishPinbars (non-zero price value when bullish pinbar detected)
   // Buffer 1 = BearishPinbars (non-zero price value when bearish pinbar detected)
   // Combined: 1 if any pinbar detected on this bar, 0 otherwise
   ...
   // ============================================================
   // PRIMARY PATH: Try API Gateway first (if enabled)
   // ============================================================
   if(EnableAPIGateway)
   {
      ...
      {
         // Circuit is open — skip API, go straight to SQLite backup
         // (Don't log every skip — too verbose)
      }
      else
      {
         bool apiSuccess = PublishToAPIGateway(
         ...
         if(apiSuccess)
         {
            // ✅ Success! Data validated and queued by API Gateway
         ...
         // Only log if circuit is not yet open (avoid log spam)
         if(!g_circuitBreaker.isOpen)
         {
            Print("⚠️ API Gateway failed for ", sanitizedName, " ", EnumToString(tf),
   ...
   // ============================================================
   // FALLBACK PATH: SQLite backup (only runs if API Gateway failed/skipped/disabled)
   // ============================================================
   ...
   // ❌ Both paths failed - critical error
   Print("❌ CRITICAL: Both API Gateway AND SQLite failed for ",
         sanitizedName, " ", EnumToString(tf));
   return false;
}

//+------------------------------------------------------------------+
//| Publish to API Gateway (Primary path) - COMPLETE VERSION         |
//+------------------------------------------------------------------+
bool PublishToAPIGateway(string symbol, ENUM_TIMEFRAMES tf, MqlRates &rate,
                        ...
                        double zigzag_peak, double zigzag_bottom, double ema_26,
                        double dual_tema_high, double dual_tema_low, int pinbar)
{
   // Build complete JSON payload with all columns + collected_at
   ...
   {
      // Success - data validated and queued
      return true;
   }
   else if(httpCode == 429)
   {
      // Rate limit hit - will retry via backfill
      Print("⚠️ Rate limited by API Gateway for ", symbol, " ", EnumToString(tf));
      return false;
   }
   else if(httpCode == 400)
   {
      // Validation error - data is bad, log for debugging
      string response = CharArrayToString(resultData);
      Print("❌ Validation error for ", symbol, " ", EnumToString(tf));
      Print("   API Response: ", response);
      return false;
   }
   else if(httpCode == 401 || httpCode == 403)
   {
      // Authentication error - critical configuration issue
      Print("❌ CRITICAL: API authentication failed (check APIKey)");
      return false;
   }
   else if(httpCode < 0)
   {
      // Network error (timeout, connection failed)
      // Don't log every network error - too verbose
      return false;
   }
   else
   {
      // Other error (5xx, etc.)
      Print("⚠️ API Gateway error for ", symbol, " ", EnumToString(tf),
            " (HTTP ", httpCode, ")");
      return false;
   }
}
...
//+------------------------------------------------------------------+
//| Write to SQLite backup (Fallback only) - COMPLETE VERSION        |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Mark bar for backfill — FIXED: append mode instead of truncate    |
//+------------------------------------------------------------------+
void MarkForBackfill(string symbol, ENUM_TIMEFRAMES tf, datetime timestamp)
{
   string csvPath = DatabasePath + "backfill_queue.csv";

   // FIX: Use FILE_READ|FILE_WRITE to open in append mode
   // v2.24 used FILE_WRITE only which truncates the file each time!
...
void PrintCollectionStats()
{
   ...
   if(totalAttempts > 0)
   {
      ...
      Print("API Gateway Success Rate: ", DoubleToString(successRate, 2), "%");
   }

   if(g_stats.lastAPIFailure > 0)
   {
      Print("Last API Gateway Failure: ", TimeToString(g_stats.lastAPIFailure));
   }
