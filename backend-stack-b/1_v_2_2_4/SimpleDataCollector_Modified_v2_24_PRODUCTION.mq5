...
void EnforceBarLimit(int slotIndex)
{
   ...
   int result = symbolDatabases[dbIndex].db.Query(countSQL);
   
   if(result != SQLITE_OK)
      return;
   
   int currentCount = (int)StringToInteger(symbolDatabases[dbIndex].db.GetFieldValue(0));
   symbolDatabases[dbIndex].db.FreeResult();
